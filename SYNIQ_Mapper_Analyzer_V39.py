...
(function() {
  ...
  function render(paths) {
    ...
    window.v39Export = function(){
      ...
      // V39.2: capture the CURRENT on-screen positions as-is. Do NOT re-tick
      // the simulation (that collapses nodes toward forceCenter and stacks
      // them). Instead, if the layout is still moving, ask the user to wait.
      try {
        if (typeof simulation !== 'undefined' && simulation && simulation.alpha) {
          var a = simulation.alpha();
          if (a > 0.02) {
            alert('The graph is still settling (alpha=' + a.toFixed(3) + '). '
                + 'Wait a few seconds until it stops moving, then click Export again.');
            return;
          }
        }
      } catch(e) {}
