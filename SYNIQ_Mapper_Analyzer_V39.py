"""
SYN-IQ Mapper Analyzer V38 — Streamlit App
...
(function() {
  ...
  function render(paths) {
    ...
    // ================= V39: SETTLED-SVG EXPORT (fit-to-frame) =================
    // Adds a floating button that serializes the LIVE settled graph to an SVG,
    // auto-fitting ALL nodes into a fixed frame so you never zoom or pan and
    // every question exports comparably. Honors window.V39_EXPORT.scale_mode:
    //   'fit'      -> each graph fills the frame (default)
    //   'absolute' -> node radius = k*sqrt(membership), identical across graphs
    (function addExportButton(){
      if (document.getElementById('v39-export-btn')) return;
      var btn = document.createElement('button');
      btn.id = 'v39-export-btn';
      btn.textContent = '\u{1F4F8} Export settled SVG';
      ...
      btn.onclick = function(){ if(window.v39Export) window.v39Export(); };
      document.body.appendChild(btn);
    })();

    window.v39Export = function(){
     try {
      var CFG = window.V39_EXPORT || {scale_mode:'fit', frame_w:1400, frame_h:1100, pad:60, abs_k:16};
      var srcSvg = document.querySelector('#canvas svg');
      if (!srcSvg) { alert('Graph not ready yet.'); return; }

      // V39.3: capture CURRENT on-screen positions. The pies and this export
      // both read d.x/d.y, so if the graph looks settled on screen it will
      // export settled. The failure mode is exporting while nodes are still
      // piled near the center (unsettled): they stack and only one pie shows.
      // So we DETECT that directly by measuring how spread out the nodes are,
      // and refuse (with a visible on-page banner, since iframe alerts can be
      // swallowed) if they haven't spread yet.
      try { if (typeof updatePies === 'function') updatePies(); } catch(e) {}
      (function(){
        var xs=[], ys=[];
        srcSvg.querySelectorAll('path, circle').forEach(function(el){
          var d=el.__data__;
          if(d && typeof d.x==='number' && typeof d.y==='number'){ xs.push(d.x); ys.push(d.y); }
        });
        if(xs.length<3) return;
        function spread(a){ var mn=Math.min.apply(null,a), mx=Math.max.apply(null,a); return mx-mn; }
        var sx=spread(xs), sy=spread(ys);
        // unsettled: nodes occupy a tiny area relative to how many there are
        if (Math.max(sx,sy) < 120) {
          var warn=document.getElementById('v39-warn');
          if(!warn){ warn=document.createElement('div'); warn.id='v39-warn';
            warn.style.cssText='position:fixed;top:56px;left:14px;z-index:99999;padding:10px 14px;'
              +'background:#b3261e;color:#fff;font-family:sans-serif;font-size:13px;border-radius:6px;'
              +'max-width:320px;box-shadow:0 2px 6px rgba(0,0,0,.25);';
            document.body.appendChild(warn);
          }
          warn.textContent='\u26A0 The graph has not finished spreading out yet '
            +'(nodes still clustered). Wait until it stops moving on screen, then '
            +'click Export again.';
          warn.style.display='block';
          setTimeout(function(){ if(warn) warn.style.display='none'; }, 6000);
          throw {v39_unsettled:true};
        }
      })();

      // 1) gather nodes (settled positions from bound data) and edges
      var nodes = [], byId = {};
      srcSvg.querySelectorAll('path, circle').forEach(function(el){
        var d = el.__data__;
        if (!d || typeof d.x !== 'number' || typeof d.y !== 'number') return;
        var id = d.id || d.name || (''+d.x+'_'+d.y);
        if (byId[id]) return; byId[id]=1;
        nodes.push(d);
      });
      if (!nodes.length){ alert('No settled nodes found.'); return; }

      // 2) bounding box of node CENTERS + radii
      function rawR(d){ var sz=d.size||1, mod=d.size_modifier||10;
        return Math.sqrt(Math.max(sz*50*mod,1)/Math.PI) * (d.v21_pie_radius_scale||1.4); }
      // absolute mode overrides radius with k*sqrt(membership)
      function membership(d){
        if (d.v22_panel && typeof d.v22_panel.size==='number') return d.v22_panel.size;
        return d.size||1;
      }
      function nodeR(d){
        if (CFG.scale_mode==='absolute') return CFG.abs_k * Math.sqrt(Math.max(membership(d),1));
        return rawR(d);
      }
      var x0=1e9,y0=1e9,x1=-1e9,y1=-1e9;
      nodes.forEach(function(d){ var r=nodeR(d);
        x0=Math.min(x0,d.x-r); y0=Math.min(y0,d.y-r);
        x1=Math.max(x1,d.x+r); y1=Math.max(y1,d.y+r); });
      var gw=x1-x0, gh=y1-y0;

      // 3) fit transform: scale the graph to fill frame (minus padding), centered
      var FW=CFG.frame_w, FH=CFG.frame_h, PAD=CFG.pad;
      var sc = Math.min((FW-2*PAD)/gw, (FH-2*PAD)/gh);
      var tx = PAD + (FW-2*PAD - gw*sc)/2 - x0*sc;
      var ty = PAD + (FH-2*PAD - gh*sc)/2 - y0*sc;
      function TX(x){ return x*sc+tx; }
      function TY(y){ return y*sc+ty; }

      // 4) build clean SVG: white bg, edges, then agent-colored pies
      var AGC = {'#377eb8':1,'#4daf4a':1,'#984ea3':1,'#e41a1c':1};
      var out = ['<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 '+FW+' '+FH+'" width="'+FW+'" height="'+FH+'">'];
      out.push('<rect width="'+FW+'" height="'+FH+'" fill="white"/>');
      // edges: read the live line elements, map endpoints through transform
      srcSvg.querySelectorAll('line').forEach(function(ln){
        var d=ln.__data__; var x1p,y1p,x2p,y2p;
        if (d && d.source && d.target && typeof d.source.x==='number'){
          x1p=d.source.x; y1p=d.source.y; x2p=d.target.x; y2p=d.target.y;
        } else {
          x1p=+ln.getAttribute('x1'); y1p=+ln.getAttribute('y1');
          x2p=+ln.getAttribute('x2'); y2p=+ln.getAttribute('y2');
        }
        if ([x1p,y1p,x2p,y2p].some(function(v){return isNaN(v);})) return;
        out.push('<line x1="'+TX(x1p).toFixed(1)+'" y1="'+TY(y1p).toFixed(1)+'" x2="'
          +TX(x2p).toFixed(1)+'" y2="'+TY(y2p).toFixed(1)+'" stroke="#bcbcbc" stroke-width="'+(0.8*sc).toFixed(2)+'"/>');
      });
      // pies: draw each node's wedges as SVG arcs at fitted position + fitted radius
      function arc(cx,cy,r,a0,a1){
        var x0a=cx+r*Math.cos(a0), y0a=cy+r*Math.sin(a0);
        var x1a=cx+r*Math.cos(a1), y1a=cy+r*Math.sin(a1);
        var large=(a1-a0)>Math.PI?1:0;
        return 'M'+cx.toFixed(1)+','+cy.toFixed(1)+' L'+x0a.toFixed(1)+','+y0a.toFixed(1)
          +' A'+r.toFixed(1)+','+r.toFixed(1)+' 0 '+large+' 1 '+x1a.toFixed(1)+','+y1a.toFixed(1)+' Z';
      }
      nodes.forEach(function(d){
        var cx=TX(d.x), cy=TY(d.y), r=nodeR(d)*sc;
        var wedges=d.v21_wedges||[{color:'#999',frac:1}];
        var start=-Math.PI/2;
        wedges.forEach(function(w){
          if (!w.frac || w.frac<=0) return;
          var end=start + w.frac*2*Math.PI;
          out.push('<path d="'+arc(cx,cy,r,start,end)+'" fill="'+w.color+'" stroke="#fff" stroke-width="'+(0.4*sc).toFixed(2)+'"/>');
          start=end;
        });
      });
      out.push('</svg>');

      // 5) deliver the SVG. Streamlit runs the graph in a SANDBOXED iframe,
      // which silently blocks blob downloads. So we (a) flash a visible
      // confirmation that the handler ran, and (b) open the SVG in a NEW TAB
      // (not sandbox-blocked) where the user can right-click > Save As. We also
      // attempt a normal download for browsers/contexts that allow it.
      var svgText = out.join('\n');
      var ttl = (document.title||'mapper').replace(/[^a-z0-9]+/gi,'_').slice(0,60);

      // visible confirmation flash
      var flash = document.getElementById('v39-flash');
      if(!flash){ flash=document.createElement('div'); flash.id='v39-flash';
        flash.style.cssText='position:fixed;top:56px;left:14px;z-index:99999;padding:8px 12px;'
          +'background:#1a7f37;color:#fff;font-family:sans-serif;font-size:12px;border-radius:6px;'
          +'box-shadow:0 2px 6px rgba(0,0,0,.2);';
        document.body.appendChild(flash);
      }
      flash.textContent='\u2713 SVG built ('+svgText.length+' bytes) — opening in new tab';
      flash.style.display='block';
      setTimeout(function(){ if(flash) flash.style.display='none'; }, 4000);

      // open in a new tab via data URL (works inside sandbox with allow-popups)
      try {
        var dataUrl = 'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svgText);
        var w = window.open();
        if (w && w.document) {
          w.document.write('<title>'+ttl+'_settled.svg</title>'
            + '<p style="font-family:sans-serif;font-size:13px">Right-click the image and choose '
            + '&quot;Save Image As&quot; to save the SVG.</p>'
            + '<img src="'+dataUrl+'" style="max-width:100%;border:1px solid #ccc"/>'
            + '<hr><textarea style="width:100%;height:200px">'+svgText.replace(/</g,'&lt;')+'</textarea>');
        } else {
          // popup blocked — fall back to replacing our own location won't work in iframe;
          // show the SVG text in a textarea overlay to copy-paste
          showTextFallback(svgText, ttl);
        }
      } catch(e) {
        showTextFallback(svgText, ttl);
      }

      // also try a normal download (harmless if blocked)
      try {
        var blob=new Blob([svgText],{type:'image/svg+xml'});
        var url=URL.createObjectURL(blob);
        var a=document.createElement('a'); a.href=url; a.download=ttl+'_settled.svg';
        document.body.appendChild(a); a.click();
        setTimeout(function(){URL.revokeObjectURL(url);a.remove();},1000);
      } catch(e){}

      function showTextFallback(txt, name){
        var box=document.getElementById('v39-fallback');
        if(!box){ box=document.createElement('div'); box.id='v39-fallback';
          box.style.cssText='position:fixed;top:90px;left:14px;right:14px;z-index:99998;'
            +'background:#fff;border:2px solid #1a7f37;border-radius:8px;padding:12px;';
          document.body.appendChild(box);
        }
        box.innerHTML='<b style="font-family:sans-serif">Copy this SVG and save as .svg</b>'
          +'<button onclick="this.parentNode.style.display=\'none\'" style="float:right">close</button>'
          +'<textarea style="width:100%;height:240px;margin-top:8px">'+txt.replace(/</g,'&lt;')+'</textarea>';
        box.style.display='block';
      }
     } catch(_e){ if(!(_e && _e.v39_unsettled)) { console.error('v39 export error', _e); } }
    }
  }
...
# ── V39 — SETTLED-SVG EXPORT ─────────────────────────────────────────────
# A button on the graph serializes the live settled layout to SVG, auto-fitting
# ALL nodes into a fixed frame (no zoom/pan needed, and every question exports
# comparably). The scale toggle chooses how node size is treated:
st.sidebar.markdown("---")
st.sidebar.markdown("### 📸 Settled export")
_v39_scale = st.sidebar.radio(
    "Node scaling for export:",
    ["Fit to frame", "Absolute (comparable across graphs)"],
    index=0,
    help="Fit to frame: each graph fills the frame (best for a single clean "
         "figure). Absolute: a node's size = k·sqrt(membership), identical in "
         "every graph, so node sizes are directly comparable across the five "
         "questions. Both auto-fit all nodes into the frame so you never zoom."
)
_v39_absk = 16
if _v39_scale.startswith("Absolute"):
    _v39_absk = st.sidebar.slider("Absolute size k", 6, 40, 16, key="v39_absk",
        help="Pixel radius per sqrt(membership). Use the SAME value across all "
             "five questions so sizes stay comparable.")
_v39_export_cfg = {
    "scale_mode": "absolute" if _v39_scale.startswith("Absolute") else "fit",
    "frame_w": 1400, "frame_h": 1100, "pad": 60, "abs_k": int(_v39_absk),
}
        ...
        if st.session_state.mapper_analysis_data is not None:
            ...
            if st.session_state.mapper_results_html:
                ...
                try:
                    ...
                    # V39: inject the settled-export config too (live-refresh)
                    _viz_html = _re32.sub(r"<script>window\.V39_EXPORT\s*=.*?</script>", "", _viz_html, flags=_re32.DOTALL)
                    _exp_cfg = "<script>window.V39_EXPORT = " + _json32.dumps(_v39_export_cfg) + ";</script>"
                    _viz_html = _viz_html.replace("</body>", _exp_cfg + "</body>", 1)
