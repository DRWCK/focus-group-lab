...
(function() {
  ...
  function render(paths) {
    ...
    window.v39Export = function(){
      ...
      // V39.1: force the layout to FULLY SETTLE before capturing, so it never
      // matters when you click. Run the simulation to convergence, then read
      // the final node positions. Without this, an early click captured a
      // half-spread graph.
      try {
        if (typeof simulation !== 'undefined' && simulation && simulation.tick) {
          simulation.stop();
          for (var _i=0; _i<400; _i++) { simulation.tick(); }
          if (typeof updatePies === 'function') updatePies();
        }
      } catch(e) { /* if no sim handle, capture as-is */ }
