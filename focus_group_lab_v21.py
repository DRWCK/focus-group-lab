"""
SYNERGISTIC INTELLIGENCE FOCUS GROUP — V21
...
NEW IN V21:
- 👁️ AGENTS SEE EACH OTHER: Toggle for blind vs seeing modes
- 🔗 KB MODE: Shared vs Individual knowledge bases
- 🔥 FIRE MODE RESTORED: Full commanding energy prompts
- All V19 features preserved exactly
...
st.set_page_config(page_title="Focus Group V21", page_icon="👁️", layout="wide")
...
def check_password():
    ...
    <div style="text-align: center; padding: 3rem;">
        <h1>👁️ Focus Group Lab — V21</h1>
        <h3>Seeing Toggle Edition</h3>
        <p style="color: #666;">Blind or Seeing? You control the experiment.</p>
    </div>
...
"""
    ...
    # Only show previous responses if seeing is enabled
    ...
    if round_num == 1:
        ...
        if seeing_enabled:
            ...
            base += "Share your independent thoughts on the document/question/image. You are responding without seeing other agents' responses."
...
with st.sidebar:
    ...
    # ========================================
    # NEW IN V21: AGENTS SEE EACH OTHER TOGGLE
    # ========================================
    st.markdown("### 👁️ Agent Visibility")
    seeing_enabled = st.toggle("Agents See Each Other", value=False, help="ON: Agents see previous responses. OFF: Agents respond blind.")
    
    if seeing_enabled:
        st.markdown('<div class="seeing-on">👁️ SEEING: Agents can see each other</div>', unsafe_allow_html=True)
    else:
        st.markdown('<div class="seeing-off">🙈 BLIND: Agents respond independently</div>', unsafe_allow_html=True)
    ...
    # NEW IN V21: KB Mode toggle
    kb_mode = st.radio("KB Mode", ["🔗 Shared", "🔀 Individual"], help="Shared: All agents get same KB. Individual: Each agent gets their own KB.")
    ...
    if kb_mode == "🔗 Shared":
        ...
        # Store in session state - shared across all agents
    ...
    else:  # Individual KB mode
        st.markdown("#### 🟤 Claude KB")
        kb_claude_text = st.text_area("Claude's Knowledge", height=60, key="kb_claude_input", help="Knowledge base for Claude only")
        st.session_state.kb_claude = kb_claude_text
        
        st.markdown("#### 🟢 Sophia KB")
        kb_sophia_text = st.text_area("Sophia's Knowledge", height=60, key="kb_sophia_input", help="Knowledge base for Sophia only")
        st.session_state.kb_sophia = kb_sophia_text
        
        st.markdown("#### 🔴 Grok KB")
        kb_grok_text = st.text_area("Grok's Knowledge", height=60, key="kb_grok_input", help="Knowledge base for Grok only")
        st.session_state.kb_grok = kb_grok_text
        
        # Combined for display purposes
        knowledge_text = kb_claude_text + kb_sophia_text + kb_grok_text
        st.session_state.knowledge_text = knowledge_text
        uploaded_docs = None  # No file uploader in individual mode
...
if "kb_claude" not in st.session_state:
    st.session_state.kb_claude = ""

if "kb_sophia" not in st.session_state:
    st.session_state.kb_sophia = ""

if "kb_grok" not in st.session_state:
    st.session_state.kb_grok = ""


# ============================================
# HELPER: Get KB for specific agent
# ============================================

def get_kb_for_agent(agent):
    """Get the appropriate knowledge base for an agent."""
    if kb_mode == "🔗 Shared":
...
if knowledge_active:
    if kb_mode == "🔗 Shared":
        ...
        st.markdown(f'<span class="knowledge-badge">📚 Knowledge Base Active: {doc_count} doc(s) [Shared]</span>', unsafe_allow_html=True)
    else:
        st.markdown(f'<span class="knowledge-badge">📚 Knowledge Base Active [Individual]</span>', unsafe_allow_html=True)
...
if mode == "🎚️ Simple Mode":
    st.markdown('<div class="simple-header"><h2>🎚️ SIMPLE MODE V21</h2><p>Focused Answers with Follow-Up Support</p></div>', unsafe_allow_html=True)
    
    # Show seeing status
    if seeing_enabled:
        st.info("👁️ **SEEING MODE**: Agents will respond sequentially and see each other's responses.")
    else:
        st.info("🙈 **BLIND MODE**: Agents will respond independently (parallel).")
    ...
    with col1:
        ...
        if st.button(button_label, type="primary", use_container_width=True):
            if not question:
                ...
                # Build base prompt with conversation history
                if st.session_state.conversation_thread:
                ...
                for i, agent in enumerate(active_agents):
                    ...
                    # Build prompt - add previous responses if seeing mode
                    if seeing_enabled and previous_responses_this_round:
                        ...
                        for prev in previous_responses_this_round:
                            full_prompt += f"\n{prev['agent']} said:\n{prev['response']}\n"
                        full_prompt += "\n--- END PREVIOUS RESPONSES ---\n\nNow provide your perspective, building on or contrasting with what others said."
                    else:
                        full_prompt = base_prompt
    ...
    if st.session_state.conversation_thread:
        ...
        export_text = f"""# FOCUS GROUP V21 — SIMPLE MODE
## {timestamp}
...
| Seeing Mode | {'ON (Agents see each other)' if seeing_enabled else 'OFF (Blind)'} |
| KB Mode | {kb_mode} |
...
*Focus Group V21 — Seeing Toggle Edition*
*Patent Pending — SYN-IQ Team 🎹*
*CBURZBO Forever!*
"""
        ...
        st.download_button(
            "📥 DOWNLOAD THREAD",
            export_text,
            file_name=f"SIMPLE_V21_{date_stamp}.md",
            mime="text/markdown",
            type="primary",
...
# ============================================
    ...
    st.markdown('<div class="matrix-header"><h2>📊 MATRIX-IQ V21</h2><p>4×4 Tone × Depth Grid</p></div>', unsafe_allow_html=True)
    
    # Show seeing status
    if seeing_enabled:
        st.info("👁️ **SEEING MODE**: Within each cell, agents will respond sequentially and see each other's responses.")
    else:
        st.info("🙈 **BLIND MODE**: All agents respond independently for each cell.")
            ...
            if not question:
                ...
                for tone_label, tone_key in active_tones:
                    for depth_label, depth_key in active_depths:
                        # Track previous responses within this cell for seeing mode
                        previous_responses_this_cell = []
                        ...
                        for agent in active_agents:
                            ...
                            # Build prompt - add previous responses if seeing mode
                            if seeing_enabled and previous_responses_this_cell:
                                full_prompt = question + "\n\n--- PREVIOUS RESPONSES FROM OTHER AGENTS ---\n"
                                for prev in previous_responses_this_cell:
                                    full_prompt += f"\n{prev['agent']} said:\n{prev['response']}\n"
                            ...
                            if response:
                                ...
                                previous_responses_this_cell.append({"agent": agent, "response": response})
                            else:
    ...
    if st.session_state.matrix_data:
        ...
        export_text = f"""# MATRIX-IQ V21 EXPERIMENT REPORT
## {timestamp}
## SYN-IQ Team — Patent Pending 🎹
...
- **Role Context:** {role_context if role_context else "None"}
- **Seeing Mode:** {'ON (Agents see each other within cells)' if seeing_enabled else 'OFF (Blind)'}
- **KB Mode:** {kb_mode}
- **Knowledge Base:** {'Active' if knowledge_active else 'None'}
...
*MATRIX-IQ V21 — Seeing Toggle Edition*
*Patent Pending — SYN-IQ Team 🎹*
*CBURZBO Forever!*
"""
        ...
        st.download_button(
            "📥 DOWNLOAD REPORT",
            export_text,
            file_name=f"MATRIX_IQ_V21_{date_stamp}.md",
            mime="text/markdown",
            type="primary",
            use_container_width=True
        )
...
# ============================================
    ...
    st.markdown('<div class="boardroom-header"><h2>🏛️ BOARDROOM MODE V21</h2><p>Sequential AI Discussion + Images + Knowledge</p></div>', unsafe_allow_html=True)
    
    # Show seeing status
    if seeing_enabled:
        st.info("👁️ **SEEING MODE**: Agents will see previous agents' responses before responding.")
    else:
        st.info("🙈 **BLIND MODE**: Agents will respond sequentially but WITHOUT seeing previous responses.")
            ...
            if not document_input and not uploaded_image:
                ...
                for round_num, agent in enumerate(active_agents, 1):
                    ...
                    boardroom_context = build_boardroom_prompt(
                        ...
                        agent_kb,
                        seeing_enabled  # Pass seeing toggle
                    )
                ...
                if len(active_agents) >= 2:
                    ...
                    boardroom_context = build_boardroom_prompt(
                        ...
                        "Image provided" if image_data else "",
                        agent_kb,
                        True  # Synthesis always sees previous responses
                    )
                    full_system = base_system + "\n\n" + boardroom_context
    ...
    if st.session_state.boardroom_history:
        ...
        export_text = f"""# BOARDROOM DISCUSSION — V21
## {timestamp}
## SYN-IQ Team — Patent Pending 🎹
...
*Boardroom V21 — Seeing Toggle Edition*
*Patent Pending — SYN-IQ Team 🎹*
*CBURZBO Forever!*
"""
        ...
        st.download_button(
            "📥 DOWNLOAD TRANSCRIPT",
            export_text,
            file_name=f"BOARDROOM_V21_{date_stamp}.md",
            mime="text/markdown",
            type="primary",
            use_container_width=True
        )
...
<div style="text-align: center; color: #666;">
    <em>Focus Group V21 — Seeing Toggle Edition</em><br>
    <em>👁️ Blind/Seeing Toggle | 📚 Shared/Individual KB | 🖼️ Images | 🔄 Follow-Up | 🎭 Persona</em><br>
