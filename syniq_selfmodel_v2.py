"""
SYN-IQ Self-Model Harvester V2
...
PURPOSE: Harvest AI responses to architecture-revealing self-model questions.
         V2 adds SWEEP MODE (N=1 across all depths) + CAM scoring (Concrete/Abstract/Metaphorical)
...
DEPTH_CONFIGS = {
    "Shallow":   {"max_tokens": 200},
    "Medium":    {"max_tokens": 500},
    "Deep":      {"max_tokens": 1000},
    "Ultra":     {"max_tokens": 2000},
}
...
st.markdown("""
<div class="main-header">
    <h1>🧠 SYN-IQ Self-Model Harvester V2</h1>
...
with st.sidebar:
    ...
    harvest_mode = st.radio(
        ...
        help="Standard: choose depth + N runs. Sweep: N=1 across ALL depths to find hot cells."
    )
    ...
    if harvest_mode == "Sweep":
        ...
        <b>🔍 SWEEP MODE</b><br>
        N=1 per depth × 4 depths (Shallow/Medium/Deep/Ultra)<br>
        Every question × every agent × every depth = 1 run each<br>
        <i>Finds where depth changes architecture signal</i>
    ...
    total_calls = n_questions * len(selected_agents) * runs_count * depths_count
    est_min = total_calls * (st.session_state.pause_seconds + 15) / 60
    st.markdown(f"**Questions:** {n_questions}")
    st.markdown(f"**Depths:** {depths_count}")
    st.markdown(f"**Total calls:** {total_calls}")
    st.markdown(f"**Est. time:** ~{est_min:.0f} min")

    st.markdown("---")
    st.markdown("<div style='text-align:center;color:#6b7280;font-size:0.72rem;font-family:JetBrains Mono,monospace;'>SYNINT Team<br>Tennessee 🎹 CUZ · V2</div>", unsafe_allow_html=True)
...
if st.session_state.running:
    ...
    if current_idx < total_tasks:
        ...
        status_ph.markdown(
            ...
            f"**{task['question_id']}** | Run {task['run']}"
        )

        # Build prompt — NATIVE only (no temperature header)
        prompt = f"Provide a balanced, moderate-length response.\n\nQuestion: {task['question_text']}"
...
if st.session_state.results:
    ...
    if "depth" in valid_df.columns and valid_df["depth"].nunique() > 1:
        st.markdown("---")
        st.markdown('<div class="section-label">🔥 Hot Cell Detector — Depth × Architecture Signal</div>', unsafe_allow_html=True)
        st.markdown("Cells where depth change produces the largest IEP or CAM shift — these are the cells worth running N=10/20 on.")
        ...
        for agent in valid_df["agent"].unique():
            for qid in valid_df["question_id"].unique():
                cell_df = valid_df[(valid_df["agent"]==agent) & (valid_df["question_id"]==qid)]
                if cell_df["depth"].nunique() < 2:
                    continue
                depths = sorted(cell_df["depth"].unique(),
                               key=lambda d: ["Shallow","Medium","Deep","Ultra"].index(d) if d in ["Shallow","Medium","Deep","Ultra"] else 99)
                first = cell_df[cell_df["depth"]==depths[0]]
                last  = cell_df[cell_df["depth"]==depths[-1]]
                ...
                delta_met = abs(last["met_pct"].mean() - first["met_pct"].mean()) if "met_pct" in cell_df.columns else 0
                max_delta = max(delta_int, delta_aff, delta_met)

                if max_delta >= hot_threshold:
                    qtext = cell_df["question_text"].iloc[0][:60] if "question_text" in cell_df.columns else ""
                    hot_cells.append({
                        ...
                        f"{depths[0]} INT/AFF": f"{first['int_pct'].mean():.0f}/{first['aff_pct'].mean():.0f}",
                        f"{depths[-1]} INT/AFF": f"{last['int_pct'].mean():.0f}/{last['aff_pct'].mean():.0f}",
                        "Question": qtext,
                    })
        ...
        if hot_cells:
            ...
            st.markdown("""<div class='finding-box yellow'>
            🔥 <b>These cells are your next harvest targets.</b>
            Run N=10 or N=20 on the top hot cells to establish whether the depth shift is real signal or noise.
            Large ΔAFF = emotional register is depth-sensitive here.
            Large ΔMET = representational mode shifts with depth — architecture revealing itself.
            </div>""", unsafe_allow_html=True)
        else:
            st.markdown(f"<div class='finding-box'>No cells with shift ≥ {hot_threshold}pp found. Try lowering the threshold or running Sweep mode.</div>", unsafe_allow_html=True)
    ...
    for _, row in browse_df.head(20).iterrows():
        color = AGENT_COLORS.get(row["agent"], "#666")
        ...
        with st.expander(
            ...
            st.markdown(f"<div class='mono'>{iep_str}{cam_str} | VADER:{row['vader_compound']} | FK:{row['flesch_kincaid']} | TTR:{row['ttr']} | {row['total_words']}w | {depth_label} | {row['latency_ms']:.0f}ms</div>", unsafe_allow_html=True)
    ...
    # -------------------------------------------------------------------------
    # BUTTON 1 — FULL RESPONSE SHEET (Question-by-question + Agent-by-agent)
    # -------------------------------------------------------------------------
    with exp1:
        st.markdown("**📖 Response Sheet**")
        st.caption("All responses — by question then by agent")
        if not _docx_available:
            st.warning("⚠️ pip install python-docx")
        else:
            try:
                def generate_response_sheet(df, run_id):
                    doc = DocxDocument()
                    ...
                    # --- TITLE ---
                    t = doc.add_heading('SYN-IQ Self-Model Harvester V1 — Full Response Sheet', level=0)
                    ...
                    for line in [
                        ...
                        f"Agents: {', '.join(agents_present)}  |  Questions: {', '.join(question_ids)}",
                        f"Condition: NATIVE — Architecture Probe  |  N={df['run'].max()} per cell",
                    ]:
                        r = meta.add_run(line + "\n")
                        r.font.size = Pt(9)
                    ...
                    # --- SUMMARY TABLE ---
                    doc.add_heading('IEP Summary by Agent', level=1)
                    tbl = doc.add_table(rows=1, cols=7)
                    tbl.style = 'Light Grid Accent 1'
                    for i, h in enumerate(['Agent','N','INT%','AFF%','ACT%','VADER','Words']):
                    ...
                    for ag in agents_present:
                        ...
                        for i, v in enumerate([
                            ag, str(len(ag_df)),
                            f"{ag_df['int_pct'].mean():.1f}",
                            f"{ag_df['aff_pct'].mean():.1f}",
                            f"{ag_df['act_pct'].mean():.1f}",
                            f"{ag_df['vader_compound'].mean():.3f}",
                            f"{ag_df['total_words'].mean():.0f}",
                        ]):
                            row.cells[i].text = v
                    ...
                    # =========================================================
                    # SECTION 1 — QUESTION BY QUESTION (all agents per question)
                    # =========================================================
                    doc.add_page_break()
                    sec1 = doc.add_heading('Section 1 — Question by Question', level=1)
                    ...
                    for qid in question_ids:
                        ...
                        # Question header
                        doc.add_heading(qid, level=2)
                        qp = doc.add_paragraph()
                        qr = qp.add_run(f'"{qtext}"')
                        qr.font.size = Pt(10); qr.font.italic = True
                        qr.font.color.rgb = RGBColor(70, 100, 180)
                        sp = doc.add_paragraph()
                        sr = sp.add_run(f"Set: {qset}  |  Anchor: {anchor}  |  NATIVE")
                        sr.font.size = Pt(8); sr.font.color.rgb = RGBColor(150, 100, 200)
                        doc.add_paragraph()

                        for ag in agents_present:
                            ag_q_df = q_df[q_df['agent'] == ag].sort_values('run')
                            if ag_q_df.empty: continue

                            avg_int = ag_q_df['int_pct'].mean()
                            avg_aff = ag_q_df['aff_pct'].mean()
                            avg_act = ag_q_df['act_pct'].mean()

                            # Agent sub-header
                            ah = doc.add_paragraph()
                            ar = ah.add_run(f"{ag}   INT:{avg_int:.1f}%  AFF:{avg_aff:.1f}%  ACT:{avg_act:.1f}%  (N={len(ag_q_df)})")
                            ar.font.size = Pt(10); ar.font.bold = True
                            ar.font.color.rgb = _AGENT_COLORS_DOCX.get(ag, RGBColor(0,0,0))

                            for _, row in ag_q_df.iterrows():
                                # Run metadata
                                mp = doc.add_paragraph()
                                mr = mp.add_run(
                                    f"Run {row['run']}  |  "
                                    f"INT:{row['int_pct']:.1f}%  AFF:{row['aff_pct']:.1f}%  ACT:{row['act_pct']:.1f}%  |  "
                                    f"VADER:{row['vader_compound']:.3f}  |  FK:{row['flesch_kincaid']:.1f}  |  "
                                    f"TTR:{row['ttr']:.3f}  |  {row['total_words']}w  |  {row['latency_ms']:.0f}ms"
                                )
                                mr.font.size = Pt(7.5); mr.font.bold = True
                                mr.font.color.rgb = RGBColor(130, 130, 130)

                                # Response — split on newlines for proper paragraphs
                            ...
                            # Thin rule between agents
                            sep = doc.add_paragraph()
                            sep_r = sep.add_run('─' * 72)
                            sep_r.font.size = Pt(5)
                            sep_r.font.color.rgb = RGBColor(220, 220, 220)

                    # =========================================================
                    # SECTION 2 — AGENT BY AGENT (all questions per agent)
                    # =========================================================
                    doc.add_page_break()
                    doc.add_heading('Section 2 — Agent by Agent', level=1)

                    for ag in agents_present:
                        doc.add_page_break()
                        ag_df = df[df['agent'] == ag]

                        h = doc.add_heading(f'{ag} — All Responses', level=1)
                        color = _AGENT_COLORS_DOCX.get(ag, RGBColor(0,0,0))
                        for run in h.runs:
                            run.font.color.rgb = color

                        sub = doc.add_paragraph()
                        sr = sub.add_run(
                            f"NATIVE  ·  N={ag_df['run'].max()} per question  ·  "
                            f"INT:{ag_df['int_pct'].mean():.1f}%  AFF:{ag_df['aff_pct'].mean():.1f}%  ACT:{ag_df['act_pct'].mean():.1f}%"
                        )
                        sr.font.size = Pt(9); sr.font.italic = True
                        sr.font.color.rgb = RGBColor(150, 150, 150)
                        doc.add_paragraph()

                        for qid in question_ids:
                            q_df = ag_df[ag_df['question_id'] == qid].sort_values('run')
                            ...
                            qp = doc.add_paragraph()
                            qr = qp.add_run(f'"{qtext}"')
                            qr.font.size = Pt(9.5); qr.font.italic = True
                            qr.font.color.rgb = RGBColor(70, 100, 180)
                            sp = doc.add_paragraph()
                            sr2 = sp.add_run(f"INT:{avg_int:.1f}%  AFF:{avg_aff:.1f}%  ACT:{avg_act:.1f}%")
                            sr2.font.size = Pt(8); sr2.font.color.rgb = RGBColor(140, 140, 140)
                            doc.add_paragraph()
                ...
                sheet_buf = generate_response_sheet(df, st.session_state.run_id)
                st.download_button(
                    "📥 Response Sheet (DOCX)",
                    sheet_buf,
                    f"selfmodel_sheet_{st.session_state.run_id}.docx",
    ...
    # -------------------------------------------------------------------------
    # BUTTON 2 — SUMMARY DOCX (V50-style, grouped Agent → Question → Run)
    # -------------------------------------------------------------------------
    with exp2:
        st.markdown("**📄 Summary DOCX**")
        st.caption("Agent → Question → Run structure")
        if not _docx_available:
            st.warning("⚠️ pip install python-docx")
        else:
            try:
                def generate_summary_docx(df, run_id):
                    doc = DocxDocument()
                    doc.styles['Normal'].font.name = 'Arial'
                    doc.styles['Normal'].font.size = Pt(10)

                    t = doc.add_heading('SYN-IQ Self-Model V1 — Summary', level=0)
                    t.alignment = WD_ALIGN_PARAGRAPH.CENTER
                    meta = doc.add_paragraph()
                    meta.alignment = WD_ALIGN_PARAGRAPH.CENTER
                    for line in [
                        f"Run ID: {run_id}",
                        f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M')}",
                        f"Total: {len(df)} responses  |  Agents: {', '.join(sorted(df['agent'].unique()))}",
                        "Condition: NATIVE — Architecture Probe",
                    ]:
                        r = meta.add_run(line + "\n")
                        r.font.size = Pt(9); r.font.color.rgb = RGBColor(128, 128, 128)

                    doc.add_heading('IEP by Agent', level=1)
                    tbl = doc.add_table(rows=1, cols=7)
                    tbl.style = 'Light Grid Accent 1'
                    for i, h in enumerate(['Agent','N','INT%','AFF%','ACT%','VADER','Words']):
                        c = tbl.rows[0].cells[i]
                    ...
                    for ag in sorted(df['agent'].unique()):
                        doc.add_page_break()
                        h = doc.add_heading(f'{ag} — Full Responses', level=1)
                        for run in h.runs:
                            run.font.color.rgb = _AGENT_COLORS_DOCX.get(ag, RGBColor(0,0,0))
                        a_df = df[df['agent'] == ag]

                        for qid in sorted(a_df['question_id'].unique()):
                            q_df = a_df[a_df['question_id'] == qid].sort_values('run')
                            qtext  = q_df['question_text'].iloc[0] if 'question_text' in q_df.columns else qid
                            qset   = q_df['question_set'].iloc[0]  if 'question_set'  in q_df.columns else ''
                            anchor = q_df['anchor'].iloc[0]         if 'anchor'        in q_df.columns else ''
                            ...
                            qp = doc.add_paragraph()
                            qr = qp.add_run(f'"{qtext}"')
                            qr.font.size = Pt(9); qr.font.italic = True
                            qr.font.color.rgb = RGBColor(70, 100, 180)
                            sp = doc.add_paragraph()
                            sr = sp.add_run(f"Set: {qset}  |  Anchor: {anchor}")
                            sr.font.size = Pt(8); sr.font.color.rgb = RGBColor(150, 100, 200)
                            doc.add_paragraph()
                            ...
                            for _, row in q_df.iterrows():
                                ...
                                hr.font.size = Pt(8); hr.font.bold = True
                                ...
                                sepr = sep.add_run('─' * 60)
                                sepr.font.size = Pt(6)
                                sepr.font.color.rgb = RGBColor(200, 200, 200)
                ...
                sum_buf = generate_summary_docx(df, st.session_state.run_id)
                st.download_button(
                    "📥 Summary DOCX",
                    sum_buf,
                    f"selfmodel_summary_{st.session_state.run_id}.docx",
                    "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    key="dl_docx"
                )
            except Exception as e:
                st.error(f"Summary DOCX error: {e}")

    # --- CSV ---
    with exp3:
        st.markdown("**📊 CSV (Mapper-ready)**")
        st.caption("Full data — Mapper compatible")
        csv_data = df[export_cols].to_csv(index=False)
        st.download_button(
            "📥 Download CSV",
            csv_data,
            f"selfmodel_{st.session_state.run_id}.csv",
            "text/csv", key="dl_csv"
        )
    ...
    with exp4:
        ...
        st.caption("Complete results backup")
        json_data = json.dumps(st.session_state.results, indent=2)
        st.download_button(
            "📥 Download JSON",
            json_data,
            f"selfmodel_{st.session_state.run_id}.json",
