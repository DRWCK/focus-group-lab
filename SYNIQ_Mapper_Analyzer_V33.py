  ...
  function render(paths) {
    ...
    function updatePies() {
      ...
      if (LBL.mode && LBL.mode !== 'none') {
        ...
        // ---- V33: membership panel for the LABELED cubes (static, exports) ----
        if (LBL.panel) {
          var AGC = {"Claude":"#377eb8","ChatGPT":"#4daf4a","Grok":"#e41a1c","Gemini":"#984ea3"};
          // collect membership for each labeled cube from bound data
          var rows = [];
          ...
          paths.each(function(d){
            ...
            rows.push({num: raw, agents: ag, size: (d.v22_panel? d.v22_panel.size : '')});
          });
          rows.sort(function(a,b){return (parseInt(a.num)||0)-(parseInt(b.num)||0);});
          ...
          if (!host) {
            ...
            css.textContent = '#v33-members{position:fixed; top:50%; right:20px; transform:translateY(-50%);'
              + 'width:210px; max-height:70vh; overflow-y:auto; background:#fff; border:1px solid #d0d0d0;'
              + 'border-radius:6px; padding:10px 12px; font-family:-apple-system,sans-serif; font-size:12px;'
              + 'color:#1a1a1a; box-shadow:0 2px 8px rgba(0,0,0,0.08); z-index:9998;}'
              + '#v33-members h4{margin:0 0 8px 0;font-size:12px;color:#555;font-weight:600;'
              + 'text-transform:uppercase;letter-spacing:0.5px;}'
              + '#v33-members .v33-cube{padding:5px 0;border-top:1px solid #eee;}'
              + '#v33-members .v33-cube:first-of-type{border-top:none;}'
              + '#v33-members .v33-num{font-weight:700;font-family:monospace;font-size:12px;margin-bottom:2px;}'
              + '#v33-members .v33-row{display:flex;justify-content:space-between;padding:1px 0;}'
              + '#v33-members .v33-dot{display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:5px;}'
              + '#v33-members .v33-a{color:#333;} #v33-members .v33-c{color:#666;font-variant-numeric:tabular-nums;}';
            document.head.appendChild(css);
          }
          var html = '<h4>Labeled nodes</h4>';
          if (!rows.length) { html += '<div style="color:#888;font-style:italic;">none</div>'; }
          for (var ri=0; ri<rows.length; ri++) {
            var rr = rows[ri];
            html += '<div class="v33-cube"><div class="v33-num">cube '+rr.num+(rr.size!==''?' &middot; '+rr.size+'pt':'')+'</div>';
            for (var ai=0; ai<rr.agents.length; ai++) {
              var an = rr.agents[ai][0], ac = rr.agents[ai][1], acol = AGC[an]||'#888';
              html += '<div class="v33-row"><span class="v33-a"><span class="v33-dot" style="background:'+acol+'"></span>'+an+'</span><span class="v33-c">'+ac+'</span></div>';
            }
            html += '</div>';
          }
          host.innerHTML = html;
...
if label_mode != "None":
    show_member_panel = st.sidebar.checkbox(
        "📋 Show membership panel (labeled cubes)",
        value=True,
        help="A static box on the right listing each labeled cube and its "
             "agent composition. Exports with the figure."
    )
