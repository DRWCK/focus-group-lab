"""
SYN-IQ Mapper Analyzer V22 — Streamlit App
...
# =============================================================================
st.set_page_config(
    page_title="SYN-IQ Mapper Analyzer V22",
    page_icon="🗺️",
    layout="wide"
)
  ...
  function render(paths) {
    ...
    function updatePies() {
      paths.each(function(d) {
        ...
        var r = baseR * scale;
        var nodeKey = 'v21pie_' + (d.id !== undefined ? d.id : (d.name || ''));
...
def inject_v21_node_coloring(
    ...
    for n in gj.get("nodes", []):
        ...
        if mode == "overlap_orange":
            ...
            # V22: tooltip text showing per-agent counts when user hovers a pie
            tooltip_parts = [f"{a}: {counts[a]}" for a in ordered if counts.get(a, 0) > 0]
            n["v21_tooltip_html"] = "  ·  ".join(tooltip_parts) if tooltip_parts else ""
