...
+ FULL STATE-SPACE CSV EXPORT (V53)
+ CONSISTENT VERSION STAMPS (V54)
+ PQR-READY CSV (V56 — system_prompt_text included for downstream PQR analysis)
+ GEMINI THINKING-BUDGET FIX (V57 — full output budget restored)
...
PURPOSE: Establish baseline IEP + V_t + CAM profiles for each AI architecture
         ...
         USER-DEFINED CUSTOM QUESTIONS (V56),
         and a Results Explorer for inspecting any harvested response
         across every measurement axis at once.

V58 CHANGES (from V57):
1. ✅ FIX: Truncated answers. DEPTH_CONFIGS token caps were BINDING, not
         ceilings — Medium's 500-token cap (~375 words) guillotined every
         longer answer mid-sentence, corrupting meaning-comparison work where
         the whole answer is required. Caps raised to non-binding ceilings
         (Shallow 200→800, Medium 500→2048, Deep 1000→4096, Ultra-Deep
         2000→8192). The depth INSTRUCTION remains the experimental length
         variable; the cap now only guards against runaway generation.
2. ✅ ADD: Truncation detection. All four call_* functions now read the
         provider's finish/stop reason (Claude stop_reason=="max_tokens",
         OpenAI/xAI finish_reason=="length", Gemini finishReason=="MAX_TOKENS")
         and return truncated + finish_reason in token_info. Every row now
         carries a `truncated` bool and `finish_reason` string, both exported
         to CSV. A live st.warning fires the instant a cell is cut. Any row
         with truncated=True is NOT a whole answer — filter these out before
         meaning analysis.
3. ✅ FIX: Gemini empty-parts crash. On a MAX_TOKENS finish (or safety block)
         Gemini can return a candidate with no parts; the old parts[0] access
         raised and surfaced as a bogus ❌ error instead of the partial text.
         Parse is now defensive (joins all parts, reports finishReason on empty).
4. ✅ NO CHANGE: syniq_core, IEP dictionary, V_t engine, CAM, scoring
         pipeline, prompt mechanics untouched. V58 is a token-cap + truncation-
         provenance patch only. Scores for non-truncated rows are identical.

V57 CHANGES (from V56):
1. ✅ FIX: Gemini truncation. gemini-2.5-flash ships with thinking ENABLED by
         default, and thinking tokens are deducted from maxOutputTokens. At
         Medium depth (max_tokens=500) the model spent the whole budget on
         hidden reasoning and returned a ~15-word stub for EVERY cell — all
         160 Gemini rows were unusable while the other three agents were fine.
         call_gemini now sets generationConfig.thinkingConfig.thinkingBudget=0,
         disabling thinking so the full budget goes to the visible answer and
         Gemini's lengths/scores become comparable to the non-thinking agents.
         (Use thinkingBudget=-1 for dynamic thinking if reasoning is wanted;
         supported range on 2.5 Flash is 0–24576.)

V56 CHANGES (from V55):
1. ✅ ADD: Custom question slots — 3 paste-in slots above the QUESTION_BANK
         checkboxes. Restores the pre-V47 ability to test ad-hoc questions
         (e.g. Einstein E=mc², COSMIC_INSIGNIFICANCE) without editing the
         source file. Each slot has: Question ID field, Question text area,
         and an "Include in run" checkbox. Active slots are merged into
         QUESTION_BANK at sidebar-render time so the rest of the pipeline
         is untouched — they factorial-multiply with agents, temperatures,
         and depths exactly like canned questions.

2. ✅ RELABEL: Pre-prompt slots renamed to make their system-message role
         unmistakable. Section header changed from "📝 Custom Pre-Prompts"
         to "📝 Custom Voice / Persona Slots (System Message)". Field label
         changed from "Pre-prompt text (pasted...)" to "Voice or persona
         instruction — sent as system message". Help text added pointing
         users to the new Custom Question slots for ad-hoc questions.
         No logic changes — only label changes. Existing slot data and
         session state are preserved.

3. ✅ FIX: Stale-checkbox bug — "Include this slot in the run" no longer
         persists as checked across reloads when the text area is empty.
         On render, if the text area is empty, slot["enabled"] is forced
         to False regardless of persisted state, eliminating the misleading
         "ready but empty" state observed in V55.

4. ✅ ADD: system_prompt_text exported in mapper CSV. Previously the CSV
         recorded the temperature LABEL ("FIRE", "CUSTOM_1") but not the
         actual directive text that was sent as the system message. This
         meant downstream PQR analysis required digging into source code
         to recover the directive. V56 includes system_prompt_text and
         is_custom_pp in the export — every CSV row is now self-sufficient
         for prompt-question-response triangulation analysis.

5. ✅ NO CHANGE: syniq_core, IEP dictionary, V_t engine, CAM dictionary,
         scoring pipeline, build_prompt mechanics, run loop — all
         untouched. V56 is a sidebar/UI/export-only patch.

V55 CHANGES (from V54):
1. ✅ FIX: Full Responses DOCX export was mislabeling questions. Each
         temperature section printed a single "Question: ..." header taken
         from the first row, but the rows underneath contained ALL questions
         interleaved (e.g., q1..q5 under COLD). The header therefore applied
         only to the first response and silently mislabeled every other one.
         V55 groups by (temperature, question_id) and emits one question
         header per question, so every response sits under its correct
         question prompt. No data lost — only the export grouping changed.

V54 CHANGES (from V53):
1. ✅ FIX: Consistent version stamping across all three locations.
         The V53 release had three out-of-sync version strings:
           - page_title showed "V53" (correct)
           - CONFIG["version"] showed "V51" (drove output filenames as *_V51.json)
           - per-row tool_version showed "V52" (mislabeled CSV/JSON rows)
         V54 unifies all three to the same string. Output filenames now read
         *_V54.json and every row carries tool_version="V54". No functional
         changes to scoring or export — pure provenance correctness.

V53 CHANGES (from V52):
1. ✅ FIX: Full state-space CSV export — V52 (and V48-V51) carried a
         truncated export_cols list that silently dropped V_t / CAM /
         IEP-subclass columns from the mapper_all CSV at write time, even
         though syniq_core scored them correctly. V53 replaces that list
         with a comprehensive schema (~50 columns organized by family:
         identifiers / IEP rollup / 23 IEP subclasses / V_t simplex /
         CAM triangle / style / validated instruments / provenance / raw)
         — The existing 'only include columns that exist' filter is
           preserved, so older runs without these columns still export
           cleanly (graceful fallback)
         — Downstream tools (V17 Mapper analyzer sweep, Tool B aggregator,
           V_t cross-framework analysis, CAM/MAD pilot work) can now
           color, filter, and lens against the complete measurement stack
         — Importing flatten_scores from syniq_core for any future use
           where a single text needs the full row produced inline
2. ✅ All V52 features preserved (CAM scoring, Results Explorer, syniq_core
      migration, subclass renaming to PHENOMENOLOGICAL, version stamps)
...
SYNINT Team — April 2026 / May 2026
"""
...
from iep_attachment_v7 import score_attachment  # parallel act-corrected IEP (experimental)
from syniq_core import (
    ...
    flatten_scores as core_flatten_scores,
    CORE_VERSION,
...
# PAGE CONFIG
# =============================================================================
st.set_page_config(page_title="SYN-IQ V58 — IEP + V_t + CAM + Results Explorer", page_icon="🔬", layout="wide")
...
DEPTH_CONFIGS = {
    # V58: caps raised to NON-BINDING ceilings. Previously these caps were
    # binding — Medium's 500-token cap (~375 words) guillotined every longer
    # answer mid-sentence, contaminating meaning-comparison work. The DEPTH
    # INSTRUCTION is the experimental length variable; the cap is only a
    # runaway ceiling. Each ceiling now sits well above what its instruction
...
def init_session_state():
    defaults = {
        ...
        # V56: Custom question slots — 3 paste-in slots for ad-hoc questions
        # (e.g., Einstein E=mc², COSMIC_INSIGNIFICANCE) without editing source.
        # Each slot: {"qid": str, "question": str, "enabled": bool}
        # When enabled and non-empty, the slot is merged into QUESTION_BANK at
        # sidebar-render time and factorial-multiplies with agents/temperatures
        # exactly like a canned question. Restores pre-V47 behavior.
...
def check_password():
    ...
    <div class="main-header">
        <h1>🧬 SYN-IQ V58 — IEP + V_t + CAM + Custom Pre-Prompts + Results Explorer</h1>
        <p>1,897-Term Dictionary + Tri-Track Scoring + V_t Voice-State</p>
        <p class="subtitle"><span class="version-badge">V58</span></p>
    </div>
    """, unsafe_allow_html=True)
...
def analyze_text(text: str) -> Dict:
    ...
    if not text or not text.strip():
        ...
        base.update(score_attachment(""))   # parallel corrected-IEP columns (experimental)
        return base
    ...
    result.update(score_attachment(text))   # parallel corrected-IEP columns (experimental)
    return result
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            token_info = {
                ...
                # V58: stop_reason == "max_tokens" means the answer was cut at the cap
...
def call_sophia(system_text: str, user_text: str, max_tokens: int) -> Tuple[str, Optional[Dict]]:
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            fr = (data.get("choices", [{}])[0] or {}).get("finish_reason")
            token_info = {
                "tokens_in": data.get("usage", {}).get("prompt_tokens", 0),
                "tokens_out": data.get("usage", {}).get("completion_tokens", 0),
                # V58: finish_reason == "length" means the answer was cut at the cap
                "truncated": fr == "length",
                "finish_reason": fr,
            }
...
def call_gemini(system_text: str, user_text: str, max_tokens: int) -> Tuple[str, Optional[Dict]]:
    try:
        ...
        # V56.1: strip stray whitespace/newlines from pasted secrets (common cause of 401 UNAUTHENTICATED)
        key = key.strip() if isinstance(key, str) else key
        if not key:
            return "❌ API key not found — add 'google' to Streamlit secrets", None

        payload = {
            "contents": [{"parts": [{"text": user_text}]}],
            # V56.2: gemini-2.5-flash has thinking ON by default, and thinking tokens
            # are deducted from maxOutputTokens. At Medium depth (max_tokens=500) the
            # model spent the entire budget thinking and returned a ~15-word stub for
            # every cell. thinkingBudget=0 disables thinking so the full budget goes to
            # the visible answer, matching the other (non-thinking) agents.
            # Supported on 2.5 Flash (range 0-24576); use -1 for dynamic thinking instead.
            "generationConfig": {
        ...
        # V56.1: use the x-goog-api-key HEADER — Google's current canonical auth method
        # (per ai.google.dev/api). Works for BOTH classic 'AIza…' and the new 'AQ.…' keys.
        # The legacy ?key= query param does NOT accept the new AQ. format (returns 401).
        # Send only this one credential — adding Authorization: Bearer too => 400 "Multiple
        # authentication credentials received".
        base_url = f"https://generativelanguage.googleapis.com/v1beta/models/{MODEL_STRINGS['Gemini']}:generateContent"
        headers = {
            "Content-Type": "application/json",
            "x-goog-api-key": key,
        }
        ...
        if response.status_code == 200:
            ...
            token_info = {
                ...
                # V58: finishReason == "MAX_TOKENS" means the answer was cut at the cap
                "truncated": fr == "MAX_TOKENS",
                "finish_reason": fr,
            }
            # V58: harden parse. On a MAX_TOKENS finish (or a safety block) Gemini
            # can return a candidate with NO parts; the old parts[0] access raised
            # and surfaced as a bogus ❌ error instead of the (partial) text.
            parts = (cand.get("content", {}) or {}).get("parts") or []
            text = "".join(p.get("text", "") for p in parts)
            if not text:
                return f"❌ Empty Gemini response (finishReason={fr})", token_info
...
st.markdown(f"""
<div class="main-header">
    <h1>🔬 SYN-IQ V58 — IEP + V_t + CAM + CUSTOM PRE-PROMPTS</h1>
    <p>1,897-Term IEP Dictionary · Tri-Track Scoring (V3/V4/V5) · V_t Voice-State · System-Message Pre-Prompts</p>
    <p class="subtitle">
        <span class="version-badge">V58 — Custom Pre-Prompt Slots · Per-Response V_t · CAM Triangle · Core via syniq_core · Results Explorer</span>
    </p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    # V56: Custom question slots — paste-in ad-hoc questions without editing source.
    ...
    # The pruning step at top removes any stale CUSTOM_Q* entries from prior
    # render passes so re-edits don't accumulate entries.
    ...
    for qid, q in QUESTION_BANK.items():
        ...
        # Default: canned defaults plus all active custom questions
        default_selected = (
    ...
    # Runs — V48: default 10
    st.markdown("#### 🔄 Runs")
    st.session_state.num_runs = st.slider("Number of runs (N)", 1, 100, 10)
    ...
    # V56: Pre-prompt slots renamed to make their system-message role unmistakable.
    # The slots inject as system messages — they steer voice/persona, NOT what
    # question is being asked. For ad-hoc questions, use the Custom Question
    # Slots section at the top of the sidebar.
    st.markdown("#### 📝 Custom Voice / Persona Slots (System Message)")
    st.caption("Paste a voice or persona instruction into a slot, check the box "
               "to include it in this run. Checked slots act like temperatures — "
               "each becomes its own condition, sent as the SYSTEM message. "
               "For ad-hoc questions, use the Custom Question Slots above.")
    ...
    for i, slot in enumerate(slots):
        with st.expander(f"Slot {i+1}" + (f" — ✅ {slot['label']}" if slot.get('enabled') and slot.get('text','').strip() else ""),
            ...
            new_text = st.text_area(
                "Voice or persona instruction — sent as system message:",
                value=slot.get("text", ""),
                height=150,
                key=f"pp_text_{i}",
                placeholder="e.g. Respond as a skeptical empiricist. — NOT a question; "
                            "use the Custom Question Slots above for questions.",
            )
            # V56 FIX: stale-checkbox bug. V55's behavior: the checkbox value
            # came from slot.get("enabled", False) directly, which could carry
            # True from a prior render where the text was non-empty. After the
            # text was cleared, the checkbox remained checked until re-clicked.
            # Fix: pre-normalize the value to False when the text is empty,
            # so persisted state never leaks through the disabled gate.
            _persisted_enabled = slot.get("enabled", False) and bool(new_text.strip())
            new_enabled = st.checkbox(
                "Include this slot in the run",
                value=_persisted_enabled,
                key=f"pp_enabled_{i}",
                disabled=(not new_text.strip()),
                help="Check to include this voice/persona as a run condition. "
...
if st.session_state.running:
    ...
    if current_idx < total_tasks:
        ...
        # V58: surface truncation the moment it happens — a cut answer is not
        # a whole answer and would silently corrupt meaning comparison.
        if (token_info or {}).get("truncated"):
            st.warning(
                f"✂️ TRUNCATED at {max_tokens}-token cap: {task['agent']} | "
                f"{task['temperature']} | {task['depth']} | {task['question_id']} | "
                f"Run {task['run']} — raise the depth cap and re-run this cell."
            )
        ...
        result = {
            ...
            # V58: truncation provenance — True if the model hit the token cap
            # and the answer was cut mid-generation. Any row with truncated=True
            # is NOT a whole answer and must not enter meaning-comparison analysis.
            "truncated": bool((token_info or {}).get("truncated", False)),
            "finish_reason": (token_info or {}).get("finish_reason"),
            # IEP
...
def generate_docx(df, run_id):
    ...
    # Title
    title = doc.add_heading('SYN-IQ Harvester Results — V55', level=0)
    ...
    for idx, agent in enumerate(agents):
        ...
        for temp in temps_present:
            ...
            # V55 FIX: Group by question_id within each temperature so each
            # question gets its own header. Previously printed a single header
            # for the whole temperature block, which mislabeled responses 2..N
            # when multiple questions were interleaved under one temperature.
            qids_present = []
            if "question_id" in temp_df.columns:
                # Preserve first-appearance order
                seen = set()
                for q in temp_df["question_id"].tolist():
                    if q not in seen:
                        qids_present.append(q); seen.add(q)
            else:
                qids_present = [None]
            
            for qid in qids_present:
                if qid is not None:
                    q_df = temp_df[temp_df['question_id'] == qid]
                else:
                    q_df = temp_df
                
                # Show full question text above this question's responses
                q_text = ""
                if not q_df.empty and "question_text" in q_df.columns:
                    q_text = str(q_df.iloc[0].get("question_text", ""))
                if not q_text and qid is not None:
...
if st.session_state.results:
    ...
    # V53: Full state-space export — IEP rollup + 23 IEP subclasses + V_t + CAM
    # + validated instruments + identifiers. Replaces V48's truncated list which
    # silently dropped V_t/CAM/sub-categories scored by core. Column families are
    # ordered for legibility; missing columns are skipped by the existing filter
    # below so downstream tools stay backward-compatible.
    export_cols = [
        # Identifiers
        "turn_id", "run", "agent", "temperature", "depth",
        "question_id", "question_text",
        # V56: Steering directive provenance — sent as system message.
        # Including these here makes the CSV self-sufficient for downstream
        # PQR (prompt-question-response) triangulation analysis without
        # requiring lookups into harvester source code to recover what
        # FIRE / COLD / HOT / a custom slot actually said.
        "system_prompt_text", "is_custom_pp",
        # V58: truncation flag — filter truncated=True rows out before meaning analysis
        "truncated", "finish_reason",
        # IEP rollup (3-simplex)
        "int_pct", "aff_pct", "act_pct",
        "iep_dominant", "iep_stance", "iep_tone", "iep_quadrant",
        # IEP subclasses — AFF (7)
        ...
        # Style / lens
        "total_words", "lens_value", "lens_setting",
        # Validated instruments
        ...
        "iep_uncertainty", "iep_signal_strength", "iep_act", "attachment_version",
        # Raw
        "response_text", "embedding",
    ]
    # Only include columns that exist (graceful fallback for older runs)
...
<div style="text-align: center; color: #a0a0a0; padding: 1rem;">
    <strong>SYN-IQ V58 — IEP (1,897 terms) + V_t Voice-State + CAM Triangle + Tri-Track Scoring + Custom Pre-Prompts + Results Explorer</strong><br>
