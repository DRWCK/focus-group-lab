"""
Focus Group Lab V42.1 — Research Edition
...
st.set_page_config(
    page_title="Focus Group Lab V42.1",
    page_icon="🧬",
    layout="wide",
...
def call_claude(prompt: str, system: str, max_tokens: int = 4096) -> str:
    try:
        ...
        if r.status_code == 200:
            d = r.json()
            return _tag_if_cut(d["content"][0]["text"], d.get("stop_reason") == "max_tokens")
...
def export_to_markdown() -> str:
    md  = f"# Focus Group Lab V42.1 — Session Export\n"
    ...
    if st.session_state.round1_responses:
        ...
        for agent, response in st.session_state.round1_responses.items():
            md += f"### {AGENT_EMOJIS.get(agent,'🤖')} {agent}\n{response}\n\n---\n\n"
    md += "\n---\n*Focus Group Lab V42.1 — Research Edition · SYNINT Team · April 2026*\n"
    return md
...
if session_type == "🔬 Auto Run":
    ...
    if run_auto_btn:
        ...
        experiment_run_id = datetime.now().strftime('%Y%m%d_%H%M%S') + "_V42_1"
