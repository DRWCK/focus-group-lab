...
V4: February 24, 2026 — Fixed unicode/smart quote encoding error in CSV export
V5: February 24, 2026 — Fixed API key loading conflict (widget key vs session_state),
    supports both "anthropic" and "ANTHROPIC_API_KEY" secret naming conventions
...
with st.sidebar:
    ...
    st.caption("Keys from Secrets load automatically. Override here if needed.")

    # Load from secrets once into _loaded_ keys (not the same key as the widget)
    # Supports both naming conventions: "anthropic" and "ANTHROPIC_API_KEY"
    def _get_secret(short_name, long_name):
        return (st.secrets.get(short_name) or
                st.secrets.get(long_name) or "")

    for loaded_key, short, long in [
        ("_loaded_anthropic", "anthropic",  "ANTHROPIC_API_KEY"),
        ("_loaded_openai",    "openai",     "OPENAI_API_KEY"),
        ("_loaded_xai",       "xai",        "XAI_API_KEY"),
        ("_loaded_google",    "google",     "GOOGLE_API_KEY"),
    ]:
        if loaded_key not in st.session_state:
            st.session_state[loaded_key] = _get_secret(short, long)

    # Widgets use separate keys so there's no conflict
    override_anthropic = st.text_input("Anthropic (Claude)", type="password",
        value=st.session_state["_loaded_anthropic"], key="input_anthropic")
    override_openai    = st.text_input("OpenAI (ChatGPT)",   type="password",
        value=st.session_state["_loaded_openai"],    key="input_openai")
    override_xai       = st.text_input("xAI (Grok)",         type="password",
        value=st.session_state["_loaded_xai"],       key="input_xai")
    override_google    = st.text_input("Google (Gemini)",    type="password",
        value=st.session_state["_loaded_google"],    key="input_google")

    # Resolved keys — widget value wins if non-empty, else fall back to loaded secret
    st.session_state["api_anthropic"] = override_anthropic or st.session_state["_loaded_anthropic"]
    st.session_state["api_openai"]    = override_openai    or st.session_state["_loaded_openai"]
    st.session_state["api_xai"]       = override_xai       or st.session_state["_loaded_xai"]
    st.session_state["api_google"]    = override_google    or st.session_state["_loaded_google"]

    # Show which keys are loaded
    key_status = []
    for label, key in [("Claude","api_anthropic"),("GPT","api_openai"),("Grok","api_xai"),("Gemini","api_google")]:
        if st.session_state.get(key):
            key_status.append(f"✅ {label}")
        else:
            key_status.append(f"❌ {label}")
    st.caption(" | ".join(key_status))
    ...
    st.caption("Italics Emergence Experiment v4.0")
...
st.title("✨ Italics Emergence Experiment v4")
st.markdown("""
...
if "results" in st.session_state and st.session_state["results"]:
    ...
    def clean_unicode(val):
        """Replace smart quotes and other non-latin-1 characters with ASCII equivalents."""
    ...
    writer.writeheader()
    for r in results:
        row = {k: clean_unicode(v) for k, v in r.items() if k in fieldnames}
        writer.writerow(row)

    st.download_button(
        "📥 Download CSV (metrics only)",
        csv_buffer.getvalue().encode('utf-8'),
        f"italics_v5_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
        "text/csv"
    )
    ...
    st.download_button(
        ...
        json_str,
        f"italics_v5_full_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
        "application/json"
    )
