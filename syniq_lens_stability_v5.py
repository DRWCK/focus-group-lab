"""
SYN-IQ Lens Stability Analyzer V5
...
st.set_page_config(
    page_title="SYN-IQ Lens Stability Analyzer V5",
    page_icon="🔬",
    layout="wide"
)
...
def check_password():
    ...
    <div style="background: linear-gradient(135deg, #0a0a1a 0%, #1a0a2e 40%, #0f2460 100%);
        ...
        <h1 style="color: #a78bfa;">🔬 SYN-IQ Lens Stability Analyzer V5</h1>
...
<div style="background: linear-gradient(135deg, #0a0a1a 0%, #1a0a2e 40%, #0f2460 100%);
    ...
    <h1 style="color: #a78bfa; margin: 0;">🔬 SYN-IQ Lens Stability Analyzer V5</h1>
...
def load_input(uploaded):
    ...
    if name.endswith('.json'):
        ...
        for col in ('embedding', 'response_text', 'system_prompt_text',
...
if uploaded_file is not None:
    ...
    if 'temperature' in df.columns:
        ...
        if len(regimes_present) >= 3:
            ...
            def _style(v):
                ...
                return ''
            st.dataframe(
                mat.style.format("{:.3f}").applymap(_style),
        ...
        if len(regimes_present) >= 2:
            ...
            st.dataframe(
                ic_df.style
                    ...
                    .applymap(_ic_style, subset=num_cols),
                use_container_width=True
            )
    ...
    if 'lens_results' in st.session_state and st.session_state.lens_results:
        ...
        for cat in ['AFF', 'INT', 'ACT', 'Cross-IEP', 'Polarity', 'Geometric']:
            cat_df = rdf[rdf['Category'] == cat]
            if len(cat_df) == 0:
                continue
            st.markdown(f"### {cat} Lenses")
            display_cols = ['Lens','Type','Nodes','Edges','β₀','β₁',
                            'Largest %pts','Q-purity (wtd)','T-purity (wtd)','% Connected']
        ...
        with col2:
            # Summary text report
            report_lines = [
                "SYN-IQ Lens Stability Analysis Report (V5)",
            ...
            if 'stability_score' in dir():
                ...
                if pur_stab is not None:
                    report_lines.append(
                        f"  Weighted purity:        mean={pur_mean:.3f}, SD={pur_sd:.3f} across lenses"
                    )
            report_lines += [
                "",
        ...
        with col3:
            ...
            if 'stability_score' in dir():
                ...
                pur_clause = (
                    f" Mean weighted node purity with respect to the temperature label "
                    f"was {pur_mean:.2f} (SD across lenses = {pur_sd:.3f}), indicating that "
                    f"the COLD/NATIVE/HOT/FIRE separation persists regardless of projection."
                ) if pur_stab is not None else ""
                ...
                paragraph = (
                    ...
                    f"{pur_clause} The combined stability score was {stability_score:.2f}, which we "
...
<div style="text-align: center; color: #a0a0a0; padding: 1rem;">
    <strong>SYN-IQ Lens Stability Analyzer V5</strong><br>
