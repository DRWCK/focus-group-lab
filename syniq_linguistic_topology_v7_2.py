"""
SYN-IQ Linguistic Topology Analyzer V7.2
======================================
...
USAGE:
    streamlit run syniq_linguistic_topology_v7_2.py
...
st.set_page_config(
    page_title="SYN-IQ Linguistic Topology V7.2",
    page_icon="📐",
    layout="wide",
)
...
st.markdown("# 📐 SYN-IQ Linguistic Topology V7.2")
...
def compute_pooled_mud(df, mode='word', axis_col='depth', order=None):
    ...
    for (agent, qid), g in df.groupby(['agent', 'question_id']):
        ...
        for i in range(len(present) - 1):
            ...
            # V7.1: raw total-words per level (mean across runs) -> substitution vs accretion
            wc_from = [len(str(t).split()) for t in texts_from]
            wc_to = [len(str(t).split()) for t in texts_to]
            mean_w_from = sum(wc_from) / len(wc_from) if wc_from else 0.0
            mean_w_to = sum(wc_to) / len(wc_to) if wc_to else 0.0
            w_delta = mean_w_to - mean_w_from
            w_pct = (w_delta / mean_w_from * 100.0) if mean_w_from else 0.0
            rows.append({
                'agent': agent,
                'question_id': qid,
                'level_from': d_from,
                'level_to': d_to,
                'mud_pooled': mud,
                'mean_words_from': round(mean_w_from, 1),
                'mean_words_to': round(mean_w_to, 1),
                'words_delta': round(w_delta, 1),
                'words_pct_change': round(w_pct, 1),
...
if st.button("🚀 Compute MUD", type="primary"):
    ...
    if len(pooled) > 0:
        for mode in modes:
            ...
            # V7.1: substitution vs accretion — MUD beside total-words delta
            st.markdown(f"#### Mechanism: substitution vs accretion ({mode})")
            st.caption(
                "MUD = fraction of new vocabulary. words_delta = change in raw "
                "response length across the same pair. High MUD + flat words = "
                "SUBSTITUTION (vocabulary swapped in place). Moderate MUD + rising "
                "words = ACCRETION (old words kept, new piled on)."
            )
            mech = (sub.groupby('agent')
                    .agg(mud=('mud_pooled', 'mean'),
                         words_from=('mean_words_from', 'mean'),
                         words_to=('mean_words_to', 'mean'),
                         words_delta=('words_delta', 'mean'),
                         words_pct=('words_pct_change', 'mean'))
                    .reset_index()
                    .sort_values('mud', ascending=False))

            def _label(r):
                if r['mud'] >= 0.5 and abs(r['words_pct']) < 15:
                    return 'SUBSTITUTION'
                if r['words_pct'] >= 15:
                    return 'ACCRETION'
                if r['mud'] < 0.35 and abs(r['words_pct']) < 15:
                    return 'STABLE'
                return 'MIXED'
            mech['mechanism'] = mech.apply(_label, axis=1)
            st.dataframe(
                mech.style.format({
                    'mud': '{:.3f}', 'words_from': '{:.0f}', 'words_to': '{:.0f}',
                    'words_delta': '{:+.0f}', 'words_pct': '{:+.1f}%'
                }),
    ...
    # Combined manifest
    manifest = {
        'tool': 'syniq_linguistic_topology_v7.2',
        'generated_at': datetime.now().isoformat(),
        'pairing_axis': AXIS_COL,          # 'depth' or 'temperature' — which axis MUD walked
        'pairing_axis_label': AXIS_LABEL,  # 'Depth' or 'Condition'
        'axis_order': AXIS_ORDER,          # the ordered ladder actually used
        'modes': modes,
...
st.markdown("---")
st.caption("SYN-IQ Linguistic Topology V7.2 · Tennessee 🎹 CUZ Partnership · May 2026")
