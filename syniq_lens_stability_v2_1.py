...
<div style="background: linear-gradient(135deg, #1e3a5f 0%, #2e75b6 100%);
  ...
  <h1 style="margin: 0;">🔬 SYN-IQ Lens Stability V2.1</h1>
  <p style="margin: 0.3rem 0 0; opacity: 0.9;">
    Tool A · Per-Agent Lens Stability Sweep · §4.2 Deliverable Engine<br>
    <span style="font-size: 0.85em; opacity: 0.85;">v2.1 — DBSCAN defaults match v14 (eps=2.0, min_samples=3) · natural-scale features · coverage check</span>
  </p>
</div>
""", unsafe_allow_html=True)
...
def build_feature_matrix(df: pd.DataFrame):
    ...
    # v2.1 fix: StandardScaler matches v14 behavior. eps=2.0 is calibrated to
    # z-scored space (~2 SDs). Without any scaling, total_words (range 73-407)
    # swamps IEP percentages (0-90) in distance computation.
    from sklearn.preprocessing import StandardScaler as _StdScaler
    data = _StdScaler().fit_transform(data)
    return data, avail
...
eps          = st.sidebar.number_input("DBSCAN eps", min_value=0.1, max_value=5.0, value=2.0, step=0.1,
    help="v2.1: default 2.0 matches v14 standalone Mappers. Original spec used 0.5 but drops most points as noise at large n.")
min_samples  = st.sidebar.number_input("DBSCAN min_samples", min_value=2, max_value=10, value=3,
    help="v2.1: default 3 matches v14. Noise filter — fewer points than this in a neighborhood = noise, not a node.")
    ...
    for i, (lens_name, family, dim, kind, cols) in enumerate(LENS_SPEC):
        ...
        try:
            ...
            results.append({
                ...
                'coverage_pct': round(100 * stats['total_members'] / len(pooled), 1),
...
if 'results_df' in st.session_state:
    ...
    if len(valid) > 0:
        # ===== v2.1 COVERAGE CHECK =====
        # Are we actually clustering most of the data, or is DBSCAN dropping
        # most points as noise? At eps too tight for n, this fails silently.
        if 'coverage_pct' in valid.columns:
            mean_cov = valid['coverage_pct'].mean()
            min_cov = valid['coverage_pct'].min()
            max_cov = valid['coverage_pct'].max()
            if mean_cov < 50:
                st.error(
                    f"LOW CLUSTERING COVERAGE: Mapper is only covering "
                    f"{mean_cov:.0f}% of your {len(pooled)} data points on average "
                    f"(range {min_cov:.0f}%-{max_cov:.0f}%). DBSCAN is dropping most "
                    f"points as noise. Try INCREASING eps in the sidebar (try 2.0-5.0) "
                    f"and re-run."
                )
            elif mean_cov < 80:
                st.warning(
                    f"Moderate coverage: {mean_cov:.0f}% of points covered on average. "
                    f"Some points dropped as noise. Consider increasing eps."
                )
            else:
                st.success(
                    f"Good coverage: {mean_cov:.0f}% of {len(pooled)} points covered "
                    f"on average (range {min_cov:.0f}%-{max_cov:.0f}%)."
                )
        # ===== end v2.1 =====
