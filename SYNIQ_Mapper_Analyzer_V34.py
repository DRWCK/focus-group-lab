      ...
      if (LBL.mode && LBL.mode !== 'none') {
        ...
        if (LBL.panel) {
          ...
          paths.each(function(d){
            ...
            if (!ag.length) return;
            // plurality agent = the one with the max count
            var best=null, bestn=-1;
            for (var k=0;k<ag.length;k++){ if (ag[k][1] > bestn){ bestn=ag[k][1]; best=ag[k][0]; } }
            if (best===null) return;
            (byAgent[best] = byAgent[best] || []).push(parseInt(raw));
          });
          ...
          for (var oi=0; oi<AGORDER.length; oi++){
            ...
            any=true;
            byAgent[a].sort(function(x,y){return x-y;});
            html += '<div class="v34-row"><span class="v34-dot" style="background:'+(AGC[a]||'#888')+'"></span>'
                  ...
                  + '<span class="v34-cubes">'+byAgent[a].join(', ')+'</span></div>';
          }
