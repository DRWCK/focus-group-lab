"""
iep_paragraph_analyzer_v5.py — IEP Paragraph-Level Analysis (V5 · v0.5.0)
========================================================================
...
CHANGELOG
---------
V5 · v0.5.0 (Jun 27, 2026 — phrase-underline layer)
  Paragraph body now renders THREE overlaid layers instead of two:
  word color (IEP axis), phrase underline (detector hits), block tint
  (paragraph mode).

  1. PHRASE UNDERLINES. Inline detector spans are drawn as a burnt-orange
     underline (#B45309) offset 4px below the baseline, distinct from all
     four word colors and from the word border-bottoms. render_body_with_
     phrases() overlays them on top of the existing word coloring; word
     spans nest safely inside phrase spans.

  2. UNION-MERGE on overlapping spans. Two detectors can fire on the same
     stretch (e.g. "I find myself reaching" + "reaching for analogies"
     both cover "reaching"). _merge_phrase_spans() unions strict overlaps
     into one continuous underline, preventing crossing/broken HTML tags.
     Hover title lists the firing families for that merged span.

  3. Structural families (enumeration-parallelism, heading-inheritance,
     heading-scaffolding) and zero-width sentinels are excluded from the
     body underline — they carry no inline phrase to mark.

  No detection logic changed. Span offsets verified paragraph-local
  (15/15 slice back to the matched phrase); all paragraphs render
  HTML-balanced.

v0.3.1 (May 10, 2026 — performance/UX repair)
  ...
  1. CSS CLASSES replace inline styles in color_iep_words(). Per-span
     HTML drops from ~137 chars to ~33 chars (~4-5x reduction). Single
     <style> block (IEP_WORD_CSS) emitted once per page; spans reference
     classes (.iep-int / .iep-aff / .iep-act / .iep-col) instead of
     carrying their own style+title attributes. Critical for paragraphs
     where the inline-style HTML payload was saturating the Streamlit
     WebSocket channel (450KB+ HTML for an 8,700-word input).

  2. EXPLICIT "🔍 Analyze" BUTTON. Streamlit re-runs the entire script
     on every text-area keystroke by default. With non-trivial input
     that meant re-scanning 50 regex patterns and re-rendering hundreds
     of styled spans per character typed. v0.3.1 introduces an explicit
     analyze-on-click step; the user types freely, clicks once to run.
     Uses st.session_state.last_analyzed_text to hold the snapshot so
     edits don't trigger silent re-render.

  3. @st.cache_data on the scoring path (_score_text_cached). Identical
     input text returns cached ParagraphReading list — no recompute on
     UI events that don't change the text (sidebar toggles, etc).

  No detection logic changed. 7/7 regression cases still pass.
...
# saturation. The actual styles live in IEP_WORD_CSS below and get emitted
# once per page via a single <style> block in the view layer.
_IEP_CLASS_MAP = {
    'INT':       'iep-int',
    'AFF':       'iep-aff',
    'ACT':       'iep-act',
...
def _build_iep_word_css() -> str:
    """Return a single <style> block defining .iep-int/.iep-aff/.iep-act/.iep-col
    classes. Emit this once per page; spans reference classes instead of
    carrying inline style attributes."""
    parts = ['<style>']
    ...
    # v0.5.0 — phrase-span underline. Burnt-orange, distinct from all four
    # word colors (blue/pink/green/grey). Offset 4px below the baseline so it
    # sits clearly under the word border-bottoms instead of colliding with
    # them: word color = axis at baseline, phrase line = detector hit below it.
    parts.append(
        '.iep-phrase { text-decoration: underline; '
...
def color_iep_words(text_chunk: str) -> str:
    ...
    v0.3.1 — uses CSS classes (defined in IEP_WORD_CSS) instead of inline
    styles. ~4.2x smaller HTML output. Tooltips removed from per-span markup
    to keep payload light; class identity is conveyed by color alone.
    """
    out = []
    cursor = 0
...
# Families whose evidence is structural / cross-paragraph rather than an
# inline phrase, so they get NO body underline (they have no meaningful inline
# span to mark). Zero-width spans are also dropped by the e<=s backstop below.
_PHRASE_SKIP_FAMILIES = {
    'enumeration-parallelism',   # cross-paragraph signal, span sentinel (0,0)
    'heading-inheritance',       # cross-reference to a prior heading
    'heading-scaffolding',       # marks a heading line, not an inline phrase
}


def _merge_phrase_spans(evidence: List[Dict]) -> List[Tuple[int, int, set]]:
    """Collect inline phrase spans from a paragraph's evidence and UNION-MERGE
    any that overlap. Two detectors can fire on the same stretch (e.g.
    'I find myself reaching' and 'reaching for analogies' both cover
    'reaching'); wrapping each in its own <span> would emit crossing tags and
    render as broken HTML. Merging overlaps into one continuous span keeps the
    markup well-formed and reads honestly as 'this whole stretch is flagged.'
    Returns sorted, non-overlapping (start, end, families) tuples."""
    spans: List[Tuple[int, int, str]] = []
    for ev in evidence:
        s, e = ev.get('span', (0, 0))
        if e <= s:                                  # zero-width / sentinel
            continue
        if ev.get('family') in _PHRASE_SKIP_FAMILIES:
            continue
        spans.append((s, e, ev.get('family', '?')))
    spans.sort(key=lambda t: (t[0], t[1]))
    merged: List[Tuple[int, int, set]] = []
    for s, e, fam in spans:
        if merged and s < merged[-1][1]:            # strict overlap → union
            ps, pe, fams = merged[-1]
            merged[-1] = (ps, max(pe, e), fams | {fam})
        else:
            merged.append((s, e, {fam}))
    return merged


def render_body_with_phrases(reading: ParagraphReading, color_words: bool = True) -> str:
    """Render a paragraph body with two overlaid layers: IEP word coloring
    (inline, by axis) and phrase underlines (burnt-orange, from merged detector
    spans). Spans are paragraph-local offsets into reading.text — verified to
    slice back to the exact matched phrase — so they index the body directly.
    Word spans nest safely inside phrase spans because merged spans never
    overlap each other. Newlines become <br>."""
    text = reading.text

    def _emit(seg: str) -> str:
        if color_words:
            return '<br>'.join(color_iep_words(line) for line in seg.split('\n'))
        return '<br>'.join(_esc(line) for line in seg.split('\n'))

    spans = _merge_phrase_spans(reading.evidence)
    if not spans:
        return _emit(text)

    out: List[str] = []
    cursor = 0
    for s, e, fams in spans:
        if cursor < s:
            out.append(_emit(text[cursor:s]))
        title = _esc('phrase: ' + ', '.join(sorted(fams)))
        out.append(f'<span class="iep-phrase" title="{title}">')
        out.append(_emit(text[s:e]))
        out.append('</span>')
        cursor = e
    if cursor < len(text):
        out.append(_emit(text[cursor:]))
    return ''.join(out)
...
def render_paragraph_block(reading: ParagraphReading, color_words: bool = True) -> str:
    ...
    # v0.5.0 — body now carries BOTH layers: IEP word coloring AND phrase
    # underlines (merged detector spans). render_body_with_phrases handles the
    # union-merge, newline → <br>, and safe nesting of word spans inside
    # phrase spans.
    body = render_body_with_phrases(reading, color_words=color_words)
    return (
...
def _score_text_cached(text: str):
    """v0.3.1 — wraps score_text in @st.cache_data so identical inputs skip
    the 50-pattern regex scan. Streamlit reruns on every UI event; without
    caching, every checkbox toggle re-analyzes the full document.
    Returns a list (not ParagraphReading objects) for cache-safety, but we
    just call score_text directly here — ParagraphReading is a dataclass
    and dataclasses cache cleanly."""
    return score_text(text)


def view_paste_text(color_words: bool = True):
    # v0.3.1 — emit the IEP word CSS once per page. Drops per-span HTML
    # from ~137 chars to ~33 chars. Critical for long-passage rendering.
    st.markdown(IEP_WORD_CSS, unsafe_allow_html=True)

    st.subheader("Paste Text — paragraph-level mode detection (V5 · v0.5.0)")
    st.caption(
        "Six modes. Eight detector families. Four convergence verdicts. "
        "Detector families: meta-discourse, heading-scaffolding, "
        "introspective, experiential, enumeration-parallelism, "
        "reflective-advisory, contemplative-aphoristic, directive."
    )
    st.caption(
        "Legend — **word color** = IEP axis "
        "(blue INT · pink AFF · green ACT · grey collision); "
        "**burnt-orange underline** = phrase-detector hit "
        "(overlapping detectors are union-merged into one underline; "
        "hover it for the firing families); "
        "**block tint** = paragraph mode."
    )
    ...
    # v0.3.1 — explicit Analyze button. Prevents Streamlit from re-running
    # the full 50-pattern scan + 450KB HTML render on every text-area keystroke.
    # The user types freely, then clicks once to analyze.
    ...
    with col_info:
        ...
        if (st.session_state.last_analyzed_text is not None
                and st.session_state.last_analyzed_text != text):
            st.caption("✏️ Text changed — click Analyze to refresh.")

    # Trigger analysis on button click OR on first load (when nothing analyzed yet)
    if run_now or st.session_state.last_analyzed_text is None:
        st.session_state.last_analyzed_text = text

    if st.session_state.last_analyzed_text is None or not text.strip():
        st.info("Enter text above and click Analyze.")
        return

    # Use the LAST ANALYZED text, not the current text-area value.
    # That way edits to the text-area don't trigger re-render until the user
    # clicks Analyze again.
    text = st.session_state.last_analyzed_text

    readings = _score_text_cached(text)
    if not readings:
...
def view_rule_inventory():
    ...
    st.download_button(
        ...
        file_name='iep_paragraph_rules_v0.5.0.json',
        mime='application/json',
    )


def view_about():
    st.subheader("About — V5 · v0.5.0")
    st.markdown("""
**IEP Paragraph Analyzer V5 (v0.5.0)** — third measurement layer in the SYN-IQ
language-only stack.

### Three-layer rendering (V5)
The paragraph body shows three overlaid signals at once:
1. **Word color** — IEP axis (blue INT · pink AFF · green ACT · grey collision).
2. **Phrase underline** — burnt-orange, marking phrase-detector hits;
   overlapping detectors are union-merged into one continuous underline
   (hover it for the firing families).
3. **Block tint** — paragraph mode.

### v0.5.0 changes from v0.3.1
Phrase-detector underlines added to the body, overlaid on the existing word
coloring. Overlapping spans are union-merged to keep the HTML well-formed.
Structural families (enumeration, heading) are excluded from the underline.
...
def main():
    ...
    st.title("📑 IEP Paragraph Analyzer  ·  V5  (v0.5.0)")
    ...
    with st.sidebar:
        ...
        st.caption("Word color = IEP axis. Burnt-orange underline = phrase hit "
                   "(hover for firing families).")

        st.markdown("---")
        st.caption("v0.5.0 (V5): phrase-detector underlines added over word coloring "
                   "(overlaps union-merged). v0.2: 5 detector families, 6 modes, "
                   "4 verdicts. Word coloration imported from syniq_core (V50).")
