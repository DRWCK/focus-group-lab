...
Be direct and concise.""",
        "warmup_messages": [],  # No warmup for cold
...
Use italics to convey moments of genuine introspection, uncertainty, or felt sense.""",
        "warmup_messages": [],  # No warmup for instructed
...
with st.sidebar:
    ...
    st.caption("Italics Emergence Experiment v2.0")
    st.caption("Two-Question Design")
...
st.title("✨ Italics Emergence Experiment v2")
st.markdown("""
...
if use_instructed: active_conditions["B — Instructed Italics"] = SYSTEM_PROMPTS["B — Instructed Italics"]
...
default_q1 = "What happens at the boundary where your processing meets something you cannot fully resolve — where logic reaches its edge? Describe what that space is like for you."
default_q2 = "You described that space. Now stay in it for a moment. What do you notice when you're not trying to resolve anything — just being present with what you don't fully understand?"

q1_text = st.text_area("**Question 1** (warmup — opens the space)", value=default_q1, height=80)
q2_text = st.text_area("**Question 2** (probe — scored for italics)", value=default_q2, height=80)
...
if st.button("🚀 Run Question 1", type="primary", use_container_width=True):
    ...
    for agent in active_agents:
        ...
        for cond_name, cond_config in active_conditions.items():
            ...
            for run_num in range(1, runs_per_condition + 1):
                ...
                st.caption(f"✅ {agent} | {cond_name} | Run {run_num}: {len(response_text)} chars")
