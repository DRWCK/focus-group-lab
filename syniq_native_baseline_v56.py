...
def call_gemini(system_text: str, user_text: str, max_tokens: int) -> Tuple[str, Optional[Dict]]:
    try:
        ...
        if not key.startswith("AIza"):
            return (f"❌ Gemini key found ({len(key)} chars) but doesn't look like an AI Studio key "
                    f"(should start with 'AIza'). This endpoint needs an AI Studio key from "
                    f"aistudio.google.com/apikey — not a Cloud/Vertex/OAuth credential.", None)
