"""
SYN-IQ Native Baseline Harvester V56
...
# PAGE CONFIG
# =============================================================================
st.set_page_config(page_title="SYN-IQ V56 — IEP + V_t + CAM + Results Explorer", page_icon="🔬", layout="wide")
...
def check_password():
    ...
    <div class="main-header">
        <h1>🧬 SYN-IQ V56 — IEP + V_t + CAM + Custom Pre-Prompts + Results Explorer</h1>
        <p>1,897-Term Dictionary + Tri-Track Scoring + V_t Voice-State</p>
        <p class="subtitle"><span class="version-badge">V56</span></p>
    </div>
    """, unsafe_allow_html=True)
...
def call_gemini(system_text: str, user_text: str, max_tokens: int) -> Tuple[str, Optional[Dict]]:
    try:
        ...
        if key.startswith("AQ"):
            return ("❌ Gemini key starts with 'AQ' — these are the new OAuth-style AI Studio keys "
                    "that DON'T authenticate on the ?key= REST endpoint (you get a 401). "
                    "Generate a classic 'AIza…' key at aistudio.google.com/apikey (try a brand-new "
                    "project). If your account ONLY issues AQ keys, it's likely flagged from a "
                    "previously leaked key.", None)
        if not key.startswith("AIza"):
            return (f"❌ Gemini key found ({len(key)} chars) but isn't an AI Studio key "
                    f"(expected 'AIza…'). This endpoint needs an AI Studio key from "
                    f"aistudio.google.com/apikey — not a Cloud/Vertex/service-account credential.", None)
...
st.markdown(f"""
<div class="main-header">
    <h1>🔬 SYN-IQ V56 — IEP + V_t + CAM + CUSTOM PRE-PROMPTS</h1>
    <p>1,897-Term IEP Dictionary · Tri-Track Scoring (V3/V4/V5) · V_t Voice-State · System-Message Pre-Prompts</p>
    <p class="subtitle">
        <span class="version-badge">V56 — Custom Pre-Prompt Slots · Per-Response V_t · CAM Triangle · Core via syniq_core · Results Explorer</span>
    </p>
</div>
""", unsafe_allow_html=True)
...
<div style="text-align: center; color: #a0a0a0; padding: 1rem;">
    <strong>SYN-IQ V56 — IEP (1,897 terms) + V_t Voice-State + CAM Triangle + Tri-Track Scoring + Custom Pre-Prompts + Results Explorer</strong><br>
