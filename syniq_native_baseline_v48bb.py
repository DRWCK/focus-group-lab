"""
SYN-IQ Native Baseline Harvester V48bb
...
V48bb CHANGES (from V48b):
1. ✅ FIX: Retry wrapper (call_with_retry) now treats transient server
         errors as retryable — HTTP 412/500/502/503/529 plus 'overloaded',
         'unavailable', 'invalid state', and timeouts — not just 429/rate.
         This stops intermittent Grok 412 ('Model unavailable due to invalid
         state') from dying as permanent empty cells. Scoring path unchanged.
...
# PAGE CONFIG
# =============================================================================
st.set_page_config(page_title="SYN-IQ V48bb — Data-Driven IEP", page_icon="🔬", layout="wide")
...
def call_with_retry(api_func, *args, **kwargs) -> Tuple[str, float, Optional[Dict]]:
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            _retryable = (
                "429" in response_text or "rate" in _low
                or any(f"error {c}" in _low for c in ("412", "500", "502", "503", "529"))
                or "overloaded" in _low or "unavailable" in _low
                or "invalid state" in _low or "timeout" in _low or "timed out" in _low
            )
            if _retryable:
...
st.markdown(f"""
<div class="main-header">
    <h1>🔬 SYN-IQ V48bb — DATA-DRIVEN IEP</h1>
    <p>1,897-Term Dictionary + VADER + Flesch-Kincaid + TTR + Live Monitoring</p>
    <p class="subtitle">
        <span class="version-badge">V48bb — 1897-TERM DATA-DRIVEN + VALIDATED INSTRUMENTS</span>
    </p>
</div>
""", unsafe_allow_html=True)
...
def generate_docx(df, run_id):
    ...
    # Title
    title = doc.add_heading('SYN-IQ Focus Group Results — V48bb', level=0)
...
<div style="text-align: center; color: #a0a0a0; padding: 1rem;">
    <strong>SYN-IQ V48bb — 1,897-Term Data-Driven Dictionary + Scientific Validation + DOCX Export</strong><br>
