...
with st.sidebar:
    ...
    # Pre-populate from secrets if not already set
    for key, secret_name in [
        ("api_anthropic", "ANTHROPIC_API_KEY"),
        ("api_openai",    "OPENAI_API_KEY"),
        ("api_xai",       "XAI_API_KEY"),
        ("api_google",    "GOOGLE_API_KEY"),
    ]:
        if key not in st.session_state or not st.session_state[key]:
            st.session_state[key] = st.secrets.get(secret_name, "")
...
if "results" in st.session_state and st.session_state["results"]:
    ...
    st.download_button(
        ...
        f"italics_v4_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
        "text/csv"
    )
    ...
    st.download_button(
        ...
        json_str,
        f"italics_v4_full_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
        "application/json"
    )
