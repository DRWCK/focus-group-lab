"""
SYN-IQ Mapper Analyzer V51 — Streamlit App
...
def inject_v21_node_coloring(
    ...
    # V51 — freeze the layout into the page itself.
    #
    # Everything upstream of this was treating a symptom. The graph moved
    # because nothing told it where to go: KeplerMapper ships node data with
    # no coordinates, so every load runs a fresh force simulation, settles
    # somewhere new, and spreads past whatever fit was applied. That is why
    # the download "goes back to being live", why the same graph could not be
    # captured twice at the same scale, and why five panels could not be made
    # to match.
    #
    # Positions are computed here, deterministically, and written into each
    # node as x/y plus fx/fy. d3 treats fx/fy as fixed, so the simulation has
    # nothing left to solve: the page opens already laid out, identically
    # every time, in the app and in the downloaded file alike. The same
    # coordinates feed the SVG figure, so the HTML and the print figure are
    # the same picture rather than two attempts at it.
    try:
        if st.session_state.get("_v51_freeze_layout", True):
...
st.session_state["_v51_freeze_layout"] = st.sidebar.checkbox(
    "Freeze layout (recommended)", value=True,
    help="Writes deterministic coordinates into the graph so the force "
         "simulation has nothing to solve. The page opens already laid out, "
         "identically every time, in the app and in the downloaded HTML. "
         "Uncheck to let KeplerMapper run its simulation live.")
