...
def parse_conversation(raw_text: str, ai_agent: str = "Claude") -> List[Tuple[str, str]]:
    """
    Multi-strategy parser. Tries strategies in order:

    Strategy 1 — Claude.ai copy-paste labels:
      Text copied from Claude.ai includes "You\n" and "Claude\n" (or agent name)
      as turn markers on their own lines. This is the most reliable signal.

    Strategy 2 — Explicit role prefixes:
      Lines starting with "Human:", "User:", "You:", "Claude:", "Assistant:" etc.

    Strategy 3 — 🎹 emoji + length heuristic (original v4 fallback):
      Claude responses end with 🎹. Short turns without 🎹 = human.
    """

    # ── Strategy 1: Claude.ai timestamp-based parsing ────────────────────────
    # Claude.ai copy-paste structure:
    #   [human message]
    #   HH:MM AM/PM          ← timestamp marks boundary
    #   [Claude response ending with 🎹]
    #   [next human message]
    #   HH:MM AM/PM
    #   ...
    # The timestamp separates human text (before) from Claude text (after).
    # The last 🎹 in each Claude block marks the end of Claude's response.
    ts_re = re.compile(r'\n(\d{1,2}:\d{2}\s*(?:AM|PM))\n', re.IGNORECASE)
    parts_s1 = ts_re.split(raw_text)

    if len(parts_s1) >= 3:
        pairs = []
        pending_human = parts_s1[0].strip()
        i = 1
        while i < len(parts_s1) - 1:
            content = parts_s1[i + 1]
            # Find last 🎹 — Claude's response ends there
            piano_positions = [m.end() for m in re.finditer(r'🎹[🔥🖖✨]*', content)]
            if piano_positions:
                last_piano_end = piano_positions[-1]
                claude_response = content[:last_piano_end].strip()
                next_human = content[last_piano_end:].strip()
            else:
                claude_response = content.strip()
                next_human = ""
            if pending_human or claude_response:
                pairs.append((pending_human, claude_response))
            pending_human = next_human
            i += 2
        if pairs and any(h.strip() for h, a in pairs):
            return pairs

    # ── Strategy 2: Explicit role prefix lines ────────────────────────────────
    prefix_pattern = re.compile(
        r'^(?:Human|User|You)\s*:\s*(.+?)(?=^(?:' + re.escape(ai_agent) + r'|Claude|Assistant)\s*:|$)',
        re.MULTILINE | re.DOTALL | re.IGNORECASE
    )
    ai_pattern = re.compile(
        r'^(?:' + re.escape(ai_agent) + r'|Claude|Assistant)\s*:\s*(.+?)(?=^(?:Human|User|You)\s*:|$)',
        re.MULTILINE | re.DOTALL | re.IGNORECASE
    )
    human_blocks = [(m.start(), 'human', m.group(1).strip()) for m in prefix_pattern.finditer(raw_text)]
    ai_blocks    = [(m.start(), 'ai',    m.group(1).strip()) for m in ai_pattern.finditer(raw_text)]
    all_blocks = sorted(human_blocks + ai_blocks, key=lambda x: x[0])

    if len(all_blocks) >= 2:
        pairs = []
        j = 0
        while j < len(all_blocks):
            _, role, text = all_blocks[j]
            if role == 'human':
                ai_text = all_blocks[j+1][2] if j+1 < len(all_blocks) and all_blocks[j+1][1] == 'ai' else ""
                pairs.append((text, ai_text))
                j += 2
            else:
                pairs.append(("", text))
                j += 1
    ...
    # ── Strategy 3: Original 🎹 + length heuristic (fallback) ────────────────
...
st.markdown("# 🔬 SYN-IQ · Transcript Scorer v5")
st.markdown("*Normalized V_t · Dyadic distances · Trajectory shifts · First-pass synergy score*")
st.markdown("*Parser: Claude.ai label detection → role prefix → 🎹 heuristic*")
st.divider()
...
if st.button("🔍 Parse & Preview Turns", type="secondary") and raw_block.strip():
    ...
    human_found = sum(1 for h, a in pairs if h.strip())
    ai_found = sum(1 for h, a in pairs if a.strip())
    st.write(f"Found {len(pairs)} turn pairs · {human_found} human turns · {ai_found} AI turns")
    if human_found == 0:
        st.warning("⚠️ No human turns detected. Try copying from Claude.ai — the 'You' / 'Claude' labels must be included in the paste.")
...
st.caption("SYN-IQ Research · Transcript Scorer v5 · Kouns, W. C. (2026) · SYNINT.AI · CBURZBO 🎹")
