...
def parse_conversation(raw_text: str, ai_agent: str = "Claude") -> List[Tuple[str, str]]:
    ...
    # ── Strategy 1: Claude.ai "You" / "Claude" label markers ─────────────────
    # When copied from Claude.ai, each turn is preceded by the speaker name
    # on its own line: "You\n...message...\nClaude\n...response..."
    label_pattern = re.compile(
        r'^(You|' + re.escape(ai_agent) + r'|Claude|Assistant|Human|User)\s*$',
        re.MULTILINE | re.IGNORECASE
    )
    label_splits = label_pattern.split(raw_text)

    if len(label_splits) >= 3:
        # label_splits = [pre, label1, text1, label2, text2, ...]
        labeled_turns = []
        i = 1
        while i < len(label_splits) - 1:
            speaker = label_splits[i].strip().lower()
            content = label_splits[i+1].strip()
            # Strip timestamps from content
            content = re.sub(r'\n?\d{1,2}:\d{2}\s*(?:AM|PM)\s*\n?', '', content).strip()
            content = re.sub(r'\n?(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2}\s*\n?', '', content, flags=re.IGNORECASE).strip()
            if content:
                is_human = speaker in ['you', 'human', 'user']
                labeled_turns.append(('human' if is_human else 'ai', content))
            i += 2

        if labeled_turns:
            pairs = []
            j = 0
            while j < len(labeled_turns):
                role, text = labeled_turns[j]
                if role == 'human':
                    ai_text = labeled_turns[j+1][1] if j+1 < len(labeled_turns) and labeled_turns[j+1][0] == 'ai' else ""
