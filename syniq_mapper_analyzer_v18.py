...
# =============================================================================
st.set_page_config(
    page_title="SYN-IQ Mapper Analyzer V18",
    page_icon="🗺️",
    layout="wide"
)
...
def check_password():
    ...
    <div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%);
        ...
        <h1 style="color: #e94560;">🗺️ SYN-IQ Mapper Analyzer V18</h1>
...
<div class="main-header">
    <h1>🗺️ SYN-IQ Mapper Analyzer V18</h1>
...
def render_plotly_agent_view(graph, df, valid_indices,
    ...
    # =========================================================================
    # V18.2 — LENS-SEEDED FORCE LAYOUT WITH PER-COMPONENT SEPARATION
    # -------------------------------------------------------------------------
    # Goal: match the SHAPE produced by kepler-mapper's built-in D3 force HTML
    # renderer, so toggling color modes never silently changes the topology.
    #
    # Why the previous attempts failed:
    #   V18.0 used pure spring_layout (random init) → different shape every
    #     time; arbitrary geometry.
    #   V18.1 used pure lens-coordinate centroids → connected components that
    #     happen to have similar lens values get visually overlapped, and
    #     disconnected components don't separate. The legacy D3 view shows
    #     three distinct islands; centroids put them on top of each other.
    #
    # V18.2 fix:
    #   1. Identify connected components of the Mapper graph.
    #   2. Within EACH component, seed node positions from their lens-space
    #      centroids (so geometry remains tied to the projection).
    #   3. Run Kamada-Kawai force layout PER COMPONENT, seeded by step 2.
    #      KK preserves graph-theoretic distances and matches D3's "edges
    #      should be roughly equal length" objective much better than
    #      Fruchterman-Reingold (spring_layout).
    #   4. PLACE each component in its own region of the canvas, so
    #      disconnected islands separate visually — exactly the way D3 force
    #      shows them, because D3 has no force connecting disconnected
    #      subgraphs.
    #
    # Result: the V18 pie/overlap view reproduces the legacy topology shape
    # (separate islands, chain structure, etc.) while keeping deterministic
    # output and lens-aware seeding.
    # =========================================================================
    pos = None
    if len(G.nodes) > 0:
        ...
        # Step 2 — compute lens centroid for every node (used as seed)
        lens_centroid = {}
        if projected is not None and len(projected) > 0:
            proj = np.asarray(projected)
            n_dim = proj.shape[1] if proj.ndim > 1 else 1
            for node_id, indices in nodes.items():
                if len(indices) == 0:
                    lens_centroid[node_id] = np.array([0.0, 0.0])
                    continue
                if n_dim >= 2:
                    pts = proj[indices]
                    lens_centroid[node_id] = np.array([
                        float(np.mean(pts[:, 0])),
                        float(np.mean(pts[:, 1])),
                    ])
                else:
                    flat = proj.ravel() if proj.ndim > 1 else proj
                    lens_centroid[node_id] = np.array([
                        float(np.mean(flat[indices])),
                        0.0,
                    ])
        else:
            # No lens — use zero seed; KK will spread from there
            for node_id in nodes:
                lens_centroid[node_id] = np.array([0.0, 0.0])

        # Step 3 — Kamada-Kawai per component, seeded by lens centroids.
        # KK requires the subgraph to be connected (which by definition each
        # component is). For single-node components we just place at the
        # seed. For 2+ node components we run KK with the seed as `pos`.
        component_layouts = []
        for comp in components:
            comp_nodes = list(comp)
            if len(comp_nodes) == 1:
                # Trivial component — place at its seed
                nid = comp_nodes[0]
                local = {nid: tuple(lens_centroid[nid])}
            else:
                subG = G.subgraph(comp_nodes).copy()
                seed_pos = {nid: tuple(lens_centroid[nid]) for nid in comp_nodes}
                try:
                    local = nx.kamada_kawai_layout(subG, pos=seed_pos)
                except Exception:
                    # KK can fail on very small / pathological subgraphs;
                    # fall back to spring_layout seeded the same way.
                    local = nx.spring_layout(
                        subG, pos=seed_pos, seed=42, iterations=200,
                        k=1.0/np.sqrt(max(len(comp_nodes), 1)),
                    )
            component_layouts.append((comp_nodes, local))

        # Step 4 — place components in separate regions of the canvas.
        # We arrange component CENTROIDS in a grid, then translate each
        # component's local layout so its centroid sits at the grid slot.
        # This reproduces the visual separation D3 force shows for
        # disconnected subgraphs.
        pos = {}
        if n_components == 1:
            # Single component — use the KK layout directly
            pos = {nid: tuple(xy) for nid, xy in component_layouts[0][1].items()}
        else:
            # Sort components by size descending — biggest gets center slot,
            # smaller ones placed around it. Compute a simple grid:
            component_layouts.sort(key=lambda c: -len(c[0]))
            n_cols = int(np.ceil(np.sqrt(n_components)))
            n_rows = int(np.ceil(n_components / n_cols))
            # Spacing chosen so component-local extents (≈ [-1,1]) don't overlap
            spacing = 2.5
            for i, (comp_nodes, local) in enumerate(component_layouts):
                row = i // n_cols
                col = i % n_cols
                # Center the grid on (0,0)
                offset_x = (col - (n_cols - 1) / 2.0) * spacing
                offset_y = ((n_rows - 1) / 2.0 - row) * spacing
                # Find local centroid for this component, translate so its
                # centroid sits at (offset_x, offset_y)
                coords = np.array([local[nid] for nid in comp_nodes])
                local_cx, local_cy = coords.mean(axis=0)
                for nid in comp_nodes:
                    lx, ly = local[nid]
                    pos[nid] = (lx - local_cx + offset_x,
                                ly - local_cy + offset_y)
    else:
        pos = {}
    ...
    # V18.2 — hide axes for graph modes. Positions come from per-component
    # Kamada-Kawai force layout seeded by lens centroids, so the absolute
    # x/y values are not directly interpretable as lens values. The lens
    # only seeds the initial positions; the force simulation then untangles
    # the graph for legibility (matching what kepler-mapper's D3 view does).
    _axis_kwargs = dict(
...
def run_mapper_analysis(data, df, valid_indices, n_cubes, overlap, eps, min_samples, projection_type):
    ...
    # V18.1 — also return the lens coordinates so downstream renderers
    # (e.g. pie composition, overlap=orange) can place nodes at the
    # centroid of their member points in lens space. This is what makes
    # the graph geometry STABLE across color modes and SEMANTICALLY tied
    # to the chosen projection (PCA2D, Q_t, A_t, etc.) rather than to a
    # force-simulation artefact.
        ...
        if st.button("🚀 Run Batch Analysis", type="primary"):
            ...
            for idx, f in enumerate(uploaded_files):
                ...
                try:
                    ...
                    # V18.1 — now returns (mapper, graph, projected);
                    # batch mode doesn't need the lens coords here, so we
                    # collect into _ to keep the unpack honest.
                    _mapper, _graph, _ = run_mapper_analysis(
                        _data, _df, _valid,
    ...
    if df is not None:
        ...
        if st.button("🚀 Run Mapper Analysis", type="primary"):
            ...
            with st.spinner("Running Mapper algorithm..."):
                mapper, graph, projected = run_mapper_analysis(
            ...
            with st.spinner("Generating visualization..."):
                ...
                if _is_v18_proj or _is_v18_color:
                    ...
                    # The graph modes (pie / overlap-orange) also use them
                    # now (V18.1) — node positions = centroids in lens space.
                    _v18_projected = None
                    ...
                    if _is_v18_proj:
                        ...
                        # V18.1 — use the lens coordinates returned by
                        # run_mapper_analysis as the layout substrate for
                        # the graph modes. Guarantees identical geometry
                        # across every V18 color mode and a meaningful
                        # spatial axis (the projection lens itself).
                        _v18_projected = projected
                    ...
                    if _fig is not None:
                        ...
                        st.caption(
                            f"V18.2 render · mode={_v18_mode} · "
                            f"purity_threshold={purity_threshold:.0%} · "
                            f"layout = Kamada-Kawai force per connected component, "
                            f"seeded by {projection_type} lens centroids · "
...
<div style="text-align: center; color: #a0a0a0; padding: 1rem;">
    <strong>SYN-IQ Mapper Analyzer V18</strong><br>
