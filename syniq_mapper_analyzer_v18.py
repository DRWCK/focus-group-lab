...
def render_plotly_agent_view(graph, df, valid_indices,
    ...
    # =========================================================================
    # V18.1 — LENS-COORDINATE CENTROID LAYOUT (the geometry fix)
    # -------------------------------------------------------------------------
    # Previously this used nx.spring_layout, which is a force simulation and
    # therefore produces a DIFFERENT geometry from kepler-mapper's D3 force
    # renderer even on the same graph. That meant toggling between
    # "agent" (legacy HTML) and "agent (pie composition)" silently changed
    # the SHAPE of the topology. Bad for science.
    #
    # The fix: place each node at the CENTROID of its member points in the
    # current lens (`projected`) coordinate system. Consequences:
    #   • Geometry is stable across every V18 color mode.
    #   • Position is interpretable — "near on screen" == "near in lens space".
    #   • Switching lens (PCA2D → Q_t → A_t) visibly moves nodes to their
    #     true positions in the new lens, not to wherever a spring settled.
    #   • Edges may stretch when connected nodes have very different lens
    #     coordinates — that is a *feature*, exposing where the topology
    #     bridges distant regions of lens space.
    # If the lens is 1D (PCA1, AFF%, INT%, ACT%, S_t/A_t/Q_t/D_t/R_t single-
    # axis lenses) we synthesize a y-axis from a node-id hash for vertical
    # separation, keeping the x-axis honest to the lens. If no lens is
    # available at all, we fall back to spring_layout (legacy behavior).
    # =========================================================================
    pos = None
    if projected is not None and len(projected) > 0:
        ...
        pos = {}
        if n_dim >= 2:
            # 2D+ lens — use centroids directly
            ...
            # 1D lens — x = centroid of lens, y = deterministic spread
            # so connected nodes don't all collapse onto one horizontal line
            flat = proj.ravel() if proj.ndim > 1 else proj
            sorted_node_ids = sorted(nodes.keys())
            y_lookup = {nid: i for i, nid in enumerate(sorted_node_ids)}
            n_nodes_total = max(len(sorted_node_ids), 1)
            for node_id, indices in nodes.items():
                if len(indices) == 0:
                    cx = 0.0
                else:
                    cx = float(np.mean(flat[indices]))
                # Spread y across [-1, 1] in a deterministic order so the
                # layout is reproducible and connected nodes get separated
                cy = (y_lookup[node_id] / n_nodes_total) * 2.0 - 1.0
                pos[node_id] = (cx, cy)
    else:
        # Fallback: no lens coordinates available — keep legacy spring layout
    ...
    # V18.1 — make axes visible with lens-aware labels for graph modes,
    # since node positions are now centroids in lens space (not arbitrary
    # spring-layout coords). This documents the semantics on the figure.
    if projected is not None and len(projected) > 0:
        _proj_arr = np.asarray(projected)
        _proj_dim = _proj_arr.shape[1] if _proj_arr.ndim > 1 else 1
        if _proj_dim >= 2:
            _xlab = f"{projection_type} · axis 1"
            _ylab = f"{projection_type} · axis 2"
            _axis_kwargs = dict(
                xaxis=dict(visible=True, title=_xlab, showgrid=True,
                           gridcolor='#eeeeee', zeroline=False),
                yaxis=dict(visible=True, title=_ylab, showgrid=True,
                           gridcolor='#eeeeee', zeroline=False,
                           scaleanchor='x', scaleratio=1),
            )
        else:
            _xlab = f"{projection_type} (lens value)"
            _axis_kwargs = dict(
                xaxis=dict(visible=True, title=_xlab, showgrid=True,
                           gridcolor='#eeeeee', zeroline=False),
            ...
            with st.spinner("Generating visualization..."):
                ...
                if _is_v18_proj or _is_v18_color:
                    ...
                    if _fig is not None:
                        ...
                        st.caption(
                            f"V18.1 render · mode={_v18_mode} · "
                            f"purity_threshold={purity_threshold:.0%} · "
                            f"node position = centroid of member points in "
                            f"{projection_type} lens space · "
