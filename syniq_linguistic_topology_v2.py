...
with tabs[7]:
    ...
    - Find **peer-reviewed evidence** linking each word to INT, AFF, or ACT
...
The IEP framework classifies language into three dimensions:
- INT (Intellectual): analytical, logical, reasoning, knowledge-based language
- AFF (Affective): emotional, empathic, feeling-based language  
- ACT (Action): pragmatic, solution-oriented, behavioral language
...
VALIDATION_SUMMARY: [1 sentence suitable for a methods section of a paper.]"""
                ...
                confirmed = [r for r in results if r['conflict']=='NO' and r['validated_dim'] not in ['ERROR','']]

                col1, col2, col3 = st.columns(3)
                with col1:
                    st.markdown(f'<div class="metric-card"><div class="val">{len(confirmed)}</div><div class="lbl">Confirmed</div></div>', unsafe_allow_html=True)
                with col2:
                    st.markdown(f'<div class="metric-card"><div class="val">{len(conflicts)}</div><div class="lbl">Conflicts Found</div></div>', unsafe_allow_html=True)
                with col3:
                ...
                for r in results:
                    ...
                    conf_color = "#7eb8ff"
                    border_color = "#ff4444" if r['conflict']=='YES' else "#1a4a1a"

                    dim_colors = {'INT':'#4488ff','AFF':'#ff6688','ACT':'#44ff88','ERROR':'#888888'}
                ...
                export_cols = ['word','suggested_dim','validated_dim','confidence',
                ...
                st.caption("Import into Dictionary Builder V2 — validated_dim column overrides suggested_dim for confirmed words")
...
<div style="text-align:center; color:#444488; font-family:'JetBrains Mono',monospace; font-size:0.75rem; padding:1rem;">
    SYN-IQ Linguistic Topology Analyzer V2 · Tennessee 🎹 CUZ Partnership · SYNINT March 2026<br>
