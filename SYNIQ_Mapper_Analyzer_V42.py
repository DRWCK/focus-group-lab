"""
SYN-IQ Mapper Analyzer V42 — Streamlit App
...
with _v40_tab4:
    ...
    if _v41_df is None:
        ...
        if not _v41_qcol:
            ...
            _v41_lens = _b1.selectbox(
                "Lens for every panel", [
                    "Word TF-IDF (2D)", "PCA (2D)", "Embeddings (2D)",
                    "Sentence Embeddings (2D)",
                ], index=0, key="v41_lens",
                help="Held constant across panels — a figure whose panels use "
                     "different lenses is not one figure.")
            _v41_preset = _b2.selectbox(
                "Feature set", list(V23_FEATURE_PRESETS.keys()),
                index=list(V23_FEATURE_PRESETS.keys()).index("All 12 features")
                if "All 12 features" in V23_FEATURE_PRESETS else 0,
                key="v41_preset")
            _p1, _p2, _p3, _p4 = st.columns(4)
            _v41_cubes = _p1.number_input("N cubes", 3, 40, int(n_cubes), key="v41_cubes")
            _v41_ov = _p2.number_input("Overlap", 0.1, 0.9, float(overlap), 0.05, key="v41_ov")
            _v41_eps = _p3.number_input("DBSCAN eps", 0.1, 10.0, float(eps), 0.1, key="v41_eps")
            _v41_ms = _p4.number_input("min_samples", 2, 20, int(min_samples), key="v41_ms")
            st.caption(
                "Cover parameters are held constant across all panels, as the "
                "Beyond Words figures require."
            )
                ...
                for _i, _q in enumerate(_v41_pick):
                    try:
                        _sub = (_v41_df[_v41_df[_v41_qcol].astype(str) == _q]
                                .reset_index(drop=True))
                        _keys = V23_FEATURE_PRESETS.get(_v41_preset)
                        _dat, _vi = build_iep_features(_sub, _keys)
                        if len(_dat) == 0:
                            raise ValueError("no valid feature rows")
                        _dat = StandardScaler().fit_transform(_dat)
                        _mp, _g = run_mapper_analysis(
                            _dat, _sub, _vi, int(_v41_cubes), float(_v41_ov),
                            float(_v41_eps), int(_v41_ms), _v41_lens)
                        ...
                        _panel["feature_set"] = _v41_preset
