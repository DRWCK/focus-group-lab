...
if len(agents) < 2:
    st.error(f"Only one agent found ({agents}). This tool needs all agents pooled in one "
             "CSV. Combine the four per-agent V57 files (or check the 'agent' column / "
             "temperature filter) and re-upload.")
    st.stop()
...
for a, b in itertools.combinations(agents, 2):
    ...
    if len(np.unique(lab)) < 2 or m.sum() < 4:
        continue
