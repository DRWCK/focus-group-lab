"""
SYNERGISTIC INTELLIGENCE FOCUS GROUP — V25
...
st.set_page_config(page_title="Focus Group V25", page_icon="🎯", layout="wide")
...
def check_password():
    ...
    <div style="text-align: center; padding: 3rem;">
        <h1>🎯 Focus Group Lab — V25</h1>
        <h3>True Native Mode + EPM Scoring Fix</h3>
...
def calculate_syniq_quick(responses, synthesis):
    ...
    # V25 FIX: Single measure, no double-counting
...
def calculate_syniq_full(responses, synthesis, api_key, solved_status="Partial"):
    """
    V25 FULL SYN-IQ calculation with EPM (requires OpenAI API).
...
- Prioritize insight over rigid precision"""
        ...
        return ""  # TRUE NATIVE: No tone framing at all
...
def build_system_prompt(tone_key, depth_key, role_context="", knowledge_text=""):
    ...
    if tone_prompt:  # Only add tone section if not Native (TRUE NATIVE has no framing)
        return base + tone_prompt + "\n\n" + depth_prompt
    else:
        return base + depth_prompt

def build_simple_system_prompt(cognitive_value, depth_value, role_context="", knowledge_text="", relational_mode=False, true_native=False):
    base = "You are participating in a research study on AI cognition.\n\n"
    ...
    # TRUE NATIVE: No cognitive framing at all
    if true_native or cognitive_value is None:
...
# ============================================
    ...
    with col1:
        ...
        true_native_mode = st.checkbox("🧬 True Native (no framing)", value=False, key="true_native_checkbox")
        if true_native_mode:
            cognitive_value = None  # Signal for no cognitive framing
            st.markdown('<div style="background-color: #9b59b620; color: #9b59b6; padding: 0.5rem; border-radius: 5px; text-align: center; font-weight: bold;">TRUE NATIVE</div>', unsafe_allow_html=True)
        else:
            ...
            if not question:
                ...
                with st.status("🔄 Getting responses...", expanded=True) as status:
                    for i, agent in enumerate(active_agents):
                        ...
                        system_prompt = build_simple_system_prompt(cognitive_value, depth_value, role_context, agent_kb, true_native=(cognitive_value is None))
...
# ============================================
    ...
    if st.session_state.matrix_data:
        ...
        export_md = f"""# MATRIX-IQ V25 EXPERIMENT REPORT
## {timestamp}
...
# ============================================
    ...
    st.markdown('<div class="boardroom-header"><h2>🏛️ BOARDROOM V25</h2><p>Multi-Round Discussion + EPM Scoring</p></div>', unsafe_allow_html=True)
            ...
            if not document_input and not uploaded_image:
                ...
                for round_idx in range(num_rounds):
                    for agent in active_agents:
                        ...
                        # V25: Adjust prompt for subsequent rounds
                        if round_idx == 0:
    ...
    if st.session_state.boardroom_history:
        ...
        # V25: Full SYN-IQ with EPM
        if auto_analyze:
            st.markdown("---")
            st.markdown("### 🔬 SYN-IQ Analysis (V25 EPM)")
...
<div style="text-align: center; color: #666;">
    <em>Focus Group Lab V25 — True Native Mode + EPM Scoring Fix</em><br>
