...
🎯 TRUE NATIVE MODE — NO COGNITIVE PROMPTING

NEW IN V25:
- 🧠 TRUE NATIVE MODE: No cognitive/tone prompts — let each AI be itself
- 🎯 AGENT-CALIBRATED: Each AI starts from its true baseline
- 📊 CLEAN BASELINE: Native responses reflect actual AI personality, not instructions
- ✨ Based on IEP baseline data (Jan 24, 2026):
    - Gemini: 68% Intellectual (coldest)
    - Sophia: 31% Affective (warmest) 
    - Claude: 55/27 Balanced
    - Grok: 24% Action (most action-oriented)

PRESERVED FROM V24:
- 🧬 EPM SCORING: PFR + Semantic Distance in scoring
- 🔄 MULTI-ROUND BOARDROOM: Configurable rounds (1-5)
- ✅ SOLVE STATUS: Quick dropdown to rate outcome
- 📊 FULL V4 SCORING: Proper weighted formula
- 🔬 INTEGRATED SYN-IQ SCORING
- 📊 REAL-TIME EPM
- 🔥 RELATIONAL MODE (⚠️ confounded - use with caution)
- 👁️ SEEING TOGGLE
- 📚 KNOWLEDGE BASE
...
- Systematic approach"""
        ...
        return ""  # V25: TRUE NATIVE = NO PROMPT - let AI be itself
...
- Prioritize insight over rigid precision"""
    ...
    else:  # native - V25: TRUE NATIVE = NO PROMPT
        return ""  # Let the AI be itself - no instructions
...
def build_system_prompt(tone_key, depth_key, role_context="", knowledge_text=""):
    # V25: TRUE NATIVE mode = minimal framing
    tone_prompt = get_tone_prompt(tone_key)
    
    # If native mode (empty tone), just send the question with minimal context
    if not tone_prompt:
        base = ""
        if role_context.strip():
            base += f"CONTEXT: {role_context}\n\n"
        if knowledge_text:
            base += build_knowledge_context(knowledge_text)
        return base  # No cognitive instructions at all
    
    # Non-native modes: full prompt structure
...
def build_simple_system_prompt(cognitive_value, depth_value, role_context="", knowledge_text="", relational_mode=False):
    ...
    # V25: TRUE NATIVE mode (balanced slider) = minimal framing
    if not cognitive_prompt and not relational_mode:
        base = ""
        if role_context.strip():
            base += f"CONTEXT: {role_context}\n\n"
        if knowledge_text:
            base += build_knowledge_context(knowledge_text)
        return base  # No cognitive instructions - let AI be itself
