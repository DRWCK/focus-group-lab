...
SECRET_MAP = {
    "Claude":   ["anthropic", "ANTHROPIC_API_KEY", "anthropic_api_key", "ANTHROPIC"],
    "ChatGPT":  ["openai", "OPENAI_API_KEY", "openai_api_key", "OPENAI"],
    "Grok":     ["xai", "XAI_API_KEY", "xai_api_key", "XAI", "grok"],
    "Gemini":   ["google", "GOOGLE_API_KEY", "google_api_key", "GOOGLE", "gemini"],
}


def get_key(agent: str) -> str | None:
    for secret in SECRET_MAP[agent]:
        try:
            val = st.secrets.get(secret)
            if val:
                return val
        except Exception:
            continue
...
for agent in selected:
    if not get_key(agent):
        missing_keys.append(f"{agent} (tried: {', '.join(SECRET_MAP[agent])})")
