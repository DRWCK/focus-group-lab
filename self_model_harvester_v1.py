...
if not st.session_state.authenticated:
    st.markdown("""
    <div style="max-width:400px;margin:8rem auto;background:#0D0D0F;
         border:1px solid #2A2A3A;border-radius:12px;padding:2.5rem;text-align:center;">
        <div style="font-family:JetBrains Mono,monospace;color:#7C3AED;
                    font-size:1.4rem;margin-bottom:0.5rem;">🧠 SYN-IQ</div>
        <div style="color:#6B6B80;font-size:0.85rem;margin-bottom:1.5rem;">
            Self-Model Harvester v1.0</div>
    </div>
...
def clean_preview(text: str, max_chars: int = 350) -> str:
    """Strip markdown formatting and truncate for display."""
    import re as _re
...
def render_transcript():
    if not st.session_state.transcript:
        return
    for entry in reversed(st.session_state.transcript[-20:]):
        agent = entry["agent"]
        css = agent_css_class(agent)
        cfg = AGENTS[agent]
        preview = clean_preview(entry["text"])
        # Escape HTML chars in preview
        preview = preview.replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;")
        st.markdown(f"""
<div style="background:#13131A;border:1px solid #2A2A3A;border-radius:6px;
            padding:0.8rem 1rem;margin-bottom:0.5rem;font-family:'JetBrains Mono',monospace;">
  <div style="font-size:0.7rem;color:#6B6B80;margin-bottom:0.4rem;">
    <span style="color:{cfg['color']};font-weight:600;">{cfg['emoji']} {agent}</span>
    &nbsp;·&nbsp; Run {entry['run']} of {RUNS_PER_AGENT}
    &nbsp;·&nbsp; {entry['ts']}
    &nbsp;·&nbsp; {entry['length']} chars
  </div>
  <div style="font-size:0.78rem;color:#E8E8F0;white-space:pre-wrap;line-height:1.6;">{preview}</div>
</div>
""", unsafe_allow_html=True)
