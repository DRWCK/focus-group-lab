...
with tabs[2]:
    ...
    if 'scored' not in st.session_state:
        ...
        def simplex_fig(title, x, y, scored, color="#44ff88"):
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=[0,1,0.5,0],y=[0,0,np.sqrt(3)/2,0],
                mode='lines',line=dict(color='#334466',width=1),showlegend=False))
            for px2,py2,lbl in [(0,0,'INT'),(1,0,'AFF'),(0.5,np.sqrt(3)/2,'ACT')]:
                fig.add_annotation(x=px2,y=py2,text=f'<b>{lbl}</b>',showarrow=False,font=dict(color='#7eb8ff',size=12))
            if 'question_id' in scored.columns:
                for q in scored['question_id'].unique():
                    mask=scored['question_id']==q
                    fig.add_trace(go.Scatter(x=x[mask],y=y[mask],mode='markers',name=q,
                        marker=dict(color=qcolors.get(q,'#888888'),size=5,opacity=0.7)))
            fig.update_layout(title=dict(text=title,font=dict(color=color,size=14)),
                paper_bgcolor='#060b14',plot_bgcolor='#060b14',
                font=dict(color='#c8d8e8',family='JetBrains Mono'),
                height=350,margin=dict(l=10,r=10,t=40,b=10),
                xaxis=dict(showgrid=False,showticklabels=False,zeroline=False),
                yaxis=dict(showgrid=False,showticklabels=False,zeroline=False),
                legend=dict(bgcolor='#0a1020',bordercolor='#1a2a4a',font=dict(size=9)))
            return fig
        
        col1,col2,col3 = st.columns(3)
        with col1:
            x3,y3 = to_2d(scored,'int_v3','aff_v3','act_v3')
            st.plotly_chart(simplex_fig("V3 — Word Lexical",x3,y3,scored,"#44ff88"), use_container_width=True)
        with col2:
            x4,y4 = to_2d(scored,'int_v4','aff_v4','act_v4')
            st.plotly_chart(simplex_fig("V4 — Word POS-Aware",x4,y4,scored,"#4488ff"), use_container_width=True)
        with col3:
            x5,y5 = to_2d(scored,'int_v5','aff_v5','act_v5')
            st.plotly_chart(simplex_fig("V5 — Phrase Level",x5,y5,scored,"#ffaa44"), use_container_width=True)
        
        # Centroid distances
        st.markdown("### Centroid Distance — All Three")
        center_x=0.5; center_y=np.sqrt(3)/6
        d3=np.sqrt((np.mean(x3)-center_x)**2+(np.mean(y3)-center_y)**2)
        d4=np.sqrt((np.mean(x4)-center_x)**2+(np.mean(y4)-center_y)**2)
        d5=np.sqrt((np.mean(x5)-center_x)**2+(np.mean(y5)-center_y)**2)
        col1,col2,col3 = st.columns(3)
