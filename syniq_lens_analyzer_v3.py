...
st.set_page_config(
    page_title="SYN-IQ · Vₜ Lens Analyzer v2",
    page_icon="🔬",
    layout="wide"
)
...
st.markdown("""
<div class="hero">
    <h1>🔬 SYN-IQ · Vₜ Lens Analyzer v2</h1>
...
with st.sidebar:
    ...
    sel_2d = {}
    for pair in VT_LENSES_2D:
        label = f"{pair[0]} × {pair[1]}"
        default = pair in [("S_t","R_t"),("Q_t","D_t"),("A_t","R_t")]
...
# ── Load and merge CSVs ───────────────────────────────────────────────────────
REQUIRED_COLS = {"int_pct","aff_pct","act_pct","S_t","A_t","Q_t","D_t","R_t"}
...
if active_2d:
    ...
    if pair_rows:
        pair_df = pd.DataFrame(pair_rows).sort_values("Max distance", ascending=False)
    ...
    for agent in agents:
        for cond in conditions:
            ...
            breakdown_rows.append({
                ...
                "S_t":  round(sub["S_t"].mean(), 3),
                "A_t":  round(sub["A_t"].mean(), 3),
                "Q_t":  round(sub["Q_t"].mean(), 3),
                "D_t":  round(sub["D_t"].mean(), 3),
                "R_t":  round(sub["R_t"].mean(), 3),
            })
    if breakdown_rows:
...
st.markdown(
    ...
    "SYN-IQ · Vₜ Lens Analyzer v2 · Kouns, W. C. (2026) · SYNINT.AI · Tennessee 🎹 · "
