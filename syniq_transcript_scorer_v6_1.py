...
st.set_page_config(page_title="SYN-IQ Transcript Scorer v6.1", layout="wide")
st.title("🔬 SYN-IQ Transcript Scorer v6.1")
st.markdown(
    ...
    f"V6.1 fixes Claude.ai docx-export parsing and ports the V5.2 preview-line dedup."
)
...
if not turn_df.empty:
    ...
    with e1:
        ...
        st.download_button("📥 Turn-level CSV", csv_turns,
                           file_name=f"transcript_v6_1_{session_id}_turns.csv", mime="text/csv")
    with e2:
        csv_speaker = speaker_summary.to_csv(index=False).encode("utf-8")
        st.download_button("📥 Speaker summary CSV", csv_speaker,
                           file_name=f"transcript_v6_1_{session_id}_speaker.csv", mime="text/csv")
    with e3:
        csv_combined = combined_summary.to_csv(index=False).encode("utf-8")
        st.download_button("📥 Combined summary CSV", csv_combined,
                           file_name=f"transcript_v6_1_{session_id}_combined.csv", mime="text/csv")

st.markdown("---")
st.caption(
    f"SYN-IQ Transcript Scorer v6.1 · syniq_core v{CORE_VERSION} · "
