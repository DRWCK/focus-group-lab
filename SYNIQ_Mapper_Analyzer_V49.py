...
(function() {
  ...
  function render(paths) {
    ...
    // Fit once, as soon as the layout stops moving.
    (function autoFit(){
      ...
      var iv = setInterval(function(){
        ...
        try {
          ...
          if (!sv) { if (tries > 60) clearInterval(iv); return; }
          var xs = [], ys = [];
          sv.querySelectorAll('path, circle').forEach(function(el){
            var d = el.__data__;
            if (d && typeof d.x === 'number') { xs.push(d.x); ys.push(d.y); }
          });
          if (xs.length < 3) { if (tries > 60) clearInterval(iv); return; }
          var cur = [Math.min.apply(null, xs), Math.max.apply(null, xs),
                     Math.min.apply(null, ys), Math.max.apply(null, ys)];
          if (prev && cur.every(function(v, i){ return Math.abs(v - prev[i]) < 1; })) {
            stable++;
          } else { stable = 0; }
          prev = cur;
          if (stable >= 2 || tries > 60) { clearInterval(iv); window.v49Zoom('fit'); }
