"""
SYN-IQ Mapper Analyzer V48 — Streamlit App
...
(function() {
  ...
  function render(paths) {
    ...
    (function addDumpButton(){
      ...
      // V47 — fit the settled graph into the viewport. KeplerMapper starts at
      // zoom 1 regardless of how far the simulation spreads, so a graph that
      // settles wider than the frame is simply cut off. This sets a single d3
      // zoom transform; it changes the view only. Node coordinates are
      // untouched, so a position capture taken after fitting is identical to
      // one taken before.
      var fit = document.createElement('button');
      fit.id = 'v47-fit-btn';
      fit.textContent = '\u2922 Fit to window';
      fit.style.cssText = btn.style.cssText.replace('left:14px', 'left:210px');
      fit.onmouseover = function(){ fit.style.background = '#f0f0f0'; };
      fit.onmouseout  = function(){ fit.style.background = '#fff'; };
      fit.onclick = function(){
        var ok = window.v47Fit && window.v47Fit(true);
        if (!ok) {
          v40Banner('Could not fit: the graph viewer did not expose its zoom '
            + 'controls. Try again once the graph has finished drawing.',
            '#b3261e');
        }
      };
    ...
    window.v47Fit = function(animate){
      try {
        // V48 — reach for KeplerMapper's globals through `window` explicitly.
        // This function is defined inside the injected render(), which
        // declares its own `var g` for the pie wedge group; hoisting made the
        // bare name `g` resolve to that local, so the V47 guard saw undefined
        // and returned silently every time. Nothing failed loudly because
        // nothing failed at all — it just never looked at the right object.
        var G = window.g, SVG = window.svg, ZM = window.zoom, D3 = window.d3;
        if (!G || !SVG || !ZM || !D3) {
          console.warn('v48Fit: KeplerMapper globals not reachable',
                       {g: !!G, svg: !!SVG, zoom: !!ZM, d3: !!D3});
          return false;
        }
        var gn = G.node(), sn = SVG.node();
        if (!gn || !sn) return false;
        var b = gn.getBBox();
        if (!b || b.width < 1 || b.height < 1) return false;
        var r = sn.getBoundingClientRect();
        var W = r.width  || +SVG.attr('width')  || 900;
        var H = r.height || +SVG.attr('height') || 700;
        // 0.92 leaves a margin so the outermost nodes are not clipped by
        // their own radius, which the bounding box already includes.
        var k = Math.min(W / b.width, H / b.height) * 0.92;
        k = Math.max(0.1, Math.min(7, k));          // honour scaleExtent
        var tx = W / 2 - k * (b.x + b.width  / 2);
        var ty = H / 2 - k * (b.y + b.height / 2);
        var t = D3.zoomIdentity.translate(tx, ty).scale(k);
        if (animate) { SVG.transition().duration(400).call(ZM.transform, t); }
        else { SVG.call(ZM.transform, t); }
        return true;
      } catch (e) { console.error('v47Fit', e); return false; }
    };

    // Auto-fit once, as soon as the layout stops moving. Polls displacement
    // rather than a fixed delay, since settling time depends on node count.
    (function autoFit(){
      ...
      var iv = setInterval(function(){
        ...
        try {
          ...
          prev = cur;
          if (stable >= 2) { clearInterval(iv); window.v47Fit(true); }
          if (tries > 60) { clearInterval(iv); window.v47Fit(false); }
