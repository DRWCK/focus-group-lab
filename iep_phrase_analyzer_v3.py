"""
iep_phrase_analyzer_v3.py — IEP Phrase-Level Analysis (v3.0.1)
================================================================

CHANGELOG
---------
v3.0.1 (May 10, 2026 — spaCy load latency fix)
  Diagnosed: "won't load" on first page hit. Root cause was the
  synchronous spaCy + en_core_web_sm load running BEFORE the title
  rendered, so the user saw a blank Streamlit shell for 10-30 seconds
  on cold start while the 50MB model loaded from disk. Worse, the
  module-level _SPACY_STATUS guard didn't survive Streamlit reruns
  reliably across all versions, so subsequent page interactions could
  re-trigger the load.

  Two surgical fixes:
  1. @st.cache_resource on _load_spacy_cached(). Model load runs ONCE
     per Streamlit server lifetime, not once per page load. The
     decorator's built-in spinner ("Loading spaCy model...") gives the
     user visible feedback during the one-time cold load.
  2. _load_spacy() call moved to AFTER st.title() renders. User sees
     the app header immediately; spaCy load happens visibly underneath
     instead of behind a blank page.

  No detection logic changed. Paragraph analyzer v4 unaffected
  (doesn't use spaCy).
...
def _load_spacy():
    ...
    v3.0.1: Streamlit reruns the script on every interaction. Without
    @st.cache_resource, the module-level globals get re-evaluated on each
    rerun in some Streamlit versions, and the en_core_web_sm load (~5-15s
    on first call) reblocks every interaction. We delegate the actual
    load to a cached helper and just expose globals for status reporting."""
    global _SPACY_NLP, _SPACY_STATUS, _SPACY_VERSION
    if _SPACY_STATUS != 'unknown':
        return _SPACY_NLP
    _SPACY_NLP, _SPACY_STATUS, _SPACY_VERSION = _load_spacy_cached()
    return _SPACY_NLP


@st.cache_resource(show_spinner="Loading spaCy model (one-time, ~10s)...")
def _load_spacy_cached():
    """Cached across reruns AND across sessions. The en_core_web_sm model
    is ~50MB and takes 5-15s to load on cold disk. With @st.cache_resource,
    this runs ONCE per Streamlit server lifetime, not once per page load."""
    try:
        import spacy
        version = spacy.__version__
        try:
            nlp = spacy.load('en_core_web_sm')
            return nlp, 'ok', version
        except OSError:
            return None, 'no_model', version
    except ImportError:
        return None, 'no_spacy', None
    except Exception:
...
st.set_page_config(page_title="IEP Phrase Analyzer v3.0.1",
...
def main():
    ...
    st.title("🔗 IEP Phrase Analyzer  ·  v3.0.1")
    st.caption("Grammatical phrase detection via spaCy dependency parser. "
               "SSOT: syniq_core. Show why, not just what.")

    # v3.0.1: spaCy preload happens AFTER title renders so user sees the
    # app appear immediately. @st.cache_resource on the loader ensures the
    # ~10s model load runs ONCE per server lifetime, not once per page load.
    # The cache_resource decorator provides its own spinner.
    _load_spacy()
