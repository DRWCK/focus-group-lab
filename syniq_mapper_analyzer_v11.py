...
# =============================================================================
st.set_page_config(
    page_title="SYN-IQ Mapper Analyzer V11",
    page_icon="🗺️",
    layout="wide"
)
...
<div class="main-header">
    <h1>🗺️ SYN-IQ Mapper Analyzer V11</h1>
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 Run Mapper Analysis", type="primary"):
            ...
            with st.spinner("Generating visualization..."):
                ...
                # ── White background via CSS injection (see white_css above) ──
                # Hex replacement kept as belt-and-suspenders fallback
                for dark_hex in ['#212121', '#111111', '#191919', '#000000', '#1a1a1a']:
                    html_content = html_content.replace(dark_hex, '#ffffff')
                    html_content = html_content.replace(dark_hex.upper(), '#ffffff')

                # ── CSS injection — white background guaranteed regardless of JS timing ──
                white_css = """
<style>
  body, html { background: #ffffff !important; }
  svg { background: #ffffff !important; }
  rect { fill: #ffffff !important; }
  .link, line { stroke: #555555 !important; stroke-opacity: 0.7 !important; stroke-width: 1.5px !important; }
  #logo { display: none !important; }
  [id="logo"] { display: none !important; }
</style>
"""
                html_content = html_content.replace('</head>', white_css + '</head>')

                # ── MutationObserver — fires exactly when D3 finishes drawing ─
                pub_script = """
<script>
(function() {
  var palette = [
  ...
  function getCount(circle) {
    // Try title element first (KeplerMapper puts member count there)
    var g = circle.parentNode;
    var title = g ? g.querySelector('title') : null;
    var txt = title ? title.textContent : '';
    // Match "8 members", "n=8", or plain number
    var m = txt.match(/\\b(\\d+)\\s+members?\\b/i) ||
            txt.match(/n\\s*=\\s*(\\d+)/i) ||
            txt.match(/^\\s*(\\d+)\\s*$/);
    if (m) return parseInt(m[1]);
    // Fallback: try r attribute as proxy (KeplerMapper sets r by count)
    var r = parseFloat(circle.getAttribute('r') || '1');
  ...
  function applyStyle() {
    ...
    // Collect counts
    var counts = Array.from(circles).map(getCount);
    var maxCount = Math.max.apply(null, counts.concat([1]));
    var minR = 12, maxR = 36;

    circles.forEach(function(c, i) {
      // Node sizing — square root scale with floor
      var r = minR + (maxR - minR) * Math.sqrt(counts[i] / maxCount);
      c.setAttribute('r', r.toFixed(1));
      // Keep KeplerMapper's own color gradient — it encodes AFF% meaningfully
      // Just ensure stroke is white for clean separation on white background
      c.style.stroke = '#ffffff';
      c.style.strokeWidth = '2px';
      c.style.fillOpacity = '0.92';
    });
    ...
    // Darker edges for white background
    document.querySelectorAll('svg line, svg path.link').forEach(function(el) {
  ...
  // MutationObserver — watches for D3 to add circles to the SVG
  var observer = new MutationObserver(function(mutations) {
    for (var i = 0; i < mutations.length; i++) {
      var nodes = mutations[i].addedNodes;
      for (var j = 0; j < nodes.length; j++) {
        if (nodes[j].nodeName === 'circle' ||
            (nodes[j].querySelector && nodes[j].querySelector('circle'))) {
          // D3 has rendered — apply style and disconnect
  ...
  // Also run on load as safety net
  window.addEventListener('load', function() { setTimeout(applyStyle, 200); });
  document.addEventListener('DOMContentLoaded', function() { setTimeout(applyStyle, 200); });
  // Final safety net for already-rendered content
        ...
        if st.session_state.mapper_analysis_data is not None:
            ...
            if st.session_state.mapper_results_html:
                st.download_button(
                    ...
                    file_name="syniq_mapper_v11_output.html",
                    mime="text/html"
                )
...
<div style="text-align: center; color: #a0a0a0; padding: 1rem;">
    <strong>SYN-IQ Mapper Analyzer V11</strong><br>
