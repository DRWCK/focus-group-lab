"""
SYN-IQ Mapper Analyzer V52 — Streamlit App
Topological Data Analysis for AI Response Profiles

V52 — LAYOUT OVERRIDE OFF BY DEFAULT (reverses V51)
  * V51 pinned coordinates into every node on the premise that KeplerMapper's
    force layout settled differently on each load. That premise was wrong and
    was never tested. d3-force is deterministic: the same graph returns
    identical coordinates on every run, measured at exactly zero difference
    across repeated runs of a 46-node graph. Figures from V20 onward
    reproduced exactly, which is precisely what was reported.
  * Overriding the layout therefore changed every graph's appearance for no
    benefit, and is now off by default. Graphs look as they did before.
  * The genuine defects were unrelated to layout: the viewport was never
    fitted to the window (V49/V50), and the SVG exporter dropped every
    single-architecture node through a degenerate full-turn arc and rescaled
    radii per panel (V40). Those fixes stand.
  * The override is kept as an option for the one case where it helps —
    packing disconnected components so they cannot overlap and imply
    adjacency absent from the nerve.

V51 — FREEZE THE LAYOUT (the fix the previous ten were working around)
  * Root cause: KeplerMapper ships node data with no coordinates, so every
    load runs a fresh force simulation. That single fact produced all of it —
    the download reverting to a live, unfitted figure; the same graph refusing
    to be captured twice at the same scale; five panels that could not be made
    to match; the settle-detection, the capture step, the re-fitting loop.
  * Positions are now computed deterministically in Python and written into
    every node as x/y and fx/fy. d3 treats fx/fy as pinned, so the simulation
    has nothing to solve. The page opens already laid out, identically every
    time, in the app and in the downloaded file alike, and the fit converges
    on the first pass because nothing subsequently moves.
  * The same coordinates feed the SVG figure, so the HTML and the print
    figure are one picture rather than two attempts at it. A position capture
    is no longer needed for a panel, though it still works.
  * Components are laid out separately and packed to the frame aspect, so
    disconnected pieces (beta_0 is a reported statistic) cannot drift into
    each other and imply adjacency that is not in the nerve.
  * Switchable from the sidebar if a live simulation is ever wanted.

V50 — THE FIT HOLDS IN THE DOWNLOADED FILE
  * V49 fitted once. In the app that mostly looked right, because the layout
    was already near-settled by the time the fit ran. In a downloaded HTML the
    simulation starts from scratch, so the fit landed early and the still-
    expanding layout grew straight back out of the frame: the file opened live
    and unfitted every time.
  * The fit is now re-applied whenever the layout moves, and releases once
    three consecutive checks show no movement. A resized window re-fits too,
    since a resize changes what fitting means.
  * Auto-fitting stops the moment the reader pans, zooms or drags a node.
    Overriding a deliberate action is worse than not fitting at all. The zoom
    bar is exempt, so its buttons keep working afterwards.
  * The confirmation banner is suppressed while the layout cools, so the
    repeated fits are silent, and speaks again once the view is the reader's.

V49 — ZOOM BAR: Fit, 100%, 80%, 60%, 40%, 25%
  * V47 and V48 both drove KeplerMapper's own d3 zoom, which meant depending
    on its `g`, `svg` and `zoom` being reachable from the injected scope. They
    were not, and the control did nothing. V49 stops asking: it locates the
    SVG and its container group by walking the DOM and sets the transform
    attribute directly. No d3, no globals, no cooperation from the viewer.
  * If KeplerMapper's zoom does happen to be reachable, its internal state is
    updated afterwards so a later pan or wheel continues from the new
    position rather than snapping back. That step is optional and wrapped;
    the DOM transform stands on its own if it fails.
  * Percentages are relative to the fit, so 60% is reliably smaller than the
    fitted view rather than depending on whatever zoom level happened to be
    current.
  * Every button reports what it did, including the resulting scale. A
    control that silently does nothing cost two versions here.

V48 — FIT-TO-WINDOW ACTUALLY REACHES THE VIEWER (fixes V47)
  * V47's fit never ran. It referred to KeplerMapper's `g`, `svg` and `zoom`
    by bare name, but it is defined inside the injected render(), which
    declares its own `var g` for the pie wedge group. Hoisting made `g`
    resolve to that local, so the guard saw undefined and returned. Nothing
    failed loudly because nothing failed at all — it never looked at the right
    object. The globals are now reached through `window` explicitly.
  * The Fit button now reports when it cannot reach the viewer instead of
    doing nothing, and logs which of the four globals was missing. A control
    that silently does nothing is worse than one that fails.

V47 — FIT THE GRAPH TO THE WINDOW
  * KeplerMapper opens at zoom 1 no matter how far the simulation spreads, so
    a graph that settles wider than the frame is simply cut off and has to be
    scrolled or pinched back into view. The graph now auto-fits once the
    layout stops moving, and a "Fit to window" button re-fits on demand.
  * Implemented as a single d3 zoom transform on the existing container group,
    honouring KeplerMapper's own scaleExtent, with a 0.92 margin so outer
    nodes are not clipped by their own radius. Pan and zoom continue to work
    normally afterwards.
  * The fit changes the view only. Node coordinates are untouched, so a
    position capture taken after fitting is identical to one taken before,
    and figure output is unaffected.
  * Auto-fit polls the layout's bounding box rather than waiting a fixed
    delay, since settling time depends on node count.

V46 — STANDALONE HTML FIGURE EXPORT
  * The figure exports as a self-contained HTML page carrying the same visual
    language and the same hover detail as the KeplerMapper export — cube id,
    response count, architecture breakdown — with the coordinates already
    fixed. KeplerMapper's page is recognisable but not reproducible: a live
    force simulation settles somewhere different on every load, which is the
    reason the same graph could not be captured twice at the same scale. This
    page opens identically every time, on any machine, offline.
  * The page carries a provenance block: scale mode, shared position scale,
    node radius constant, frame, and per-panel node, edge and largest-node
    counts. A reader can check the figure against the reported numbers
    without the tool that made it.
  * Node elements carry data attributes in the SVG itself, so the same file
    remains a plain vector figure when placed in the manuscript.

V45 — FIGURE STYLE AS A FILE
  * Every figure in a paper has to be rendered identically, and six slider
    positions reproduced from memory across separate sessions is not a
    guarantee of that — it is an opportunity to be inconsistent. The render
    settings download as a style profile and load back, so Figure 6 and
    Figure 7 are rendered from the same file rather than the same intention.
    Node radius is k*sqrt(members), so an identical k means a 40-response
    node is the same physical size in both.
  * The style is stamped into every SVG as a trailing comment: k, scale mode,
    frame, padding, stroke widths, label settings. A figure can be checked
    against its siblings later without depending on anyone's notes, and a
    mismatched panel can be identified from the file alone.

V44 — BATCH HONOURS THE FEATURE TYPE RADIO (fixes V41/V43)
  * V41 and V43 always built features with build_iep_features, ignoring the
    sidebar Feature Type radio. With "Embeddings (pre-computed)" or "SBERT"
    selected, the on-screen graph was built from hundreds of embedding
    dimensions while the batch panels were built from twelve IEP features, on
    the same rows — which is enough on its own to turn a 49-node graph into a
    15-node one. The batch now runs the same four-way branch the main path
    runs.
  * The inherited-settings table shows the feature type, and the build result
    table shows the feature dimensionality per panel, so a mismatch between
    the panels and the graph is visible in a number rather than inferable
    from a node count.
  * Each panel records feature_type and n_dims alongside the cover
    parameters.

V43 — ONE SET OF PARAMETERS (fixes V41/V42 batch build)
  * THE BATCH INHERITS THE SIDEBAR SETTINGS. V41 gave the batch tab its own
    lens, feature-set, cover and DBSCAN widgets. Two sets of controls for the
    same computation means the panels can be built from a different
    configuration than the graph on screen, so the same data produces a
    different node count and nothing in the figure records why: a 49-node
    Grief graph rendered as 15 nodes with the source CSV unchanged. Feature
    selection in particular diverged, since the sidebar drives per-feature
    checkboxes while the batch offered presets.
  * The inherited values are displayed above the build button, with the
    feature list expandable, so what the panels will be built from is visible
    before the build rather than inferable after it. They are changed in the
    sidebar, which is where the on-screen graph reads them from.
  * feature_set on each panel now records the actual feature keys rather than
    a preset name, so a panel JSON says what it was built from.

V42 — EDITABLE PANEL CAPTIONS (adds to V41)
  * Panel captions are editable in Figure Builder tab 2, after the panels are
    built. The batch path takes its caption from question_id, so a
    misspelling in that column would otherwise reach all five captions at
    once and be fixable only by rebuilding. The caption is presentation, not
    provenance: the panel key, node data and cover parameters are unchanged
    by editing it, and question_id stays in the run log where a reviewer
    needs it.

V41 — BATCH PANEL BUILD (adds to V40)
  * ALL PANELS FROM ONE BUTTON. A five-panel figure previously meant, per
    panel: set the parameters, switch the lens, build, wait for the browser
    force simulation to settle, copy positions, paste, label, capture — then
    change the lens back and do it again. Figure Builder tab 0 loops the
    questions in Python with one set of parameters held constant across all
    of them, which is what a multi-panel figure requires anyway.
  * LAYOUT COMPUTED IN PYTHON, DETERMINISTICALLY. A force layout is an
    arbitrary embedding of the nerve whichever engine produces it, so there
    is nothing to be gained from the browser's. Kamada-Kawai (deterministic,
    no seed) with seeded spring layout as fallback returns the same
    coordinates every run, so a figure can be regenerated months later and
    match — which a live simulation cannot promise.
  * COMPONENTS LAID OUT SEPARATELY, THEN PACKED. Mapper graphs are routinely
    disconnected (beta_0 is a reported statistic) and a whole-graph force
    layout lets components drift into each other, showing a reader adjacency
    that is not in the nerve. Each component is laid out alone, scaled by
    sqrt(size), and shelf-packed. Packing row width is chosen to match the
    frame aspect: a wide, short arrangement wastes the lower half of the
    panel, and since the shared scale is set by the panel needing the most
    room, wasted area in one panel shrinks every panel in the figure.
  * The V40 browser capture path is retained unchanged, for reproducing an
    arrangement that was tuned by hand in the viewer.

V40 — FIGURE RENDERING MOVES TO PYTHON (replaces the V39 in-browser exporter)
  * THE BROWSER RETURNS x/y AND NOTHING ELSE. V39 assembled the whole figure
    inside the iframe by scraping the live DOM for membership, wedge colors,
    wedge fractions and radii — all of which Python had already computed a
    moment earlier and then thrown away. Four rewrites each repaired one
    scrape and exposed the next. The graph now carries a "Copy node
    positions" button that returns a {node_name: [x, y]} map; the Figure
    Builder at the bottom of the page renders from the panel payload Python
    stashed at build time.
  * PURE NODES NOW DRAW. A single-architecture node has one wedge at
    frac = 1.0, so V39's arc ran a full turn and its start and end points
    coincided. The SVG specification drops an elliptical arc with coincident
    endpoints, so every pure node rendered as nothing and only mixed nodes
    survived — 186 of 245 nodes across the five-question set are
    single-architecture. Full circles are emitted as <circle>.
  * ONE SCALE FOR THE WHOLE FIGURE. A per-panel fit gives each question its
    own scale factor, then multiplies node radii and stroke widths by it, so
    the same 40-response node is a different physical size in each panel —
    the artifact the export was built to prevent, arrived at automatically.
    The position scale is computed once across every panel in the set;
    radius (k·√members), stroke widths and font sizes are in output units and
    are never multiplied by it.
  * RADIUS USES members, NOT kepler's size. V38 documented that kepler's
    `size` field is a drawing bucket which disagreed with true membership on
    61 of 61 nodes; V39's default path still drew from it, so node area did
    not encode what a reviewer reads it as encoding.
  * BOUNDING BOX FROM NODE CENTERS ONLY. V39's absolute mode mixed
    output-pixel radii into a layout-unit box, which is circular: larger
    radii inflate the box, which shrinks the scale, which shrinks the radii.
  * SETTLE TEST BY DISPLACEMENT. V39 tested spread, which only catches nodes
    piled at the center; a fully spread graph can still be drifting. Two
    snapshots 700 ms apart, largest displacement under 0.75 px.
  * PANELS PERSIST. Each captured panel downloads as a JSON, so a five-panel
    figure can be assembled across sessions — necessary, since each question
    needs its own graph build.
  * PCA ALIGNMENT (optional, on by default). A force layout settles at an
    arbitrary rotation per run. Rotating each panel so its principal axis is
    horizontal changes no topology and makes five panels read as a series.

V38 — VALIDATION AND EXPORT INTEGRITY (new in this version)
  * PERMUTATION VALIDATION OF AGENT-PURITY. Optional sidebar toggle, off by
    default, ~1 s for 2,000 relabellings on a 59-node graph. The graph is
    held fixed and only architecture labels are shuffled, so node
    membership, node sizes, the cover and the clusterer are identical under
    the null. This matters because purity has a floor set by the node-size
    distribution — a one-member node scores 1.0 by construction — so the
    null lands near 0.38, not 0, and "0.93 is high" needs that floor stated
    to mean anything. Purity is a descriptive property of a graph with no
    sampling distribution, so an ANOVA cannot supply significance for it;
    permutation manufactures the null the statistic lacks. Two statistics
    are tested: weighted agent-purity, and the count of nodes containing
    all architectures. Reported as perm_* columns in the run log
    (log_schema 3) and as a caption under the graph. Blank when the toggle
    is off, so schema-2 rows stay valid. Failure is caught and never costs
    a completed graph.
  * RUN LOG SURVIVES A SCHEMA BUMP. Appending a wider row to a narrower
    file yields a CSV no parser can read ("expected 29 fields, saw 37") —
    the log_schema integer records a column addition but cannot prevent
    the breakage. The log now detects a header mismatch, takes a
    timestamped .bak, widens the existing rows onto the current columns
    with blanks, and rewrites once. Subsequent writes are plain appends.
    Verified against the real 6-row schema-2 log: 6 rows/29 cols became
    7 rows/37 cols with every prior value intact.
  * `members` ON EVERY EXPORTED NODE. kepler writes a node field called
    `size` that is a drawing bucket, not a membership count. On the
    Consciousness graph `size` ranged 2-4 and summed to 168, while true
    membership ranged 3-46 and summed to 770; the two disagreed on 61 of 61
    nodes. An external script reading `size` as membership drew a
    41-response node as if it held 4 — flattening the node-size range by a
    factor of 2.6 — and the error was silent because both fields are
    plausible small integers. Every exported node now also carries
    `members`, written by all three v22_panel writers plus a backstop sweep
    that falls back to the panel payload and then to the tooltip counts.
    The internal node_table was always correct and is unchanged; this
    closes the hole in the exported artifact only.
  * No user-facing label carries a version number. Figure captions and
    sidebar panels are deliberately clean: a caption sits directly under
    the graph and can be caught in a screenshot bound for a manuscript
    figure. Provenance that must survive lives in the exported HTML title
    and in the run log, both of which are files rather than images. The
    caption still carries harvester and IEP-dictionary versions, which come
    from the data and are what a reviewer needs.

V37 — RUN LOG AND VERSION HYGIENE
      (shipped; produced the 2026-08-09 Grief and Consciousness sweeps,
       log_schema 1-2)
  * RUN-LEVEL METRICS LOG. Every graph build appends one row to a
    persistent CSV (default mapper_runs_log.csv), so a parameter sweep
    produces a pasteable table directly instead of a set of screenshots to
    be transcribed by hand. Append-only; header written once. Columns: log
    meta (run_id, tool_version, log_schema), run identity (timestamp,
    question_id, feature_set, source_csv), parameters (n_cubes,
    perc_overlap, dbscan_eps, dbscan_min_samples, projection,
    feature_scaler, nerve_min_intersection), graph structure (nodes, edges,
    beta_0, largest_component_nodes, total_samples, unique_samples,
    responses_unassigned), purity (agent/question x weighted/unweighted),
    and node composition (nodes_1agent .. nodes_4agent).
    Three failures this removes: table values had to be read off
    screenshots and typed in, which already produced one mislabeled row;
    the overlap value behind a given result was recorded nowhere, so rows
    could not be traced to their runs; and responses dropped by DBSCAN as
    noise were invisible, so purity denominators could not be checked.
    Log is graph-level only, one row per run. Per-node detail stays in the
    separate V28/V29 membership export.
  * `feature_scaler`, not `scaler` (log_schema 2). kepler's own meta block
    reports the PROJECTION scaler, a different stage, so the old name
    looked like it contradicted the exported HTML. Position-preserving
    rename: schema-2 rows append to a schema-1 file without misaligning.
  * SINGLE SOURCE OF TRUTH FOR THE VERSION STRING. TOOL_VERSION is defined
    once, immediately below this docstring, and every place that displays
    or records a version reads from it. V36 shipped with its filename
    reading V36, its docstring header V34 and its page title V29 — three
    disagreeing stamps in one file, which means no output can be traced to
    the code that produced it. That was the second occurrence: V27 shipped
    with docstring, page title, both H1 headers and the footer all still
    reading V25. Searching this file for a quoted version string returns
    exactly one match, the constant itself.

V34 — AGENT KEY BOX (revises V33 panel)
  * The right-side membership panel is replaced by a compact AGENT KEY:
    a static rectangular box, bottom-right, listing each labeled cube under
    its plurality (dominant) agent, colored by agent. Cube numbers only —
    no counts, since the pies already show distribution. Static (not the
    draggable hover panel), so it is publication-safe and exports with the
    figure. Fixes the V33 overlap with the hover card.

V33 — SMALL-NODE LABELS + MEMBERSHIP PANEL (adds to V32)
  * Leader line now requires a real move (>=12px) so tiny/zero offsets on
    small nodes no longer spawn a stray glyph — fixes the '78' doubled look.
  * Label halo scales with font size, so numbers stay legible ON small nodes.
  * NEW: membership panel — a static box on the right (vertically centered)
    listing each LABELED cube and its agent composition. Mirrors the hover
    panel style, updates live with the label selection, exports with the
    figure. Toggle: '📋 Show membership panel'.

V32 — LIVE LABELS (adds to V31)
  * Label size cap raised 20 -> 40 (default 14).
  * Label controls now apply LIVE: size, placement, hide-list, and per-cube
    nudges update the displayed graph WITHOUT rebuilding the mapper. The
    current window.V29_LABELS config is re-injected into the stored HTML on
    every render, so moving a slider updates labels immediately. This fixes
    V30/V31 where label changes only took effect on a full graph rebuild.

V31 — LABEL NUDGE (adds to V30)
  * Per-cube label offset: pick a cube, slide its label off the node with
    dx/dy sliders (±120px). A thin leader line links the nudged label back
    to its node. Fixes overlaps on crowded nodes for publication figures.
  * Offsets accumulate in session_state; 'Clear all nudges' resets.
  * Passed as LBL.offsets in window.V29_LABELS. No other change.

V30 — CUBE LABELS (adds to V29)
  * Sidebar "🔢 Cube Labels": None / All cubes / Selected only.
  * "Selected only" + a cube list labels just those nodes.
  * "Turn OFF these cubes" suppresses specific numbers in any mode.
  * Label size + placement (center/below) controls.
  * Labels are drawn INSIDE the V21 updatePies() tick using each
    node's real d.x/d.y, so they match the on-screen kepler layout
    exactly and export with the figure. Config passed as
    window.V29_LABELS. No change to scoring, coloring, or layout.

V29 (new in that version)
  * Node labels are {QUESTION}-{FEAT}-{LENS}-N{index}, e.g. CONSC-IEAP-PCA2-N001.
    Numbered by TRUE membership descending, so N001 is always the largest node.
    Replaces cube{N}_cluster{M}, which collided across runs and encoded grid
    position rather than anything about the responses.
  * cube_id retained in every export and shown in the Inspect panel.
  * Cover parameters written into every exported CSV (n_cubes, perc_overlap,
    dbscan_eps, dbscan_min_samples, projection, feature_set, feature_preset).
    Previously recorded nowhere and reconstructed from memory.
  * Export filenames are per-run; V28 wrote v28_node_membership.csv every time
    and repeated exports overwrote each other.
  * Node explainer: position and tightness against a matched baseline with a
    2,000-draw size-matched shuffle control, a degenerate-channel guard, and
    a leave-one-out column so single-member leverage is visible.
  * Query subtypes (Q_total, Q_clarifying, Q_invitational, Q_rhetorical)
    picked up automatically when the CSV carries them.

V28
=========================
  * Answer Numbers by Node. Resolves every Mapper node down to the
    individual harvested answers inside it, and every answer to one of
    CLUSTERED / SOLITARY / UNPLACED.
      - SOLITARY   sits in a node by itself.
      - UNPLACED   in no node at all (DBSCAN noise). These are absent
                   from the rendered graph entirely, so before V28 they
                   were invisible rather than merely isolated.
      - lone_of_agent  shares a node but is its agent's only delegate
                   there.
    Cover patches overlap, so one answer can occupy several nodes;
    status reports the strongest condition holding anywhere and
    n_nodes shows the spread.
  * Three tabs: By node, By answer, Inspect one node (with response
    text). Both tables export to CSV.
  * stored_valid_indices now persisted to session_state. V27 did not
    keep it, so the results block had no way to map graph member
    indices back to dataframe rows.
  * Version stamps corrected. V27 shipped with its docstring, page
    title, both H1 headers and the footer still reading V25.
...
# =============================================================================
# Every place that displays or records a version reads from this constant.
# Nothing below hardcodes a tool-version string. Historical references inside
# the changelog docstring above, and internal identifiers naming the version
# that introduced a feature (V23_FEATURE_PRESETS, _v21_lens_to_col, the
# _v21_render_note_parts internals), are deliberately left alone: they name a
# feature's origin in code, not the running tool. No user-facing label carries
# a version number: figure captions and sidebar panels are deliberately clean,
# since a caption can be caught in a screenshot bound for a manuscript figure.
TOOL_VERSION = "V52"
RUNLOG_SCHEMA = 3
RUNLOG_PATH = "mapper_runs_log.csv"
...
# =============================================================================
st.set_page_config(
    page_title=f"SYN-IQ Mapper Analyzer {TOOL_VERSION}",
    page_icon="🗺️",
    layout="wide"
)
...
def check_password():
    ...
    <div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%);
        ...
        <h1 style="color: #e94560;">🗺️ SYN-IQ Mapper Analyzer {TOOL_VERSION}</h1>
...
<div class="main-header">
    <h1>🗺️ SYN-IQ Mapper Analyzer {TOOL_VERSION}</h1>
...
SCHEMA_FAMILIES = {
    ...
    "response_text": ["response_text"],   # V26: gates Word TF-IDF lens
}
...
def _v28_answer_membership(graph, df, valid_indices, id_col="turn_id",
                           label_map=None):
    """V28: resolve every Mapper node down to the individual answer numbers
    inside it, and classify each answer as clustered or solitary.

    Why this exists
    ---------------
    Through V27 the node was the smallest visible unit. You could see that a
    node held 3 Claude and 1 Grok response, but not *which* responses. This
    resolves the node to its members so a point on the graph can be traced
    back to a specific harvested answer.

    Three distinct senses of "alone", all reported separately because they
    answer different questions:

      SOLITARY   the answer sits in a node by itself (node size == 1).
      LONE-AGENT the answer shares a node, but is the only response from its
                 agent in that node — the agent has a single delegate in a
                 crowd of others.
      UNPLACED   the answer landed in no node at all. Under DBSCAN this is
                 noise: it failed min_samples in every cover patch. These
                 never appear on the graph, so without this table they are
                 invisible rather than merely isolated.

    Note on overlap: KeplerMapper covers overlap, so one answer can belong to
    several nodes at once. An answer can therefore be solitary in one node and
    clustered in another. Status reports the strongest condition that holds
    anywhere, and n_nodes shows the spread.

    Returns
    -------
    (node_table, answer_table, summary) : (DataFrame, DataFrame, dict)
    """
    ...
    if id_col not in subset.columns:
        subset = subset.copy()
        subset[id_col] = [f"row{ix}" for ix in valid_indices]

    def _lab(i):
        return subset.iloc[i][id_col]

    has_agent = "agent" in subset.columns
    has_q = "question_id" in subset.columns

    ans_nodes = _dd(list)       # member idx -> [V29 label]
    ans_cubes = _dd(list)       # member idx -> [original cube id]
    ans_sizes = _dd(list)       # member idx -> [size of each node it is in]
    ans_lone_agent = _dd(bool)  # member idx -> lone delegate somewhere

    node_rows = []
    for node_id, members in graph.get("nodes", {}).items():
        mem = [m for m in members if 0 <= m < n]
        size = len(mem)
        if size == 0:
            continue

        agent_of = {m: (subset.iloc[m]["agent"] if has_agent else "?") for m in mem}
        acount = Counter(agent_of.values())

        _lbl = (label_map or {}).get(str(node_id), str(node_id))
        for m in mem:
            ans_nodes[m].append(_lbl)
            ans_cubes[m].append(str(node_id))
            ans_sizes[m].append(size)
            if size > 1 and acount[agent_of[m]] == 1:
                ans_lone_agent[m] = True

        # "Claude[12, 44]; Grok[7]" — answers grouped under their agent
        by_agent = _dd(list)
        for m in mem:
            by_agent[agent_of[m]].append(str(_lab(m)))
        answers_str = "; ".join(
            f"{a}[{', '.join(v)}]" for a, v in sorted(by_agent.items())
        )
        lone = [str(_lab(m)) for m in mem if size > 1 and acount[agent_of[m]] == 1]

        node_rows.append({
            # V29: user-facing label; cube_id retained so nothing an old note
            # or figure refers to becomes unrecoverable.
            "node": (label_map or {}).get(str(node_id), str(node_id)),
            "cube_id": str(node_id),
            "size": size,
            "status": "SOLITARY" if size == 1 else "CLUSTERED",
            "n_agents": len(acount),
            "agents": ", ".join(f"{a}:{c}" for a, c in sorted(acount.items())),
            "questions": ", ".join(
                f"{q}:{c}" for q, c in sorted(
                    Counter(subset.iloc[m]["question_id"] for m in mem).items()
                )
            ) if has_q else "",
            "answer_numbers": answers_str,
            "lone_agent_answers": ", ".join(lone),
        })

    node_table = pd.DataFrame(node_rows).sort_values(
        ["size", "cube_id"], ascending=[False, True]
    ).reset_index(drop=True)

    ans_rows = []
    for i in range(n):
        nodes = ans_nodes.get(i, [])
        sizes = ans_sizes.get(i, [])
        if not nodes:
            status = "UNPLACED"
        elif min(sizes) == 1:
            status = "SOLITARY"
        ...
        row = {
            ...
            "n_nodes": len(nodes),
            "nodes": ", ".join(nodes),
            "cube_ids": ", ".join(ans_cubes.get(i, [])),
            "smallest_node": min(sizes) if sizes else 0,
            "largest_node": max(sizes) if sizes else 0,
            "lone_of_agent": bool(ans_lone_agent.get(i, False)),
            "source_row": valid_indices[i],
        }
        for c in ("run", "agent", "question_id", "temperature", "depth"):
            if c in subset.columns:
                row[c] = subset.iloc[i][c]
        ans_rows.append(row)

    lead = [c for c in ("answer", "run", "agent", "question_id", "temperature",
                        "depth", "status", "n_nodes", "nodes", "cube_ids",
                        "smallest_node",
                        "largest_node", "lone_of_agent", "source_row")
            if c in (ans_rows[0] if ans_rows else {})]
    answer_table = pd.DataFrame(ans_rows)
    if lead:
        answer_table = answer_table[lead]

    summary = {
        "answers_in": n,
        "placed": int((answer_table["status"] != "UNPLACED").sum()) if n else 0,
        "unplaced": int((answer_table["status"] == "UNPLACED").sum()) if n else 0,
        "solitary": int((answer_table["status"] == "SOLITARY").sum()) if n else 0,
        "clustered": int((answer_table["status"] == "CLUSTERED").sum()) if n else 0,
        "lone_agent": int(answer_table["lone_of_agent"].sum()) if n else 0,
        "solitary_nodes": int((node_table["size"] == 1).sum()) if len(node_table) else 0,
        "id_col": id_col,
    }
    return node_table, answer_table, summary


# =============================================================================
# V29 — Node identity, run provenance, and the node explainer
# =============================================================================
# Three problems this solves, all of which cost real debugging time in V28.
#
# (1) NODE NAMES COLLIDE ACROSS RUNS. `cube30_cluster0` on the Consciousness
#     TF-IDF graph and `cube30_cluster0` on the Consciousness IEAP graph are
#     unrelated nodes sharing a name. A saved node reference is ambiguous
#     unless the lens is recorded separately, and in practice it wasn't.
#
# (2) THE NAME ENCODES GRID POSITION, WHICH IS NOT MEANINGFUL. The cube index
#     is the cover cell a cluster fell into. How many cells a response lands
#     in correlates with nothing about the response (all |r| < 0.12 against
#     centroid distance, IEP proportions, word count, TTR, abstraction) — it
#     is a function of where the response sits relative to the grid. Putting
#     it in the user-facing name invites over-reading. Worse, cube indices are
#     sparse: a 10-cube 2D cover has 100 cells, a typical graph fills ~60 of
#     them, so the numbers have gaps and "node 78" is not the 78th node.
#
# (3) `node["size"]` IS NOT MEMBERSHIP. It is kepler's drawing radius. On a
#     representative graph the two disagreed on 61 of 61 nodes (size 4 for a
#     node holding 41 responses). Two different numbers, both called size.
#
# The V29 label is  {QUESTION}-{FEAT}-{LENS}-N{index}  e.g. CONSC-IEAP-PCA2-N01
#
#   QUESTION  from question_id, or ALL5 when the graph pools questions
#   FEAT      the feature type the vectors were built from
#   LENS      the projection Mapper used
#   N index   rank by TRUE membership, descending, ties broken by cube index
#             ascending. N01 is always the largest node. Deterministic, so it
#             reproduces exactly on rerun with identical parameters.
#
# FEAT and LENS are separate slots on purpose. They are separate controls in
# this app, and collapsing them re-creates the collision the scheme exists to
# prevent: IEP-features-under-PCA2 and IEP-features-under-AFF%-lens would
# otherwise receive identical labels.
#
# The original id is retained as `cube_id` in every export. Nothing becomes
# unrecoverable, and the cover cell is still wanted for debugging cover
# behaviour.
# =============================================================================
...
V29_QUESTION_CODES = {
    ...
    "LIARS": "LIARS",
    "RURAL": "RURAL",
    "RURAL_HEALTHCARE": "RURAL",
}

# Keys are matched against the actual Feature Type radio strings in this app,
# not against invented short names. An unmatched value is reported loudly by
# v29_feature_code() rather than silently truncated, because a plausible-looking
# wrong label is the failure mode this whole scheme exists to eliminate.
V29_FEATURE_CODES = [
    ("pre-scored", "IEAP"),
    ("live score", "IEAPV3"),
    ("SBERT", "SBERT"),
    ("Embeddings (pre-computed)", "EMB"),
]

V29_LENS_CODES = [
    ("PCA (2D)", "PCA2"),
    ("PCA1 (1D)", "PCA1"),
    ("Word TF-IDF (2D)", "TFIDF2"),
    ("Word TF-IDF (1D)", "TFIDF1"),
    ("Embedding (2D)", "EMB2"),
    ("AFF% Lens", "AFF"),
    ("INT% Lens", "INT"),
    ("ACT% Lens", "ACT"),
...
def _v29_slug(text, maxlen=8):
    """Fallback code for a control value we don't have an explicit entry for."""
    s = re.sub(r"[^A-Za-z0-9]+", "", str(text)).upper()
    return s[:maxlen] if s else "NA"


def v29_feature_code(feature_selection):
    """Map the Feature Type radio string to a short code.

    Returns (code, matched). matched is False when we fell back to a slug,
    which the UI surfaces as a warning rather than hiding.
    """
    for needle, code in V29_FEATURE_CODES:
        if needle.lower() in str(feature_selection).lower():
            return code, True
    return _v29_slug(feature_selection), False


def v29_lens_code(projection_type):
    """Map the Projection / Lens selectbox string to a short code."""
    for needle, code in V29_LENS_CODES:
        if needle.lower() == str(projection_type).lower():
            return code, True
    for needle, code in V29_LENS_CODES:
        if needle.lower() in str(projection_type).lower():
            return code, True
    return _v29_slug(projection_type), False


def v29_question_code(df, valid_indices=None):
    """QUESTION slot: the single question if the graph holds one, else ALLn."""
    if "question_id" not in getattr(df, "columns", []):
        return "NOQ"
    sub = df.iloc[valid_indices] if valid_indices is not None else df
    qs = sorted(str(q) for q in sub["question_id"].dropna().unique())
    if len(qs) == 1:
        return V29_QUESTION_CODES.get(qs[0].upper(), _v29_slug(qs[0]))
    return f"ALL{len(qs)}"


def v29_cube_index(node_id):
    m = re.match(r"cube(\d+)", str(node_id))
    return int(m.group(1)) if m else 0


def v29_relabel_nodes(node_members, question_code, feat_code, lens_code):
    """Build {original_node_id: new_label}.

    node_members : dict {node_id: [member row indices]}

    Numbered by TRUE membership descending, ties by cube index ascending.
    Padded to three digits so pooled graphs (which can exceed 99 nodes) keep
    sorting correctly; two digits would break exactly where the ALLn slot is
    most used.
    """
    ordered = sorted(
        node_members.items(),
        key=lambda kv: (-len(kv[1]), v29_cube_index(kv[0])),
    )
    prefix = f"{question_code}-{feat_code}-{lens_code}"
    return {nid: f"{prefix}-N{i:03d}" for i, (nid, _) in enumerate(ordered, start=1)}


def v29_cover_params(n_cubes, overlap, eps, min_samples, projection_type,
                     feature_selection, preset=None):
    """The run parameters, as a dict to be written into every export.

    These were previously recorded nowhere in the exported file and had to be
    reconstructed from memory. Arguably the most important part of V29.
    """
    return {
        "n_cubes": n_cubes,
        ...
        "feature_preset": preset if preset is not None else "",
    }


def v29_stamp(df, params):
    """Append the cover parameters as columns to an export dataframe."""
    out = df.copy()
...
def v29_export_stem(question_code, feat_code, lens_code):
    """Per-run filename stem. V28 wrote v28_node_membership.csv every time, so
    repeated exports overwrote each other in the downloads folder."""
    return f"{question_code}_{feat_code}_{lens_code}"


# =============================================================================
# V37 — RUN-LEVEL METRICS LOG
# =============================================================================
# One row per graph build, appended to a persistent CSV. Graph-level only:
# per-node detail belongs in the V28/V29 membership export. Keeping this file
# to one row per run is what makes a parameter sweep a pasteable table.
#
# Definitions are pinned here so the numbers are unambiguous:
#   * Per-node purity is the largest single-label share of that node's members.
#   * *_weighted averages per-node purity across nodes, weighted by node size.
#   * *_unweighted averages per-node purity across nodes, each node equal.
#   * Purity is computed over node MEMBERSHIPS. A response appearing in
#     several nodes is counted once per node. Responses dropped by DBSCAN as
#     noise never enter the calculation.
#   * responses_unassigned = input rows - unique_samples.
#   * nodes_Nagent counts nodes containing exactly N distinct architectures;
#     the four columns sum to `nodes`, a built-in consistency check.
#   * Isolated nodes count as connected components, so a graph with a main
#     body and two isolated nodes has beta_0 = 3.

RUNLOG_COLUMNS = [
    # log meta
    "run_id", "tool_version", "log_schema",
    # run identity
    "timestamp", "question_id", "feature_set", "source_csv",
    # parameters
    "n_cubes", "perc_overlap", "dbscan_eps", "dbscan_min_samples",
    "projection", "feature_scaler", "nerve_min_intersection",
    # results: graph structure
    "nodes", "edges", "beta_0", "largest_component_nodes",
    "total_samples", "unique_samples", "responses_unassigned",
    # results: purity
    "agent_purity_weighted", "agent_purity_unweighted",
    "question_purity_weighted", "question_purity_unweighted",
    # results: node composition
    "nodes_1agent", "nodes_2agent", "nodes_3agent", "nodes_4agent",
    # results: permutation validation (blank unless the test is enabled)
    "perm_n", "perm_null_purity_mean", "perm_null_purity_sd",
    "perm_null_purity_max", "perm_purity_z", "perm_purity_p",
    "perm_null_4agent_mean", "perm_4agent_p",
]


def _runlog_purity(graph, subset, label_col):
    """Weighted and unweighted mean per-node purity on `label_col`.

    Returns (weighted, unweighted). Nodes whose members carry no usable label
    are skipped rather than counted as pure, which would inflate both figures.
    """
    if label_col not in subset.columns:
        return "", ""
    num_w = den_w = 0.0
    fracs = []
    for _idx in graph.get("nodes", {}).values():
        vals = subset.iloc[list(_idx)][label_col].dropna()
        n = len(vals)
        if n == 0:
            continue
        frac = vals.value_counts().iloc[0] / n
        fracs.append(frac)
        num_w += frac * n
        den_w += n
    if not fracs:
        return "", ""
    return (round(num_w / den_w, 4), round(float(np.mean(fracs)), 4))


def permutation_test(graph, subset, n_perm=2000, seed=20260809,
                     label_col="agent"):
    """Label-permutation null for agent-purity, on THIS graph.

    Agent-purity is a descriptive property of a graph: it has no sampling
    distribution, so significance cannot come from an ANOVA. Holding the
    graph fixed and permuting only the labels supplies the null the
    statistic lacks. Node membership, node sizes, the cover and the
    clusterer are identical in every permutation, so the null reflects the
    floor the node-size distribution imposes on purity — a one-member node
    scores 1.0 by construction — rather than a floor of zero.

    Two statistics are tested: weighted purity, and the count of nodes
    containing all labels present in the data.

    Returns a dict of the perm_* run-log fields, or {} if not computable.
    """
    if label_col not in subset.columns:
        return {}
    members = [np.asarray(list(v), dtype=int) for v in graph.get("nodes", {}).values()]
    if not members:
        return {}
    codes, uniq = pd.factorize(subset[label_col])
    labels = np.asarray(codes)
    if labels.size == 0:
        raise ValueError("no rows in subset")
    if len(uniq) < 2:
        raise ValueError(f"only one distinct {label_col} value; nothing to permute")
    k = len(uniq)
    # Unlabelled rows factorize to -1. Drop them from every node rather than
    # abandoning the test, which is what the purity calculation already does.
    # Both the membership indices and the label vector must be remapped onto
    # the kept rows: filtering members alone leaves -1 in the array that gets
    # shuffled, and a permutation can then place it back on a kept index.
    if (labels < 0).any():
        keep = np.flatnonzero(labels >= 0)
        pos = np.full(labels.size, -1, dtype=int)
        pos[keep] = np.arange(keep.size)
        members = [pos[idx[labels[idx] >= 0]] for idx in members]
        members = [idx for idx in members if idx.size]
        labels = labels[keep]
        if not members:
            raise ValueError("all rows unlabelled after dropping NaN")
    ...
    def _purity(lab):
        ...
        for idx in members:
            v = lab[idx]
            if v.size:
                num += np.bincount(v, minlength=k).max()
                den += v.size
        return num / den if den else np.nan

    def _kagent(lab):
        return sum(1 for idx in members if np.unique(lab[idx]).size == k)

    obs_p, obs_k = _purity(labels), _kagent(labels)
    rng = np.random.default_rng(seed)
    perm = labels.copy()
    null_p = np.empty(int(n_perm))
    null_k = np.empty(int(n_perm), dtype=int)
    for i in range(int(n_perm)):
        rng.shuffle(perm)
        null_p[i] = _purity(perm)
        null_k[i] = _kagent(perm)

    mu, sd = float(null_p.mean()), float(null_p.std(ddof=1))
    return {
        "perm_n": int(n_perm),
        "perm_null_purity_mean": round(mu, 4),
        "perm_null_purity_sd": round(sd, 4),
        "perm_null_purity_max": round(float(null_p.max()), 4),
        "perm_purity_z": round((obs_p - mu) / sd, 1) if sd else "",
        # one-sided, bounded below by the resolution of the test
        "perm_purity_p": round((int((null_p >= obs_p).sum()) + 1) / (n_perm + 1), 6),
        "perm_null_4agent_mean": round(float(null_k.mean()), 1),
        "perm_4agent_p": round((int((null_k <= obs_k).sum()) + 1) / (n_perm + 1), 6),
    }


def build_runlog_row(graph, analysis, subset, n_input_rows, *,
                     question_id, feature_set, source_csv,
                     n_cubes, overlap, eps, min_samples,
                     projection_type, scaler_name, nerve_min_intersection=1,
                     perm_fields=None):
    """Assemble one run-log row. Pure function — no I/O, no Streamlit."""
    import hashlib
    from datetime import datetime

    nodes = graph.get("nodes", {})
    total_samples = sum(len(v) for v in nodes.values())
    unique_samples = len({i for v in nodes.values() for i in v})

    # Node composition by distinct-architecture count.
    comp = {1: 0, 2: 0, 3: 0, 4: 0}
    n_other = 0
    if "agent" in subset.columns:
        for _idx in nodes.values():
            k = subset.iloc[list(_idx)]["agent"].nunique()
            if k in comp:
                comp[k] += 1
            else:
                n_other += 1

    ap_w, ap_u = _runlog_purity(graph, subset, "agent")
    qcol = ("question_id" if "question_id" in subset.columns
            else "question_label" if "question_label" in subset.columns else None)
    qp_w, qp_u = _runlog_purity(graph, subset, qcol) if qcol else ("", "")

    ts = datetime.now()
    param_sig = "|".join(str(x) for x in [
        ts.isoformat(), question_id, feature_set, source_csv, n_cubes, overlap,
        eps, min_samples, projection_type, scaler_name, nerve_min_intersection,
    ])
    run_id = hashlib.sha1(param_sig.encode("utf-8")).hexdigest()[:10]

    row = {
        "run_id": run_id,
        "tool_version": TOOL_VERSION,
        "log_schema": RUNLOG_SCHEMA,
        ...
        "nerve_min_intersection": nerve_min_intersection,
        "nodes": len(nodes),
        "edges": analysis.get("n_edges", ""),
        "beta_0": analysis.get("n_components", ""),
        "largest_component_nodes": analysis.get("largest_component", ""),
        "total_samples": total_samples,
        "unique_samples": unique_samples,
        "responses_unassigned": max(0, int(n_input_rows) - unique_samples),
        "agent_purity_weighted": ap_w,
        "agent_purity_unweighted": ap_u,
        "question_purity_weighted": qp_w,
        "question_purity_unweighted": qp_u,
        "nodes_1agent": comp[1],
        "nodes_2agent": comp[2],
        "nodes_3agent": comp[3],
        "nodes_4agent": comp[4],
    }
    for _c in RUNLOG_COLUMNS:
        if _c.startswith("perm_"):
            row[_c] = ""
    if perm_fields:
        row.update(perm_fields)
    if n_other:
        # Only possible if a run somehow carries >4 architectures. Surface it
        # rather than let the four columns silently stop summing to `nodes`.
        row["feature_set"] = f"{feature_set} [!{n_other} nodes >4 agents]"
    return row


def append_run_log(row, path=None):
    """Append one row. Creates the file with a header if absent, otherwise
    appends without rewriting the header. Never overwrites.

    Returns (ok: bool, message: str). Failure is reported, never raised: a
    logging problem must not lose the user's graph.
    """
    path = path or RUNLOG_PATH
    try:
        df = pd.DataFrame([row], columns=RUNLOG_COLUMNS)
        exists = os.path.exists(path) and os.path.getsize(path) > 0
        if not exists:
            df.to_csv(path, mode="a", header=True, index=False)
            return True, path

        # A schema bump adds columns. Appending a wider row to a narrower
        # file yields a CSV no parser can read ("expected 29 fields, saw
        # 37") — the schema integer records the change but cannot prevent
        # it. So widen the file first: read the old rows, reindex them onto
        # the current columns (missing ones blank), and rewrite once, after
        # taking a timestamped backup. Data is preserved; nothing is lost.
        with open(path, "r", encoding="utf-8-sig") as fh:
            header = fh.readline().strip()
        old_cols = [c.strip() for c in header.split(",")] if header else []
        if old_cols and old_cols != RUNLOG_COLUMNS:
            import shutil
            from datetime import datetime as _dt
            bak = f"{path}.{_dt.now().strftime('%Y%m%d_%H%M%S')}.bak"
            shutil.copy(path, bak)
            old = pd.read_csv(path)
            merged = pd.concat(
                [old.reindex(columns=RUNLOG_COLUMNS), df],
                ignore_index=True,
            )
            merged.to_csv(path, index=False)
            return True, f"{path} (schema migrated; backup {os.path.basename(bak)})"
...
def log_mapper_run(graph, analysis, subset, n_input_rows, **kw):
    """Build a row, append it, and stash it in session_state for display.

    Wrapped so a logging failure can never take down a completed graph.
    """
    perm_fields = None
    _perm_on = bool(st.session_state.get("v37_run_perm", False))
    if _perm_on:
        _n = int(st.session_state.get("v37_perm_n", 2000))
        try:
            with st.spinner(f"Permutation test ({_n:,} relabellings)…"):
                perm_fields = permutation_test(graph, subset, n_perm=_n)
            if not perm_fields:
                st.warning(
                    "Permutation test was ON but returned no result. "
                    f"subset rows={len(subset)}, "
                    f"has '{'agent'}' column={'agent' in subset.columns}, "
                    f"graph nodes={len(graph.get('nodes', {}))}."
                )
        except Exception as e:  # noqa: BLE001
            st.warning(f"⚠️ Permutation test failed: {e}. Graph is unaffected.")
    else:
        st.caption(
            "Permutation test OFF — perm_* columns will be blank. "
            "Enable it in the sidebar under Run Log, then rebuild."
        )
    try:
        row = build_runlog_row(graph, analysis, subset, n_input_rows,
                               perm_fields=perm_fields, **kw)
    except Exception as e:  # noqa: BLE001
        st.caption(f"⚠️ Run log skipped (row build failed: {e})")
        return None
    ok, msg = append_run_log(row)
    hist = st.session_state.setdefault("_v37_runlog", [])
    hist.append(row)
    if ok:
        st.caption(
            f"📝 Run logged → `{msg}` · run_id `{row['run_id']}` · "
            f"{row['nodes']} nodes · β₀={row['beta_0']} · "
            f"agent-purity {row['agent_purity_weighted']} (weighted) · "
            f"{row['responses_unassigned']} responses unassigned"
        )
    else:
        st.caption(f"⚠️ Run log could not be written ({msg}). Row kept in session.")
    if row.get("perm_purity_z") != "" and row.get("perm_purity_z") is not None:
        st.caption(
            f"🎲 Permutation null ({row['perm_n']:,}×): purity "
            f"{row['agent_purity_weighted']} vs null "
            f"{row['perm_null_purity_mean']} ± {row['perm_null_purity_sd']} "
            f"(max {row['perm_null_purity_max']}) · z = {row['perm_purity_z']} · "
            f"p ≤ {row['perm_purity_p']} · four-architecture nodes "
            f"{row['nodes_4agent']} observed vs {row['perm_null_4agent_mean']} "
            f"expected under permutation"
        )
    return row


# --- The node explainer ------------------------------------------------------
# Position and tightness, per Section 2.4 of the manuscript. Reported side by
# side and NOT interchangeable: a node whose members share an unusual mean is
# doing something different from a node whose members agree unusually closely.
#
# Two guards, both of which exist because their absence produced wrong readings:
#
#   DEGENERATE CHANNELS. If every member of a node has the identical value on
#   a channel, the SD ratio is 0 and the node looks maximally converged when
#   in fact the channel is simply absent. On a real graph, 13 of 35 mixed nodes
#   had every member at Q_t = 0; the median ratio read 0.675 with them included
#   and 1.250 with them excluded — converged versus looser-than-baseline, an
#   inverted conclusion. Degenerate node-channel pairs are flagged and counted.
#
#   SINGLE-MEMBER LEVERAGE. A leave-one-out pass reports whether dropping one
#   member changes the verdict, so a result carried by one response is visible
#   as such rather than reported as the node's leading finding.

def v29_position_tightness(df, valid_indices, members, channels,
                           n_draws=2000, seed=0):
    """Position and tightness for one node against a matched baseline.

    df, valid_indices : the scored frame and the rows that entered Mapper
    members           : member indices into the valid_indices subset
    channels          : list of column names to evaluate

    Baseline is every response NOT in this node, per Section 2.4.

    Returns a DataFrame, one row per channel:
        channel, n, node_mean, node_sd, base_mean, base_sd,
        position   (node_mean - base_mean) / base_sd
        tightness  node_sd / base_sd
        p_tight    fraction of size-matched random draws at least as tight
        degenerate True when the node has zero variance on this channel
        loo_tightness_max  worst-case tightness under leave-one-out
    """
    rng = np.random.default_rng(seed)
    subset = df.iloc[valid_indices].reset_index(drop=True)
    mem = [m for m in members if 0 <= m < len(subset)]
    rows = []
    if not mem:
        return pd.DataFrame(rows)

    other = [i for i in range(len(subset)) if i not in set(mem)]
    k = len(mem)

    for ch in channels:
        if ch not in subset.columns:
            continue
        vals = pd.to_numeric(subset.iloc[mem][ch], errors="coerce").dropna().values
        base = pd.to_numeric(subset.iloc[other][ch], errors="coerce").dropna().values
        if len(vals) == 0 or len(base) < 2:
            continue

        node_sd = float(vals.std(ddof=0))
        base_sd = float(base.std(ddof=0))
        degenerate = bool(node_sd == 0.0)
        tightness = (node_sd / base_sd) if base_sd > 0 else np.nan
        position = ((vals.mean() - base.mean()) / base_sd) if base_sd > 0 else np.nan

        # size-matched shuffle control
        p_tight = np.nan
        if base_sd > 0 and k >= 2 and len(base) >= k:
            draws = rng.choice(len(base), size=(n_draws, k), replace=True)
            null_sd = base[draws].std(axis=1, ddof=0)
            p_tight = float(np.mean(null_sd <= node_sd))

        # leave-one-out: worst (largest) tightness after dropping any one member
        loo = np.nan
        if k >= 3 and base_sd > 0:
            loo = float(max(
                np.delete(vals, j).std(ddof=0) / base_sd for j in range(len(vals))
            ))

        rows.append({
            "channel": ch,
            "n": k,
            "node_mean": float(vals.mean()),
            "node_sd": node_sd,
            ...
            "loo_tightness_max": loo,
        })

    out = pd.DataFrame(rows)
    if len(out):
        out = out.sort_values("tightness", na_position="last").reset_index(drop=True)
    return out


V29_VOICE_CHANNELS = ["S_t", "A_t", "Q_t", "D_t", "R_t"]
V29_QUERY_SUBTYPES = ["Q_total", "Q_clarifying", "Q_invitational",
                      "Q_rhetorical", "Q_other"]


def v29_available_channels(df):
    """Channels present in this CSV, in a sensible reporting order.

    Query subtypes are included when present. Q_t lumps every interrogative
    together, so a rhetorical 'What matters most to you?' scores identically to
    a genuine 'How long have you been saving?'. The subtypes separate them and
    are what a claim about a system asking the *user* something must rest on.
    """
    cols = list(getattr(df, "columns", []))
    out = [c for c in V29_VOICE_CHANNELS if c in cols]
    out += [c for c in V29_QUERY_SUBTYPES if c in cols]
    out += [c for c in ("int_pct", "aff_pct", "act_pct", "total_words",
                        "ttr", "flesch_kincaid", "vader_compound") if c in cols]
    return out
...
# V39 — SETTLED-SVG EXPORT (adds to V38)
#   * A "Export settled SVG" button on the graph serializes the LIVE settled
#     layout to a vector SVG, auto-fitting ALL nodes into a fixed frame. You
#     never have to zoom or pan to get the whole graph, and because every
#     question fits the same frame the exports are comparable by construction.
#   * Sidebar toggle "Node scaling for export": Fit to frame (each graph fills
#     the frame) vs Absolute (radius = k*sqrt(membership), identical across
#     graphs for true cross-question size comparison).
#   * Config passed as window.V39_EXPORT, live-refreshed like the label config.
#   * Pure addition: no change to scoring, layout, coloring, or the mapper.
...
(function() {
  ...
  function render(paths) {
    ...
    function updatePies() {
      ...
      // ---- V30: cube-number labels, driven by window.V29_LABELS ----
      var LBL = window.V29_LABELS || {mode:'none', only:[], hide:[]};
      if (LBL.mode && LBL.mode !== 'none') {
        var labelLayer = nodeLayer.querySelector('g.v30-label-layer');
        if (!labelLayer) {
          labelLayer = document.createElementNS('http://www.w3.org/2000/svg', 'g');
          labelLayer.setAttribute('class', 'v30-label-layer');
          nodeLayer.appendChild(labelLayer);
        }
        paths.each(function(d) {
          if (!d) return;
          var cx = d.x, cy = d.y;
          if (typeof cx !== 'number' || typeof cy !== 'number' || isNaN(cx) || isNaN(cy)) return;
          // cube number: strip 'cube' and '_clusterN' from the id/name
          var num;
          if (d.v30_num !== undefined && d.v30_num !== '') { num = String(d.v30_num); }
          else { var raw=(d.name||d.id||''); var m=/cube(\d+)/.exec(raw); num=m?m[1]:raw; }
          // selection logic
          if (LBL.mode === 'selected' && LBL.only.indexOf(num) === -1) { removeLabel(num); return; }
          if (LBL.hide && LBL.hide.indexOf(num) !== -1) { removeLabel(num); return; }
          var baseR = nodeBaseRadius(d);
          var scale = d.v21_pie_radius_scale || 1.4;
          var r = baseR * scale;
          var key = 'v30lbl_' + num;
          var t = labelLayer.querySelector('text[data-key="' + key + '"]');
          if (!t) {
            t = document.createElementNS('http://www.w3.org/2000/svg', 'text');
            t.setAttribute('data-key', key);
            t.setAttribute('class', 'v30-cube-label');
            t.setAttribute('text-anchor', 'middle');
            t.setAttribute('dominant-baseline', 'central');
            t.setAttribute('paint-order', 'stroke');
            t.setAttribute('stroke', '#ffffff');
            t.setAttribute('stroke-width', (LBL.halo != null ? LBL.halo : Math.max(3, (LBL.size||14)*0.28)));
            t.setAttribute('fill', LBL.color || '#111111');
            t.setAttribute('font-size', (LBL.size || 11));
          ...
          // base position: centered, or below the node
          var bx = cx;
          var by = (LBL.placement === 'below') ? (cy + r + (LBL.size||11)*0.6) : cy;
          // V31: per-cube manual nudge. LBL.offsets[num] = {dx,dy} in px.
          var off = (LBL.offsets && LBL.offsets[num]) ? LBL.offsets[num] : null;
          var lx = bx + (off ? (off.dx||0) : 0);
          var ly = by + (off ? (off.dy||0) : 0);
          t.setAttribute('x', lx);
          t.setAttribute('y', ly);
          // leader line from node edge to a nudged label so the link is clear.
          // V33: require a real move (>= 12px total) so small/near-zero offsets
          // never spawn a stray line — that was the '78' doubled-glyph artifact.
          var leadKey = 'v31lead_' + num;
          var existingLead = labelLayer.querySelector('line[data-key="' + leadKey + '"]');
          var offMag = off ? (Math.abs(off.dx||0) + Math.abs(off.dy||0)) : 0;
          if (off && offMag >= Math.max(12, r)) {
            var ln = existingLead;
            if (!ln) {
              ln = document.createElementNS('http://www.w3.org/2000/svg', 'line');
              ln.setAttribute('data-key', leadKey);
              ln.setAttribute('stroke', '#888888');
              ln.setAttribute('stroke-width', '0.8');
              ln.setAttribute('pointer-events', 'none');
              labelLayer.insertBefore(ln, labelLayer.firstChild);
            }
            ln.setAttribute('x1', cx); ln.setAttribute('y1', cy);
            ln.setAttribute('x2', lx); ln.setAttribute('y2', ly);
          } else if (existingLead) {
            existingLead.parentNode.removeChild(existingLead);
          }
        });
        function removeLabel(num){
          var ex = labelLayer.querySelector('text[data-key="v30lbl_' + num + '"]');
          if (ex) ex.parentNode.removeChild(ex);
          var ln = labelLayer.querySelector('line[data-key="v31lead_' + num + '"]');
          if (ln) ln.parentNode.removeChild(ln);
        }

        // ---- V34: agent key — labeled cubes grouped by PLURALITY agent ----
        // Static box, bottom-right, colored by agent, cube numbers only. No
        // counts: the pies show distribution; this box says which cube is whose.
        if (LBL.panel) {
          var AGC = {"Claude":"#377eb8","ChatGPT":"#4daf4a","Grok":"#e41a1c","Gemini":"#984ea3"};
          var AGORDER = ["Grok","Claude","ChatGPT","Gemini"];
          var wantAll = (LBL.mode === 'all');
          var byAgent = {};  // agent -> [cube numbers]
          paths.each(function(d){
            if (!d) return;
            var raw = (d.v30_num !== undefined && d.v30_num !== '') ? String(d.v30_num)
                      : (function(){var m=/cube(\d+)/.exec(d.name||d.id||''); return m?m[1]:'';})();
            if (!raw) return;
            if (!wantAll && LBL.only.indexOf(raw) === -1) return;
            if (LBL.hide && LBL.hide.indexOf(raw) !== -1) return;
            var ag = (d.v22_panel && d.v22_panel.agents) ? d.v22_panel.agents : [];
            if (!ag.length) return;
            // V35 FIX. V34 assigned each cube to the PLURALITY agent only, so a
            // shared cube vanished from every other agent's row. Tracing run 92
            // showed "Claude 11, 12, 21, 22" and nothing for ChatGPT or Gemini,
            // when ChatGPT is in 11 and 21 and Gemini is in 11 — Claude simply
            // held the plurality in all three. The box was reporting node
            // ownership while being read as membership.
            //
            // Every agent present in a cube now gets that cube. The plurality
            // holder is still marked, so both readings are available and
            // neither is inferred.
            var best=null, bestn=-1;
            for (var k=0;k<ag.length;k++){ if (ag[k][1] > bestn){ bestn=ag[k][1]; best=ag[k][0]; } }
            for (var k=0;k<ag.length;k++){
              var a2 = ag[k][0];
              if (!a2) continue;
              (byAgent[a2] = byAgent[a2] || []).push({
                num: parseInt(raw),
                count: ag[k][1],
                lead: (a2 === best)
              });
          ...
          var host = document.getElementById('v33-members');
          if (!host) {
            host = document.createElement('div');
            host.id = 'v33-members';
            document.body.appendChild(host);
            var css = document.createElement('style');
            css.textContent = '#v33-members{position:fixed; bottom:20px; right:20px;'
              + 'background:#fff; border:1px solid #c8c8c8; border-radius:6px; padding:10px 14px;'
              + 'font-family:-apple-system,sans-serif; font-size:13px; color:#1a1a1a;'
              + 'box-shadow:0 2px 8px rgba(0,0,0,0.10); z-index:9998; max-width:260px;}'
              + '#v33-members h4{margin:0 0 8px 0;font-size:11px;color:#666;font-weight:700;'
              + 'text-transform:uppercase;letter-spacing:0.6px;}'
              + '#v33-members .v34-row{display:flex;align-items:baseline;padding:3px 0;}'
              + '#v33-members .v34-dot{display:inline-block;width:11px;height:11px;border-radius:50%;'
              + 'margin-right:8px;flex:0 0 auto;position:relative;top:1px;}'
              + '#v33-members .v34-name{font-weight:600;flex:0 0 62px;}'
              + '#v33-members .v34-cubes{color:#333;font-family:monospace;font-size:12px;}'
              + '#v33-members .v34-note{color:#777;font-size:10px;margin:-2px 0 6px 0;}';
            document.head.appendChild(css);
          }
          var html = '<h4>Labeled cubes by agent</h4>'
                   + '<div class="v34-note">a cube appears under every agent present in it; <b>bold</b> = plurality holder</div>';
          var any=false;
          for (var oi=0; oi<AGORDER.length; oi++){
            var a = AGORDER[oi];
            if (!byAgent[a] || !byAgent[a].length) continue;
            any=true;
            byAgent[a].sort(function(x,y){return x.num-y.num;});
            var parts=[];
            for (var ci=0; ci<byAgent[a].length; ci++){
              var e = byAgent[a][ci];
              // bold = this agent holds the plurality of that cube.
              parts.push(e.lead ? '<b>'+e.num+'</b>' : String(e.num));
            }
            html += '<div class="v34-row"><span class="v34-dot" style="background:'+(AGC[a]||'#888')+'"></span>'
                  + '<span class="v34-name" style="color:'+(AGC[a]||'#333')+'">'+a+'</span>'
                  + '<span class="v34-cubes">'+parts.join(', ')+'</span></div>';
          }
          if (!any) html += '<div style="color:#888;font-style:italic;">none</div>';
          host.innerHTML = html;
        } else {
          var oldp = document.getElementById('v33-members');
          if (oldp) oldp.parentNode.removeChild(oldp);
        }
      } else {
        // mode none: clear any existing labels
        var oldLayer = nodeLayer.querySelector('g.v30-label-layer');
        if (oldLayer) { while(oldLayer.firstChild) oldLayer.removeChild(oldLayer.firstChild); }
      }
    }
    ...
    // ================= V40: NODE POSITION DUMP =================
    // V39 built the entire figure inside the browser by scraping the live DOM:
    // membership, wedge colors, wedge fractions and radii were all read back
    // out of elements that the force simulation and three injection layers
    // (V21 fills, V22 pies, V30 labels) had been mutating for the last minute.
    // Every one of those attributes already exists in Python, where it was
    // computed. The only fact the browser owns is where the settled simulation
    // put each node. So x and y are now the only thing it returns.
    //
    // This also removes two V39 failures by construction:
    //   * the degenerate full-circle arc (a single wedge at frac=1.0 has
    //     coincident arc endpoints, which SVG drops, so every pure node
    //     vanished and only mixed nodes drew);
    //   * per-graph fit scaling, which gave each question its own scale
    //     factor and multiplied radii and stroke widths by it.
    (function addDumpButton(){
      if (document.getElementById('v40-dump-btn')) return;
      var btn = document.createElement('button');
      btn.id = 'v40-dump-btn';
      btn.textContent = '\u{1F4CD} Copy node positions';
      btn.style.cssText = 'position:fixed;top:14px;left:14px;z-index:99999;'
        + 'padding:8px 14px;font-family:-apple-system,sans-serif;font-size:13px;'
        + 'font-weight:600;background:#fff;border:1px solid #888;border-radius:6px;'
      ...
      btn.onclick = function(){ if(window.v40Dump) window.v40Dump(); };
      document.body.appendChild(btn);

      // V49 — an explicit zoom bar. Earlier attempts drove KeplerMapper's own
      // d3 zoom, which meant depending on its internals being reachable from
      // this scope; they were not, and the control did nothing. This sets the
      // transform attribute on the SVG container group directly through the
      // DOM. It needs no d3, no globals, and no cooperation from the viewer,
      // so there is nothing left for it to fail to reach.
      var bar = document.createElement('div');
      bar.id = 'v49-zoom-bar';
      bar.style.cssText = 'position:fixed;top:14px;left:210px;z-index:99999;'
        + 'display:flex;gap:4px;align-items:center;background:#fff;'
        + 'border:1px solid #888;border-radius:6px;padding:4px 6px;'
        + 'box-shadow:0 2px 6px rgba(0,0,0,0.15);'
        + 'font-family:-apple-system,sans-serif;';
      var mk = function(label, fn, wide){
        var b = document.createElement('button');
        b.textContent = label;
        b.style.cssText = 'padding:5px ' + (wide ? '10px' : '8px')
          + ';font-size:12px;font-weight:600;background:#f7f7f7;'
          + 'border:1px solid #bbb;border-radius:4px;cursor:pointer;';
        b.onmouseover = function(){ b.style.background = '#e8e8e8'; };
        b.onmouseout  = function(){ b.style.background = '#f7f7f7'; };
        b.onclick = fn;
        return b;
      };
      bar.appendChild(mk('\u2922 Fit', function(){ window.v49Zoom('fit'); }, true));
      [100, 80, 60, 40, 25].forEach(function(pct){
        bar.appendChild(mk(pct + '%', function(){ window.v49Zoom(pct / 100); }));
      });
      document.body.appendChild(bar);
    })();

    // Returns the SVG element and the container <g> KeplerMapper draws into,
    // located by walking the DOM rather than by name.
    window.v49Parts = function(){
      var sv = document.querySelector('#canvas svg') || document.querySelector('svg');
      if (!sv) return null;
      var root = null, kids = sv.children;
      for (var i = 0; i < kids.length; i++) {
        if (kids[i].tagName && kids[i].tagName.toLowerCase() === 'g') { root = kids[i]; break; }
      }
      return root ? {sv: sv, root: root} : null;
    };

    window.v49Zoom = function(mode){
      try {
        var P = window.v49Parts();
        if (!P) { v40Banner('No graph on screen yet.', '#b3261e'); return false; }

        // Measure the drawing at scale 1. getBBox on a transformed group
        // reports untransformed user units, but clearing first removes any
        // doubt about what is being measured.
        var prev = P.root.getAttribute('transform');
        P.root.removeAttribute('transform');
        var b = P.root.getBBox();
        if (!b || b.width < 1 || b.height < 1) {
          if (prev) P.root.setAttribute('transform', prev);
          v40Banner('Graph is still drawing \u2014 try again in a moment.', '#b3261e');
          return false;
        }
        var r = P.sv.getBoundingClientRect();
        var W = r.width  || +P.sv.getAttribute('width')  || 900;
        var H = r.height || +P.sv.getAttribute('height') || 700;

        var fitK = Math.min(W / b.width, H / b.height) * 0.92;
        var k = (mode === 'fit') ? fitK : fitK * mode;
        k = Math.max(0.02, Math.min(20, k));

        var tx = W / 2 - k * (b.x + b.width  / 2);
        var ty = H / 2 - k * (b.y + b.height / 2);
        P.root.setAttribute('transform',
          'translate(' + tx.toFixed(2) + ',' + ty.toFixed(2) + ') scale(' + k.toFixed(5) + ')');

        // If KeplerMapper's own zoom happens to be reachable, tell it where we
        // moved to, so a later pan or wheel continues from here instead of
        // snapping back. Purely optional — the transform above already stands
        // on its own.
        try {
          if (window.d3 && window.zoom && window.svg) {
            window.svg.call(window.zoom.transform,
              window.d3.zoomIdentity.translate(tx, ty).scale(k));
          }
        } catch (e) { /* the DOM transform is already applied */ }

        if (!window.__v50Quiet) {
          v40Banner((mode === 'fit' ? 'Fitted to window' : Math.round(mode * 100)
            + '% of fit') + ' \u2014 scale ' + k.toFixed(3), '#1a7f37');
        }
        setTimeout(function(){
          var el = document.getElementById('v40-banner');
          if (el) el.style.display = 'none';
        }, 2200);
        return true;
      } catch (e) {
        console.error('v49Zoom', e);
        v40Banner('Zoom failed: ' + e.message, '#b3261e');
        return false;
      }
    };

    // V50 — hold the fit until the layout actually stops.
    //
    // V49 fitted once and stopped. In the app that mostly looked right,
    // because the graph was already near-settled by the time the fit ran. In
    // a downloaded file the simulation starts from scratch, so the fit landed
    // early and the still-expanding layout immediately grew back out of the
    // frame — the figure "goes back to being live and not in the window".
    // The fit is therefore re-applied whenever the layout moves, and stops on
    // its own once the layout is stable or the reader takes over.
    (function keepFitted(){
      var prev = null, stable = 0, ticks = 0, userTookOver = false;
      window.__v50Quiet = true;   // no banner spam while the layout cools

      function bbox(){
        var sv = document.querySelector('#canvas svg');
        if (!sv) return null;
        var xs = [], ys = [];
        sv.querySelectorAll('path, circle').forEach(function(el){
          var d = el.__data__;
          if (d && typeof d.x === 'number' && !isNaN(d.x)) { xs.push(d.x); ys.push(d.y); }
        });
      ...
      // Once the reader pans, zooms or drags a node, the view is theirs.
      // Auto-fitting over the top of a deliberate action is worse than not
      // fitting at all.
      ['wheel', 'mousedown', 'touchstart'].forEach(function(ev){
        document.addEventListener(ev, function(e){
          if (e.target && e.target.closest && e.target.closest('#v49-zoom-bar')) return;
          userTookOver = true;
          window.__v50Quiet = false;
        }, true);
      });
      ...
      var iv = setInterval(function(){
        ticks++;
        try {
          if (userTookOver) { clearInterval(iv); return; }
          var cur = bbox();
          if (!cur) { if (ticks > 150) clearInterval(iv); return; }
          var moved = !prev || cur.some(function(v, i){ return Math.abs(v - prev[i]) > 1; });
          prev = cur;
          if (moved) { stable = 0; window.v49Zoom('fit'); }
          else { stable++; }
          // Three quiet checks in a row means the simulation has cooled.
          // One last fit, then leave the view alone.
          if (stable >= 3 || ticks > 150) {
            window.__v50Quiet = false;      // buttons speak again from here
            window.v49Zoom('fit');
            clearInterval(iv);
          }
      ...
      // A resized window changes what "fits" means.
      var rt = null;
      window.addEventListener('resize', function(){
        if (userTookOver) return;
        clearTimeout(rt);
        rt = setTimeout(function(){ window.v49Zoom('fit'); }, 250);
      });
    })();

    function v40Banner(msg, color){
      var b = document.getElementById('v40-banner');
      if(!b){ b=document.createElement('div'); b.id='v40-banner';
        b.style.cssText='position:fixed;top:56px;left:14px;z-index:99999;padding:10px 14px;'
          +'color:#fff;font-family:sans-serif;font-size:13px;border-radius:6px;'
          +'max-width:360px;box-shadow:0 2px 6px rgba(0,0,0,.25);';
        document.body.appendChild(b);
      }
      b.style.background = color || '#444';
      b.textContent = msg;
      b.style.display = 'block';
      return b;
    }

    window.v40Dump = function(){
     try {
      var srcSvg = document.querySelector('#canvas svg');
      if (!srcSvg) { v40Banner('Graph not ready yet.', '#b3261e'); return; }

      // One node may be represented by more than one element (kepler's
      // original path.circle, hidden at opacity 0, plus anything layered over
      // it). Keying by node NAME collapses those to one entry with the same
      // coordinates, rather than deduping on a key that may not be distinct.
      function snapshot(){
        var m = {}, unnamed = 0;
        srcSvg.querySelectorAll('path, circle').forEach(function(el){
          var d = el.__data__;
          if (!d) return;
          if (typeof d.x !== 'number' || typeof d.y !== 'number') return;
          if (isNaN(d.x) || isNaN(d.y)) return;
          var key = null;
          if (d.name !== undefined && d.name !== null && d.name !== '') key = String(d.name);
          else if (d.id !== undefined && d.id !== null && d.id !== '') key = String(d.id);
          if (key === null) { unnamed++; return; }
          m[key] = [Math.round(d.x*100)/100, Math.round(d.y*100)/100];
        });
        m.__unnamed = unnamed;
        return m;
      }
      ...
      var A = snapshot();
      var unnamedA = A.__unnamed; delete A.__unnamed;
      var nA = Object.keys(A).length;
      if (nA === 0) {
        v40Banner('No named nodes carried layout coordinates. The graph may '
          + 'still be rendering — wait a few seconds and click again.', '#b3261e');
        return;
      }

      v40Banner('Found ' + nA + ' nodes. Checking whether the layout has stopped moving\u2026', '#444');

      // Settle test by displacement, not by spread. A graph can be fully
      // spread out and still drifting; V39's spread test only caught the
      // piled-at-center case.
      setTimeout(function(){
       try {
        var B = snapshot(); delete B.__unnamed;
        var worst = 0, worstName = '';
        Object.keys(B).forEach(function(k){
          if (!A[k]) return;
          var dx = B[k][0]-A[k][0], dy = B[k][1]-A[k][1];
          var dist = Math.sqrt(dx*dx + dy*dy);
          if (dist > worst) { worst = dist; worstName = k; }
        });
        if (worst > 0.75) {
          v40Banner('\u26A0 Still settling \u2014 ' + worstName + ' moved '
            + worst.toFixed(1) + ' px in 700 ms. Wait until the graph stops '
            + 'moving, then click again.', '#b3261e');
          return;
        }

        var payload = {
          n_nodes: Object.keys(B).length,
          unnamed_elements: unnamedA,
          max_drift_px: Math.round(worst*1000)/1000,
          captured: new Date().toISOString(),
          positions: B
        };
        var txt = JSON.stringify(payload);

        var box = document.getElementById('v40-out');
        if(!box){ box=document.createElement('div'); box.id='v40-out';
          box.style.cssText='position:fixed;top:100px;left:14px;right:14px;z-index:99998;'
            +'background:#fff;border:2px solid #1a7f37;border-radius:8px;padding:12px;'
            +'font-family:-apple-system,sans-serif;';
          document.body.appendChild(box);
        }
        box.innerHTML =
            '<b>' + payload.n_nodes + ' node positions captured</b> '
          + '<span style="color:#666;font-size:12px">(max drift '
          + payload.max_drift_px.toFixed(2) + ' px \u2014 settled)</span>'
          + '<button id="v40-close" style="float:right">close</button>'
          + '<div style="font-size:12px;color:#555;margin:6px 0 4px">'
          + 'Select all and copy, then paste into <b>Figure Builder \u2192 Step 1</b> below the graph.</div>'
          + '<textarea id="v40-ta" style="width:100%;height:150px;font-family:monospace;'
          + 'font-size:11px"></textarea>';
        var ta = document.getElementById('v40-ta');
        ta.value = txt;
        box.style.display='block';
        document.getElementById('v40-close').onclick = function(){ box.style.display='none'; };
        try { ta.focus(); ta.select(); } catch(e){}
        try { if (navigator.clipboard) navigator.clipboard.writeText(txt); } catch(e){}

        v40Banner('\u2713 ' + payload.n_nodes + ' positions ready \u2014 copied to '
          + 'clipboard if the browser allowed it.', '#1a7f37');
        setTimeout(function(){
          var b=document.getElementById('v40-banner'); if(b) b.style.display='none';
        }, 5000);
       } catch(e2){ console.error('v40 dump (phase 2)', e2); }
      }, 700);
     } catch(_e){ console.error('v40 dump error', _e); }
    }
  }
...
# ── V40/V51 geometry helpers (defined here because inject_v21_node_coloring
# runs during page execution, before the Figure Builder section is reached)
V40_FRAME_W = 1400
V40_FRAME_H = 1100


def v40_component_layout(node_names, edges, seed=42, aspect=(V40_FRAME_W / V40_FRAME_H)):
    """Deterministic 2-D layout for a Mapper nerve, laid out per component.

    Mapper graphs are routinely disconnected — beta_0 is a reported statistic —
    and a whole-graph force layout lets separate components drift into each
    other, so a reader sees adjacency that is not in the nerve. Each connected
    component is therefore laid out on its own, scaled by sqrt(size) so a big
    component gets proportionally more room, then shelf-packed left to right.

    Kamada-Kawai is used where it is well conditioned (3 to 400 nodes) because
    it is deterministic without a seed; spring layout with a fixed seed is the
    fallback. Either way the same input returns the same coordinates every
    time, which a live force simulation in a browser does not.

    `aspect` is the width-to-height ratio of the frame the result will be drawn
    into. Shelf packing at a single row width produces a wide, short
    arrangement that leaves the lower half of a 1400x1100 panel empty, and
    since the shared scale is set by the panel needing the most room, wasted
    area in one panel shrinks every panel in the figure. Several row widths
    are tried and the one whose packed bounding box comes closest to the frame
    aspect is kept.
    """
    ...
    # Lay each component out once; only the packing is retried.
    shapes = []
    for comp in comps:
        sub = G.subgraph(comp)
        n = sub.number_of_nodes()
        if n == 1:
            pos = {list(comp)[0]: _np.array([0.0, 0.0])}
        elif n == 2:
            a, b = list(comp)
            pos = {a: _np.array([-0.5, 0.0]), b: _np.array([0.5, 0.0])}
        else:
            try:
                pos = _nx.kamada_kawai_layout(sub)
            except Exception:
                pos = _nx.spring_layout(sub, seed=seed, k=1.0 / (n ** 0.5))
        keys = list(sub.nodes())
        P = _np.array([pos[k] for k in keys], dtype=float)
        P = P - P.mean(axis=0)
        span = max(_np.abs(P).max(), 1e-9)
        P = P / span * (n ** 0.5)                      # size-proportional room
        shapes.append((keys, P,
                       float(P[:, 0].max() - P[:, 0].min()) + 1.6,
                       float(P[:, 1].max() - P[:, 1].min()) + 1.6))

    def pack(row_limit):
        placed, shelf_x, shelf_y, row_h = {}, 0.0, 0.0, 0.0
        for keys, P, w, h in shapes:
            if shelf_x > 0 and shelf_x + w > row_limit:
                shelf_x, shelf_y, row_h = 0.0, shelf_y - row_h - 1.6, 0.0
            cx = shelf_x + w / 2.0
            for i, k in enumerate(keys):
                placed[k] = (float(P[i, 0]) + cx, float(P[i, 1]) + shelf_y)
            shelf_x += w
            row_h = max(row_h, h)
        return placed

    base = max(len(node_names), 1) ** 0.5
    widest = max((sh[2] for sh in shapes), default=1.0)
    best, best_err = None, None
    for mult in (0.8, 1.1, 1.5, 2.0, 2.6, 3.4, 4.5):
        cand = pack(max(base * mult, widest))
        xs = [p[0] for p in cand.values()]
        ys = [p[1] for p in cand.values()]
        w = max(max(xs) - min(xs), 1e-9)
        h = max(max(ys) - min(ys), 1e-9)
        err = abs(_np.log((w / h) / aspect))            # log ratio: symmetric
        if best_err is None or err < best_err:
            best, best_err = cand, err

    placed = best if best is not None else {}
    for nm in node_names:                               # safety net
        placed.setdefault(nm, (0.0, 0.0))
    return placed
...
def inject_v21_node_coloring(
    ...
    for n in gj.get("nodes", []):
        ...
        # V30: clean cube number for labels (strip 'cube' and '_clusterN')
        _mnum = _re2.search(r"cube(\d+)", str(n.get("name", nid)))
        n["v30_num"] = _mnum.group(1) if _mnum else str(n.get("name", nid))
        ...
        if mode == "overlap_orange":
            ...
            # V37: unambiguous membership at the top level of the node.
            n["members"] = int(info["size"])
    ...
    # V52 — optional layout override, OFF by default.
    #
    # V51 wrote coordinates into every node because I believed KeplerMapper's
    # force layout settled differently on each load. It does not. d3-force is
    # deterministic: the same graph returns identical coordinates every run,
    # verified at zero difference across repeated runs on a 46-node graph.
    # Every figure from V20 onward reproduced exactly for that reason, and
    # overriding the layout changed those figures for no benefit.
    #
    # The real defects were the viewport never being fitted and the SVG
    # exporter, both fixed elsewhere. This override remains available for the
    # one case it genuinely helps — packing disconnected components so they
    # cannot overlap and imply adjacency absent from the nerve — but it is not
    # on unless asked for.
    try:
        if st.session_state.get("_v51_freeze_layout", False):
            _fz_names = [str(n.get("name", n.get("id", ""))) for n in gj.get("nodes", [])]
            _fz_edges = []
            for _s2, _t2 in (graph.get("links", {}) or {}).items():
                for _t3 in (_t2 or []):
                    _fz_edges.append([str(_s2), str(_t3)])
            _fz_pos = v40_component_layout(_fz_names, _fz_edges)
            _xs = [v[0] for v in _fz_pos.values()] or [0]
            _ys = [v[1] for v in _fz_pos.values()] or [0]
            _w = max(max(_xs) - min(_xs), 1e-9)
            _h = max(max(_ys) - min(_ys), 1e-9)
            # Scale into a canvas-sized box. The zoom bar rescales for the
            # actual window; this only needs to be sane, not exact.
            _k = min(1100.0 / _w, 750.0 / _h)
            _cx = (max(_xs) + min(_xs)) / 2.0
            _cy = (max(_ys) + min(_ys)) / 2.0
            _frozen = 0
            for _n2 in gj.get("nodes", []):
                _nm2 = str(_n2.get("name", _n2.get("id", "")))
                if _nm2 in _fz_pos:
                    _px = (_fz_pos[_nm2][0] - _cx) * _k
                    _py = (_fz_pos[_nm2][1] - _cy) * _k
                    _n2["x"] = _px
                    _n2["y"] = _py
                    _n2["fx"] = _px      # d3 honours fx/fy as pinned
                    _n2["fy"] = _py
                    _frozen += 1
            gj["_v51_layout_frozen"] = _frozen
    except Exception as _e51:
        gj["_v51_layout_error"] = str(_e51)[:200]

    # V40 — stash the panel payload for the Figure Builder.
    # Everything a figure needs except x/y is known right here, at build time.
    # The browser is asked only for x/y, so no figure attribute depends on
    # reading back out of a DOM that the simulation keeps mutating.
    try:
        _v40_edges, _v40_seen = [], set()
        for _s, _ts in (graph.get("links", {}) or {}).items():
            for _t in (_ts or []):
                _key = tuple(sorted((str(_s), str(_t))))
                if _key in _v40_seen:
                    continue
                _v40_seen.add(_key)
                _v40_edges.append([_key[0], _key[1]])
        _v40_nodes = []
        for _n in gj.get("nodes", []):
            _nm = str(_n.get("name", _n.get("id", "")))
            _mem = _n.get("members")
            if _mem is None:
                _mem = (_n.get("v22_panel") or {}).get("size")
            _wd = []
            for _w in (_n.get("v21_wedges") or []):
                if _w.get("color") and _w.get("frac"):
                    _wd.append({"color": _w["color"],
                                "frac": float(_w["frac"]),
                                "label": str(_w.get("label", ""))})
            _v40_nodes.append({
                "name": _nm,
                "cube": str(_n.get("v30_num", "")),
                "members": int(_mem) if _mem is not None else 1,
                "wedges": _wd,
            })
        st.session_state["_v40_panel_graph"] = {
            "nodes": _v40_nodes,
            "edges": _v40_edges,
            "paint_mode": mode,
            "tool_version": TOOL_VERSION,
            "question_hint": str(st.session_state.get("_v40_question_hint", "")),
        }
    except Exception:
        pass
...
def inject_v23_universal_hover(html_content, graph, df, valid_indices):
    ...
    # ── V37: guarantee `members` on EVERY node ────────────────────────────
    # kepler writes a field called `size` that is a drawing bucket, not a
    # membership count: on a representative 61-node graph `size` ranged 2-4
    # and summed to 168, while true membership ranged 3-46 and summed to 770.
    # Two different numbers, both called size. An external script reading
    # `size` as membership drew a 41-response node as if it held 4, and the
    # error was silent because both fields are plausible small integers.
    # `members` is written here so no downstream consumer has to know that.
    # Falls back to the panel payload, then to the tooltip counts, and is
    # left absent rather than guessed if neither resolves.
    import re as _re
    for n in gj.get("nodes", []):
        if "members" in n:
            continue
        _panel = n.get("v22_panel") or {}
        if "size" in _panel:
            n["members"] = int(_panel["size"])
            continue
        # Wedges carry `frac`, not counts, so they cannot give membership.
        # The tooltip does: "Claude: 7  ·  ChatGPT: 2  ·  Gemini: 4".
        _tip = n.get("v21_tooltip_html") or ""
        if _tip:
            _nums = _re.findall(r":\s*(\d+)", _tip)
            if _nums:
                n["members"] = int(sum(int(x) for x in _nums))
...
def render_plotly_agent_view(graph, df, valid_indices,
                              ...
                              title=f"SYN-IQ {TOOL_VERSION} Mapper",
                              pin_runs=None,
                              axis_label="PC1"):
    ...
    if mode == "pca1_strip" or projection_type == "PCA1 (1D)":
        ...
        if has_agent:
            ...
            for ag in sorted(set(agents)):
                ...
                fig.add_trace(go.Scatter(
                    ...
                    hovertemplate="%{text}<br>"+axis_label+"=%{x:.3f}<extra></extra>"
                ))
        else:
        ...
        # ---- V27: matched-run pin arrows ----------------------------------
        # pin_runs: list of run numbers. For each pinned run, drop a labeled
        # arrow at each agent's response X-position. Same run index across all
        # agents = a fair, matched pull (no "that's just turn 64" objection).
        # Completeness is asserted: if an agent is absent for a pinned run, it
        # is reported back to the caller rather than silently skipped.
        ...
        if pin_runs and has_agent and ('run' in subset.columns):
            try:
                ...
                pin_x, pin_y, pin_text, pin_color = [], [], [], []
                for k, r in enumerate(pin_runs):
                    lift = 0.55 if k == 0 else 0.80   # stack 2nd run higher
                    for ag in all_agents:
                        ...
                        pin_x.append(xpos)
                        pin_y.append(lift)
                        pin_text.append(f"{ag}·{r}")
                        pin_color.append(get_agent_color(ag))
                        row = subset.iloc[idx]
                        pin_report.append({
                            'run': int(r), 'agent': ag, 'pca1': round(xpos, 3),
                ...
                if pin_x:
                    # vertical stems from axis up to each pin marker
                    for px, py, pc in zip(pin_x, pin_y, pin_color):
                        fig.add_trace(go.Scatter(
                            x=[px, px], y=[0, py], mode='lines',
                            line=dict(color=pc, width=1.4),
                            showlegend=False, hoverinfo='skip'))
                    # pin markers + labels on top
                    fig.add_trace(go.Scatter(
                        x=pin_x, y=pin_y, mode='markers+text',
                        marker=dict(size=13, color=pin_color, symbol='triangle-down',
                                    line=dict(width=1, color='#222')),
                        text=pin_text, textposition='top center',
                        textfont=dict(size=10),
                        showlegend=False,
                        hovertext=pin_text, hoverinfo='text'))
            except Exception as _pin_e:
                st.warning(f"Pin overlay skipped ({type(_pin_e).__name__}: {_pin_e}). "
                           "Strip shown without pins.")
                pin_report, pin_missing = [], []
        ...
        fig.update_layout(
            title=(f"{title} · {axis_label} explains {explained_variance*100:.1f}% of variance"
                   if explained_variance is not None else title),
            xaxis_title=(f"{axis_label} ({explained_variance*100:.1f}% var)"
                         if explained_variance is not None else axis_label),
            yaxis=dict(visible=False, range=[-1, 1.15]),
...
def build_iep_features(df, feature_keys=None):
    ...
    _v23_canon = [
        ...
        # V24.1: Voice-State channels as selectable lenses/features.
        # Filtered to those actually present in the loaded CSV just below,
        # so they stay invisible on schemas without the V_t columns.
        ("S_t",             "S_t"),
        ("A_t",             "A_t"),
        ("Q_t",             "Q_t"),
        ("D_t",             "D_t"),
    ...
    # V24.1: drop any canonical entry whose column is absent from this CSV
    # (keeps V_t lenses from appearing on files that lack them).
    _present = set(df.columns)
    _v23_canon = [(k, col) for (k, col) in _v23_canon if col in _present]
    ...
    for idx, row in df.iterrows():
        try:
            ...
            # V24.1: V_t channels are now first-class selectable features,
            # handled through `active`/_v23_canon above. The old unconditional
            # append was removed to avoid double-counting S_t..R_t.
            features.append(fv)
            valid_indices.append(idx)
...
V23_TOGGLE_FEATURES = [
    ...
    # V24.1: Voice-State (V_t) channels — selectable as features/lenses.
    # Schema-gated: only surfaced when the columns exist (see lens build).
    "S_t", "A_t", "Q_t", "D_t", "R_t",
]
...
V23_FEATURE_GROUPS = [
    ...
    # V24.1: structure density (S_t) + the other four voice-state channels.
    ("Voice-State (V_t)", ["S_t", "A_t", "Q_t", "D_t", "R_t"]),
]
...
def run_mapper_analysis(data, df, valid_indices, n_cubes, overlap, eps, min_samples, projection_type):
    ...
    if projection_type == "PCA (2D)":
        ...
        projected = subset[col].values.reshape(-1, 1)
    elif projection_type == "Word TF-IDF (2D)":
        from sklearn.feature_extraction.text import TfidfVectorizer
        from sklearn.decomposition import TruncatedSVD
        subset = df.iloc[valid_indices]
        texts = subset['response_text'].fillna('').astype(str).tolist()
        # Guard: need enough non-empty text to vectorize.
        nonempty = sum(1 for t in texts if t.strip())
        if nonempty < 3:
            raise ValueError(
                "Word TF-IDF lens requires at least 3 non-empty response_text "
                f"rows in the current selection (found {nonempty}). "
                "Pick a larger cell or a different lens."
            )
        ...
        try:
            ...
            raise ValueError(
                "Word TF-IDF lens could not build a vocabulary (text may be "
                "too short, too repetitive, or too uniform). Underlying: %s" % e
            )
        if X.shape[1] < 2:
            raise ValueError(
                "Word TF-IDF lens found fewer than 2 distinctive terms "
                "(text too repetitive/uniform to project). Try a different cell."
            )
        n_comp = min(2, X.shape[1] - 1)
        svd = TruncatedSVD(n_components=max(n_comp, 1), random_state=42)
        reduced = svd.fit_transform(X)
        if reduced.shape[1] == 1:               # pad to 2D if only 1 component
            import numpy as np
            reduced = np.hstack([reduced, np.zeros((reduced.shape[0], 1))])
        projected = reduced
    elif projection_type == "Word TF-IDF (1D)":
        # V27: 1D content axis on surface vocabulary. Same TF-IDF vectorizer as
        # the 2D lens, reduced to a single component. TF-IDF is sparse and
        # high-dimensional, so we use TruncatedSVD (LSA) — NOT PCA, which would
        # mean-center/densify the term matrix. Component 1 is the content axis.
        # We pad to 2D so the Mapper cover has an axis to work with; the strip
        # renderer uses only component 1.
        from sklearn.feature_extraction.text import TfidfVectorizer
        from sklearn.decomposition import TruncatedSVD
        import numpy as np
        subset = df.iloc[valid_indices]
        ...
        if nonempty < 3:
            raise ValueError(
                "Word TF-IDF (1D) lens requires at least 3 non-empty response_text "
                f"rows in the current selection (found {nonempty}). "
                "Pick a larger cell or a different lens."
            )
        tfidf = TfidfVectorizer(max_features=2000, stop_words='english',
        ...
        try:
            ...
            raise ValueError(
                "Word TF-IDF (1D) lens could not build a vocabulary (text may be "
                "too short, too repetitive, or too uniform). Underlying: %s" % e
            )
        if X.shape[1] < 2:
            raise ValueError(
                "Word TF-IDF (1D) lens found fewer than 2 distinctive terms "
                "(text too repetitive/uniform to project). Try a different cell."
            )
        svd = TruncatedSVD(n_components=1, random_state=42)
        comp1 = svd.fit_transform(X)                 # (n, 1)
        projected = np.hstack([comp1, np.zeros((comp1.shape[0], 1))])
    elif projection_type == "Embedding (2D)":
        # V26: semantic lens. Parse the per-response sentence vectors for the
        # rows in this selection and reduce to 2D with TruncatedSVD — the same
        # reducer used by the Word TF-IDF lens, so the two are directly
        # comparable (surface vocabulary vs. meaning). If agents separate under
        # TF-IDF but blur here, the agent signature is lexical, not semantic.
        import numpy as np
        from sklearn.decomposition import TruncatedSVD
        subset = df.iloc[valid_indices]
        embs = []
        for emb_str in subset['embedding'].tolist():
            try:
                if pd.isna(emb_str) or emb_str in ('[]', ''):
                    embs.append(None); continue
                v = json.loads(emb_str) if isinstance(emb_str, str) else emb_str
                embs.append(v if isinstance(v, list) and len(v) > 0 else None)
            except (json.JSONDecodeError, TypeError):
                embs.append(None)
        good = sum(1 for e in embs if e is not None)
        if good < 3:
            raise ValueError(
                "Embedding lens requires at least 3 rows with a non-empty "
                f"'embedding' vector in the current selection (found {good}). "
                "Pick a larger cell or a different lens."
            )
        dim = len(next(e for e in embs if e is not None))
        E = np.array([e if e is not None else [0.0] * dim for e in embs])
        svd = TruncatedSVD(n_components=2, random_state=42)
        projected = svd.fit_transform(E)
...
# V26: lag-free gate for the Word TF-IDF lens. The schema flags used by the
# other conditional lenses aren't set until detect_schema() runs further down,
# AFTER the Projection dropdown is built — so those lenses only appear one rerun
# after upload. To make the TF-IDF lens appear on the SAME pass the file loads,
# sniff the header here (header-only read, pointer restored) and stash a flag the
# dropdown can read immediately. CSV only; appears if ANY uploaded CSV carries
# response_text. Non-CSV / failures fall through to the session-state gate.
_v243_tfidf_available = False
_v26_embedding_available = False
if uploaded_files:
    for _f in uploaded_files:
        try:
            if _f.name.endswith('.csv'):
                _hdr = pd.read_csv(_f, nrows=0)
                _f.seek(0)
                if 'response_text' in _hdr.columns:
                    _v243_tfidf_available = True
                if 'embedding' in _hdr.columns:
                    _v26_embedding_available = True
                if _v243_tfidf_available and _v26_embedding_available:
                    break
        except Exception:
            try:
                _f.seek(0)
...
if _v21_flags.get("response_text") or _v243_tfidf_available:
    _v21_projection_list.append("Word TF-IDF (2D)")
    # V27: 1D content axis on TF-IDF — the surface-vocabulary companion to the
    # PCA1 (1D) strip. Uses TruncatedSVD (sparse-safe) rather than PCA.
    _v21_projection_list.append("Word TF-IDF (1D)")
# V26: Embedding (2D) lens — semantic (meaning-level) counterpart to the
# Word TF-IDF (surface-vocabulary) lens. Schema-gated on an 'embedding'
# column carrying per-response sentence vectors. Projects those vectors to
# 2D via TruncatedSVD, mirroring the TF-IDF pipeline but on meaning rather
# than word identity. Intended as an honest contrast: if agents separate
# under TF-IDF but blur under embeddings, the agent signature lives in word
# choice, not semantic content.
if _v26_embedding_available:
    _v21_projection_list.append("Embedding (2D)")
...
# ── V30 — CUBE NUMBER LABELS ─────────────────────────────────────────────
# Labels ride the REAL kepler layout: they are drawn in the same updatePies()
# tick that positions each pie, using the node's own d.x/d.y. This is why they
# match the on-screen graph exactly, unlike any downstream re-render.
st.sidebar.markdown("---")
st.sidebar.markdown("### 🔢 Cube Labels")
label_mode = st.sidebar.radio(
    "Label nodes:",
    ["None", "All cubes", "Selected only"],
    index=0,
    help="Draw the cube number on each node, on the real kepler layout. "
         "'Selected only' labels just the numbers you list below."
)
label_only_raw = ""
label_hide_raw = ""
if label_mode == "Selected only":
    label_only_raw = st.sidebar.text_input(
        "Cubes to label (comma/space separated)",
        value="",
        help="e.g. 13, 21, 22, 30, 31, 37, 38, 78 — only these get labeled."
    )
if label_mode != "None":
    label_hide_raw = st.sidebar.text_input(
        "Turn OFF these cubes",
        value="",
        help="Numbers to suppress even if the mode would show them. "
             "e.g. paste a few to declutter the figure."
    )
    _lc1, _lc2 = st.sidebar.columns(2)
    with _lc1:
        label_size = st.slider("Label size", 6, 40, 14, key="v30_lbl_size")
    with _lc2:
        label_place = st.selectbox("Placement", ["center", "below"], key="v30_lbl_place")
else:
    label_size = 11
    label_place = "center"

# V33: membership panel toggle (lists the labeled cubes + agent counts,
# right side, vertically centered — mirrors the hover panel style).
show_member_panel = False
if label_mode != "None":
    # V36: defaults OFF. With the V35 fix a shared cube appears under every
    # agent present, which is correct but makes the rows near-identical
    # (11, 12, 21, 22 repeated three times) — a lot of panel for little
    # information, and unreadable once shrunk to figure scale. The node
    # labels carry the cube numbers and the pies carry the composition, so
    # the box is redundant in a publication figure. Left available for
    # on-screen work.
    show_member_panel = st.sidebar.checkbox(
        "📋 Show agent key (labeled cubes by agent)",
        value=False,
        help="Optional box, bottom-right, listing every agent present in each "
             "labeled cube (bold = plurality holder). Off by default: the rows "
             "repeat heavily on shared cubes and the text is too small to read "
             "at figure scale. The node labels and pie wedges already carry "
             "this information."
    )

def _parse_cube_list(raw):
    import re as _r
    return [t for t in _r.split(r"[,\s]+", (raw or "").strip()) if t]

# V31 — per-cube label nudge. Pick a cube, slide its label clear of overlaps.
# Offsets accumulate in session_state so several labels can be adjusted.
if "v31_offsets" not in st.session_state:
    st.session_state.v31_offsets = {}   # {cube_num: {"dx":int,"dy":int}}
if label_mode != "None":
    with st.sidebar.expander("🎯 Nudge a label (fix overlaps)", expanded=False):
        _nudge_cube = st.text_input(
            "Cube number to move", value="", key="v31_nudge_cube",
            help="Type one cube number, then use the sliders to slide its "
                 "label off the node. A thin leader line links it back."
        ).strip()
        _cur = st.session_state.v31_offsets.get(_nudge_cube, {"dx": 0, "dy": 0})
        _dx = st.slider("← left · right →", -120, 120, int(_cur.get("dx", 0)), key="v31_dx")
        _dy = st.slider("↑ up · down ↓", -120, 120, int(_cur.get("dy", 0)), key="v31_dy")
        _c1, _c2 = st.columns(2)
        with _c1:
            if st.button("Apply nudge", key="v31_apply") and _nudge_cube:
                st.session_state.v31_offsets[_nudge_cube] = {"dx": _dx, "dy": _dy}
        with _c2:
            if st.button("Clear all nudges", key="v31_clear"):
                st.session_state.v31_offsets = {}
        if _nudge_cube:
            # live preview without needing Apply
            st.session_state.v31_offsets[_nudge_cube] = {"dx": _dx, "dy": _dy}
        if st.session_state.v31_offsets:
            st.caption("Nudged: " + ", ".join(
                f"{k}({v['dx']},{v['dy']})" for k, v in st.session_state.v31_offsets.items()
                if v['dx'] or v['dy']
            ) or "none")

_v30_label_cfg = {
    "mode": {"None": "none", "All cubes": "all", "Selected only": "selected"}[label_mode],
    "only": _parse_cube_list(label_only_raw),
    "hide": _parse_cube_list(label_hide_raw),
    "size": int(label_size),
    "placement": label_place,
    "color": "#111111",
    "halo": 3,
    "offsets": {k: v for k, v in st.session_state.get("v31_offsets", {}).items() if v.get("dx") or v.get("dy")},
    "panel": bool(show_member_panel),
}

# ── V40 — SETTLED-LAYOUT CAPTURE ─────────────────────────────────────────
# The graph carries a "Copy node positions" button. It returns x/y and nothing
# else; the figure is rendered in Python by the Figure Builder at the bottom of
# the page, at one scale shared across every panel in the set.
st.sidebar.markdown("---")
st.sidebar.markdown("### 📐 Figure export")
st.sidebar.caption(
    "Let the graph settle, click **📍 Copy node positions** on the figure, "
    "then paste into **Figure Builder → Capture panel** below the graph."
)
st.session_state["_v51_freeze_layout"] = st.sidebar.checkbox(
    "Override layout with Python coordinates", value=False,
    help="Off by default. KeplerMapper's own force layout is deterministic — "
         "the same graph produces identical coordinates on every run — so it "
         "already reproduces exactly, and it is the arrangement every earlier "
         "figure used. Turn this on only to force a different arrangement, "
         "e.g. to keep disconnected components from overlapping.")
_v40_panels_n = len(st.session_state.get("_v40_panels", {}))
if _v40_panels_n:
    st.sidebar.caption(f"{_v40_panels_n} panel(s) captured this session.")

st.sidebar.markdown("---")
st.sidebar.markdown("---")
# ── V37: run-level metrics log ──────────────────────────────────────────────
st.sidebar.markdown("---")
st.sidebar.markdown(f"### 📝 Run Log ({TOOL_VERSION})")
_v37_hist = st.session_state.get("_v37_runlog", [])
if _v37_hist:
    st.sidebar.caption(
        f"{len(_v37_hist)} run(s) this session. Appended to `{RUNLOG_PATH}`."
    )
    _v37_log_df = pd.DataFrame(_v37_hist, columns=RUNLOG_COLUMNS)
    st.sidebar.download_button(
        "📥 Download run log (CSV)",
        _v37_log_df.to_csv(index=False),
        file_name=RUNLOG_PATH,
        mime="text/csv",
        key="v37_runlog_dl",
        help="One row per graph build. Paste straight into a sweep table.",
    )
    with st.sidebar.expander("Last run", expanded=False):
        _v37_last = _v37_hist[-1]
        st.write({k: _v37_last[k] for k in (
            "run_id", "question_id", "projection", "n_cubes", "perc_overlap",
            "nodes", "beta_0", "agent_purity_weighted",
            "question_purity_weighted", "responses_unassigned",
        )})
else:
    st.sidebar.caption("No runs logged yet this session.")

st.sidebar.checkbox(
    "Run permutation test on each build",
    key="v37_run_perm", value=False,
    help=("Label-permutation null for agent-purity. The graph is held fixed "
          "and only architecture labels are shuffled, so node sizes and the "
          "cover are identical under the null. Adds a few seconds per build."),
)
if st.session_state.get("v37_run_perm"):
    st.sidebar.number_input(
        "Permutations", min_value=200, max_value=20000, step=200,
        key="v37_perm_n", value=2000,
        help="2,000 resolves p to 0.0005; 10,000 resolves to 0.0001.",
    )

st.sidebar.markdown("### ⚡ Rerun Mode")
...
with tab_main:
        ...
        if st.button("🚀 Run Batch Analysis", type="primary"):
            ...
            for idx, f in enumerate(uploaded_files):
                ...
                try:
                    ...
                    # V37: run-level metrics log (one row per file processed)
                    log_mapper_run(
                        _graph, _analysis,
                        _df.iloc[_valid].reset_index(drop=True), len(_df),
                        question_id=v29_question_code(_df, _valid),
                        feature_set=use_iep,
                        source_csv=f.name,
                        n_cubes=n_cubes, overlap=overlap, eps=eps,
                    ...
                    _title = f"SYN-IQ {TOOL_VERSION} · {fname_safe} · {projection_type}"
    ...
    if df is not None:
        ...
        if 'temperature' in df.columns:
            st.sidebar.markdown("---")
            st.sidebar.markdown("### 🎯 Baseline Filter")
            ...
            if _v23_native_only:
                _pre_n = len(df)
                # Normalize for matching: strip whitespace + uppercase, so
                # "native", " NATIVE", "Native" all match. Exact-string
                # matching silently skipped the filter when labels varied,
                # leaking non-native rows into a supposedly native run.
                _temp_norm = df['temperature'].astype(str).str.strip().str.upper()
                _temp_present = set(_temp_norm.dropna().unique().tolist())
                if "NATIVE" in _temp_present:
                    df = df[_temp_norm == "NATIVE"].reset_index(drop=True)
                    st.sidebar.success(
        ...
        _v22_display_sig = "|".join(str(x) for x in [
            ...
            # V27: pin state is a display change — toggling pins must re-render.
            # Widget values persist in session_state under their fixed keys.
            st.session_state.get("pin_enable", False),
            st.session_state.get("pin_run_a", None),
            st.session_state.get("pin_two", False),
            st.session_state.get("pin_run_b", None),
        ])
        ...
        if _v22_should_run:
                ...
                if "live score" in use_iep or "pre-scored" in use_iep:
                    ...
                    if len(_v23_feature_keys) < 2:
                        st.error(
                            ...
                            "Re-check more boxes in the sidebar 'Feature Selection' panel."
                        )
                        st.stop()
            ...
            with st.spinner("Analyzing graph..."):
                ...
                # V37: run-level metrics log (one row per graph build)
                log_mapper_run(
                    graph, analysis,
                    df.iloc[valid_indices].reset_index(drop=True), len(df),
                    question_id=v29_question_code(df, valid_indices),
                    feature_set=use_iep,
                    source_csv=(uploaded_file.name if uploaded_file is not None
                                else (", ".join(sorted(f.name for f in uploaded_files))
            ...
            with st.spinner("Generating visualization..."):
                ...
                _v27_is_tfidf1d  = (projection_type == "Word TF-IDF (1D)")  # V27
                ...
                # --- (a) 1D strip: PCA1 (dense) or TF-IDF SVD1 (sparse) ---
                if _v21_is_pca1 or _v27_is_tfidf1d:
                    _v18_projected = None
                    _v18_explained_var = None
                    _v27_axis_label = "PC1"
                    if _v27_is_tfidf1d:
                        # V27: TF-IDF content axis. Sparse/high-dim, so reduce
                        # with TruncatedSVD (LSA) — PCA would mean-center and
                        # densify the term matrix. Component 1 is the axis,
                        # exactly parallel to the "Word TF-IDF (2D)" lens.
                        _v27_axis_label = "TF-IDF SVD1"
                        try:
                            from sklearn.feature_extraction.text import TfidfVectorizer
                            from sklearn.decomposition import TruncatedSVD
                            _subset_tf = df.iloc[valid_indices]
                            _texts_tf = _subset_tf['response_text'].fillna('').astype(str).tolist()
                            _nonempty_tf = sum(1 for t in _texts_tf if t.strip())
                            if _nonempty_tf < 3:
                                raise ValueError(
                                    "Word TF-IDF (1D) needs at least 3 non-empty "
                                    f"response_text rows here (found {_nonempty_tf})."
                                )
                            _tfidf1d = TfidfVectorizer(max_features=2000, stop_words='english',
                                                       token_pattern=r"[A-Za-z][A-Za-z']+",
                                                       min_df=2, max_df=0.9)
                            _Xtf = _tfidf1d.fit_transform(_texts_tf)
                            if _Xtf.shape[1] < 2:
                                raise ValueError(
                                    "Word TF-IDF (1D) found fewer than 2 distinctive "
                                    "terms (text too uniform to project)."
                                )
                            _svd1 = TruncatedSVD(n_components=1, random_state=42)
                            _v18_projected = _svd1.fit_transform(_Xtf)   # (n, 1)
                            _v18_explained_var = float(_svd1.explained_variance_ratio_[0])
                        except Exception as _e:
                            st.error(f"TF-IDF (1D) fit failed: {_e}")
                            _v18_projected = None
                    else:
                        try:
                    ...
                    # --- V27: matched-run pin controls -----------------------
                    _pin_runs = []
                    _subset_pin = df.iloc[valid_indices]
                    if 'run' in _subset_pin.columns:
                        _run_min = int(_subset_pin['run'].min())
                        _run_max = int(_subset_pin['run'].max())
                        with st.expander("📍 Pin responses on the axis (matched run index)", expanded=False):
                            st.caption(
                                "Flag the same run number across all agents and drop an "
                                "arrow at each response's X-position. Matched index = fair "
                                "pull (same run for every agent). Pin a low-X and a high-X "
                                "run to show the breadth of the axis."
                            )
                            _pc1, _pc2, _pc3 = st.columns([1, 1, 1])
                            with _pc1:
                                _pin_on = st.checkbox("Enable pins", value=False, key="pin_enable")
                            with _pc2:
                                _run_a = st.number_input(
                                    "Run #1", min_value=_run_min, max_value=_run_max,
                                    value=min(17, _run_max), step=1, key="pin_run_a")
                            with _pc3:
                                _two = st.checkbox("Pin a second run", value=False, key="pin_two")
                                _run_b = st.number_input(
                                    "Run #2", min_value=_run_min, max_value=_run_max,
                                    value=min(89, _run_max), step=1, key="pin_run_b",
                                    disabled=not _two)
                            if _pin_on:
                                _pin_runs = [int(_run_a)] + ([int(_run_b)] if _two else [])
                                _pin_runs = sorted(set(_pin_runs))

                    _v21_title = f"SYN-IQ {TOOL_VERSION} · {projection_type} · {color_by}"
                    try:
                        _fig = render_plotly_agent_view(
                            ...
                            title=_v21_title,
                            pin_runs=_pin_runs,
                            axis_label=_v27_axis_label,
                        )
                    except Exception as _render_e:
                        import traceback as _tb
                        st.error(f"Strip render failed: {type(_render_e).__name__}: {_render_e}")
                    ...
                    if _fig is not None:
                        ...
                        # --- V27: flagged-response text panel + completeness --
                        _pin_rep = st.session_state.get('_pin_report', [])
                        _pin_miss = st.session_state.get('_pin_missing', [])
                        if _pin_miss:
                            _mtxt = ", ".join(f"run {r}: {a} absent" for r, a in _pin_miss)
                            st.warning(
                                "⚠️ Pinned data incomplete — "
                                f"{_mtxt}. This gap would need noting in Materials "
                                "and Methods. (Complete data pins one arrow per agent "
                                "per run with no warning.)"
                            )
                        if _pin_rep:
                            st.markdown("**Pinned responses** (position on axis + full text):")
                            for _pr in _pin_rep:
                                _hdr = (f"{_pr['agent']} · run {_pr['run']} — "
                                        f"PCA1={_pr['pca1']} · {_pr['question']} · "
                                        f"{_pr['total_words']} words")
                                # Full text, no truncation — long answers (e.g. Claude)
                                # carry the evidence for why they separate, so the
                                # complete response must be readable.
                                with st.expander(_hdr, expanded=False):
                                    st.write(_pr['response_text'])
                        _var_note = (
                            f" · {_v27_axis_label} explained_variance={_v18_explained_var*100:.1f}%"
                        ...
                        st.caption(
                            f"V29 render · {projection_type} strip plot · "
                ...
                _temps  = (sorted(subset['temperature'].astype(str).str.strip().str.upper().unique().tolist())
                           if 'temperature' in subset.columns else [])
                ...
                # V37: stamp the tool version into the exported HTML title.
                # Previously the descriptive title ("4 Agents — Native —
                # Consciousness") carried no version, so an exported figure
                # could not be traced to the code that produced it.
                _desc = " — ".join(_parts) if _parts else "SYN-IQ Mapper Analysis"
                viz_title = f"{_desc} · SYN-IQ {TOOL_VERSION}"
...
"""
                ...
                # ── V30: cube-label config → injected as window.V29_LABELS ──
                # Read by the label block inside the V21 updatePies() loop, so
                # labels sit on the true kepler positions and export with the
                # figure. Injected before </body>, ahead of the V21 script.
                try:
                    import json as _json30
                    _lbl_script = (
                        "<script>window.V29_LABELS = "
                        + _json30.dumps(_v30_label_cfg)
                        + ";</script>"
                    )
                    html_content = html_content.replace(
                        "</body>", _lbl_script + "</body>", 1
                    )
                except Exception:
                    pass
...
"""
            ...
            # V28 — needed to map graph member indices back to dataframe rows.
            # Without this the results block cannot resolve answer numbers.
            st.session_state.stored_valid_indices = list(valid_indices)
            st.session_state.chat_history = []
            st.session_state.topology_context = None
            st.session_state.chat_active = False
            st.session_state.claude_analysis = None
        ...
        if st.session_state.mapper_analysis_data is not None:
            ...
            # ── V28: Answer Numbers by Node ──────────────────────────────────
            # Resolves each node to the individual harvested answers inside it,
            # and each answer to clustered / solitary / unplaced.
            _v28_graph = st.session_state.get("stored_graph")
            _v28_df = st.session_state.get("stored_df")
            _v28_valid = st.session_state.get("stored_valid_indices")

            if _v28_graph is not None and _v28_df is not None and _v28_valid:
                st.markdown("### 🔢 Answer Numbers by Node")
                st.caption(
                    "Which individual answers sit in each node, and which stand alone. "
                    "Cover patches overlap, so one answer can appear in several nodes — "
                    "status reports the strongest condition that holds anywhere."
                )

                _id_opts = [c for c in ("turn_id", "run", "source_row")
                            if c in _v28_df.columns or c == "source_row"]
                _id_col = st.selectbox(
                    "Answer number column",
                    _id_opts,
                    index=0,
                    key="v28_id_col",
                    help="turn_id is the unique harvest ID. run is the run index "
                         "within agent x question, so it repeats across agents.",
                )

                try:
                    # ── V29: build the label map, capture run provenance ──
                    _q_code = v29_question_code(_v28_df, _v28_valid)
                    _f_code, _f_ok = v29_feature_code(use_iep)
                    _l_code, _l_ok = v29_lens_code(projection_type)
                    if not (_f_ok and _l_ok):
                        st.warning(
                            "V29 could not map "
                            + ("the Feature Type" if not _f_ok else "")
                            + (" and " if not (_f_ok or _l_ok) else "")
                            + ("the Projection / Lens" if not _l_ok else "")
                            + " to a known short code, so a generic slug was "
                              "used in node labels. The labels are still "
                              "unique within this run, but check the code "
                              "tables before citing them across runs."
                        )
                    _v29_params = v29_cover_params(
                        n_cubes, overlap, eps, min_samples,
                        projection_type, use_iep,
                        st.session_state.get("_v23_preset", ""),
                    )
                    _v29_labels = v29_relabel_nodes(
                        _v28_graph.get("nodes", {}), _q_code, _f_code, _l_code
                    )
                    _v29_stem = v29_export_stem(_q_code, _f_code, _l_code)

                    _v28_nodes, _v28_answers, _v28_sum = _v28_answer_membership(
                        _v28_graph, _v28_df, _v28_valid, id_col=_id_col,
                        label_map=_v29_labels,
                    )
                    st.caption(
                        f"Run identity: **{_q_code}-{_f_code}-{_l_code}** · "
                        f"{n_cubes} cubes · {overlap:.0%} overlap · "
                        f"DBSCAN eps {eps}, min_samples {min_samples}. "
                        "Node numbers are ranked by membership (N001 is the "
                        "largest node); the original cube id is kept in every "
                        "export as cube_id."
                    )

                    m1, m2, m3, m4, m5 = st.columns(5)
                    m1.metric("Answers in", _v28_sum["answers_in"])
                    m2.metric("Clustered", _v28_sum["clustered"])
                    m3.metric("Solitary", _v28_sum["solitary"],
                              help="Sits in a node by itself.")
                    m4.metric("Unplaced", _v28_sum["unplaced"],
                              help="In no node at all — DBSCAN noise. "
                                   "Never drawn on the graph.")
                    m5.metric("Lone agent", _v28_sum["lone_agent"],
                              help="Shares a node but is its agent's only "
                                   "delegate there.")

                    if _v28_sum["unplaced"]:
                        st.warning(
                            f"{_v28_sum['unplaced']} of {_v28_sum['answers_in']} answers "
                            f"({100*_v28_sum['unplaced']/max(_v28_sum['answers_in'],1):.1f}%) "
                            f"landed in no node and are absent from the graph. "
                            f"Raise eps or lower min_samples to place them."
                        )

                    _t1, _t2, _t3 = st.tabs(
                        ["By node", "By answer", "Inspect one node"]
                    )

                    with _t1:
                        _only_sol = st.checkbox(
                            "Solitary nodes only", value=False, key="v28_sol_nodes"
                        )
                        _nt = _v28_nodes[_v28_nodes["size"] == 1] if _only_sol else _v28_nodes
                        st.dataframe(_nt, use_container_width=True, hide_index=True)
                        st.download_button(
                            "📥 Node membership (CSV)",
                            v29_stamp(_v28_nodes, _v29_params).to_csv(index=False),
                            f"{_v29_stem}_node_membership.csv", "text/csv",
                            key="v28_dl_nodes",
                        )

                    with _t2:
                        _f1, _f2 = st.columns(2)
                        _stat = _f1.multiselect(
                            "Status",
                            ["CLUSTERED", "SOLITARY", "UNPLACED"],
                            default=["CLUSTERED", "SOLITARY", "UNPLACED"],
                            key="v28_stat",
                        )
                        _ag = None
                        if "agent" in _v28_answers.columns:
                            _agents_all = sorted(_v28_answers["agent"].dropna().unique().tolist())
                            _ag = _f2.multiselect(
                                "Agent", _agents_all, default=_agents_all, key="v28_ag"
                            )
                        _at = _v28_answers[_v28_answers["status"].isin(_stat)]
                        if _ag is not None:
                            _at = _at[_at["agent"].isin(_ag)]
                        st.dataframe(_at, use_container_width=True, hide_index=True)
                        st.download_button(
                            "📥 Answer membership (CSV)",
                            v29_stamp(_v28_answers, _v29_params).to_csv(index=False),
                            f"{_v29_stem}_answer_membership.csv", "text/csv",
                            key="v28_dl_answers",
                        )
                    ...
                    with _t3:
                        if len(_v28_nodes):
                            _pick = st.selectbox(
                                "Node",
                                _v28_nodes["node"].tolist(),
                                key="v28_pick_node",
                                format_func=lambda nd: (
                                    f"{nd}  —  "
                                    f"{int(_v28_nodes.loc[_v28_nodes['node']==nd,'size'].iloc[0])} "
                                    f"member(s)"
                                ),
                            )
                            _row = _v28_nodes[_v28_nodes["node"] == _pick].iloc[0]
                            st.markdown(
                                f"**Node {_pick}** — {_row['size']} member(s), "
                                f"{_row['status']}  \n"
                                f"Agents: {_row['agents'] or 'n/a'}  \n"
                                f"Questions: {_row['questions'] or 'n/a'}  \n"
                                f"Answers: {_row['answer_numbers']}"
                            )
                            st.caption(
                                f"cube_id `{_row['cube_id']}` — the cover cell "
                                "this cluster fell into. Kept for cross-"
                                "referencing older notes; the cell index "
                                "carries no information about the responses."
                            )
                            _ids = [s.strip() for s in
                                    re.findall(r"\[([^\]]*)\]", _row["answer_numbers"])]
                            _flat = [x.strip() for grp in _ids for x in grp.split(",") if x.strip()]
                            _sel = _v28_answers[
                                _v28_answers["answer"].astype(str).isin(_flat)
                            ]
                            _show = [c for c in ("answer", "agent", "question_id",
                                                 "run", "status", "n_nodes",
                                                 "lone_of_agent", "source_row")
                                     if c in _sel.columns]
                            st.dataframe(_sel[_show], use_container_width=True,
                                         hide_index=True)

                            if "response_text" in _v28_df.columns:
                                st.markdown("**Response text**")
                                for _, _r in _sel.iterrows():
                                    _txt = str(_v28_df.iloc[int(_r["source_row"])]
                                               ["response_text"])
                                    _hdr = f"{_r['answer']}"
                                    if "agent" in _r:
                                        _hdr += f" · {_r['agent']}"
                                    if "question_id" in _r:
                                        _hdr += f" · {_r['question_id']}"
                                    with st.expander(_hdr):
                                        st.write(_txt)

                            # ── V29: position and tightness (Section 2.4) ──
                            _chans = v29_available_channels(_v28_df)
                            if _chans and len(_sel):
                                st.markdown("**Why these sit together**")
                                st.caption(
                                    "Position is the standardized mean "
                                    "difference from every response not in "
                                    "this node. Tightness is the ratio of "
                                    "this node's SD to that baseline's; "
                                    "below 1 means the members agree more "
                                    "closely than the corpus does. p_tight "
                                    "is the fraction of 2,000 size-matched "
                                    "random draws at least as tight. The two "
                                    "are not interchangeable: an unusual "
                                    "mean and unusual agreement are "
                                    "different findings."
                                )
                                _mem_idx = [
                                    _v28_valid.index(int(_r["source_row"]))
                                    for _, _r in _sel.iterrows()
                                    if int(_r["source_row"]) in _v28_valid
                                ]
                                _pt = v29_position_tightness(
                                    _v28_df, _v28_valid, _mem_idx, _chans
                                )
                                if len(_pt):
                                    st.dataframe(_pt, use_container_width=True,
                                                 hide_index=True)
                                    _deg = _pt[_pt["degenerate"]]["channel"].tolist()
                                    if _deg:
                                        st.warning(
                                            "Zero within-node variance on: "
                                            + ", ".join(_deg)
                                            + ". Tightness reads as 0 on these, "
                                            "but that means the channel is "
                                            "absent from every member, not "
                                            "that the members converged. Do "
                                            "not report these as convergence."
                                        )
                                    if len(_sel) < 4:
                                        st.info(
                                            f"Only {len(_sel)} members — check "
                                            "loo_tightness_max before drawing "
                                            "any conclusion, since a single "
                                            "response can carry the result."
                                        )
                                    st.download_button(
                                        "📥 Position / tightness (CSV)",
                                        v29_stamp(_pt, _v29_params).to_csv(index=False),
                                        f"{_v29_stem}_{_pick}_position_tightness.csv",
                                        "text/csv", key="v29_dl_pt",
                                    )
                        else:
                            st.info("No nodes to inspect.")

                except Exception as _e:
                    st.warning(f"V28 answer-number resolution unavailable: {_e}")
            ...
            if st.session_state.mapper_results_html:
                # V32: re-inject the CURRENT label config every render so the
                # size slider, placement, hide-list and per-cube nudges apply
                # LIVE without rebuilding the mapper. The pie/label JS reads
                # window.V29_LABELS at tick time, so swapping the config script
                # in the stored HTML is enough — no expensive recompute.
                import re as _re32, json as _json32
                _viz_html = st.session_state.mapper_results_html
                try:
                    _viz_html = _re32.sub(
                        r"<script>window\.V29_LABELS\s*=.*?</script>",
                        "", _viz_html, flags=_re32.DOTALL
                    )
                    _fresh_cfg = "<script>window.V29_LABELS = " + _json32.dumps(_v30_label_cfg) + ";</script>"
                    _viz_html = _viz_html.replace("</body>", _fresh_cfg + "</body>", 1)
                    # V40: the position dump needs no config — the browser
                    # returns x/y only, and every render setting lives in the
                    # Python Figure Builder.
                    st.session_state.mapper_results_html = _viz_html
                except Exception:
                    _viz_html = st.session_state.mapper_results_html
                ...
                st.components.v1.html(_viz_html, height=800, scrolling=True)

                # V21 — surface provenance + V21 render-mode note under the figure
                _v21_render_note_parts = []
                if color_by == "agent (pie composition)":
                    _v21_render_note_parts.append("pie composition (kepler HTML + pie injection)")
...
# V40 — FIGURE BUILDER
# =============================================================================
# Renders publication figures from settled Mapper layouts, in Python.
#
# The division of labour: the browser reports x/y for each node and nothing
# else. Membership, wedge colors, wedge fractions, cube ids and the edge list
# are all read from the graph object Python already holds. Nothing about the
# figure depends on scraping a mutating DOM.
#
# The scale problem this exists to solve: a per-panel fit computes a different
# scale factor for each question, then multiplies node radii and stroke widths
# by it, so the same 40-response node renders at a different physical size in
# each panel. Here position scale is computed ONCE across every panel in the
# set, and radii, stroke widths and label sizes are specified in output units
# and never multiplied by it.

def v40_pca_align(positions):
    """Rotate a settled layout so its principal axis is horizontal.

    A force-directed layout is seed-dependent: the same graph settles at a
    different orientation on every run, so five panels of a five-panel figure
    arrive at five arbitrary rotations. Rotation is not information — the
    topology is unchanged — so canonicalizing it makes the panels read as a
    series instead of as five unrelated blobs.

    Sign is fixed deterministically (positive x-skew, positive y-skew) so the
    ...
    P = _np.array([positions[n] for n in names], dtype=float)
    P = P - P.mean(axis=0)
    try:
        _, _, Vt = _np.linalg.svd(P, full_matrices=False)
        R = Vt.T
        Q = P @ R
        if float((Q[:, 0] ** 3).sum()) < 0:
            Q[:, 0] *= -1
        if float((Q[:, 1] ** 3).sum()) < 0:
            Q[:, 1] *= -1
    except Exception:
        Q = P
    return {n: (float(Q[i, 0]), float(Q[i, 1])) for i, n in enumerate(names)}


def v40_panel_extent(positions):
    """Bounding box of node CENTERS only.

    Radii are deliberately excluded. Mixing output-unit radii into a
    layout-unit bounding box (V39 did this in absolute mode) makes the fit
    circular: bigger radii inflate the box, which shrinks the scale, which
    shrinks the radii.
    """
...
def v40_shared_scale(panels, k_radius, frame_w, frame_h, pad):
    """One position scale for the whole set of panels.

    Computed as the tightest requirement across every panel, so the panel that
    needs the most room determines the scale and every other panel inherits it.
    The returned value is layout-units-to-output-pixels and is applied to
    positions only.
    """
    best = None
    for p in panels:
        ...
        if not pos:
            continue
        x0, y0, x1, y1 = v40_panel_extent(pos)
        gw, gh = max(x1 - x0, 1e-9), max(y1 - y0, 1e-9)
        rmax = k_radius * (max(n["members"] for n in p["nodes"]) ** 0.5) if p["nodes"] else 0.0
        usable_w = max(frame_w - 2 * pad - 2 * rmax, 1.0)
        usable_h = max(frame_h - 2 * pad - 2 * rmax, 1.0)
        s = min(usable_w / gw, usable_h / gh)
        best = s if best is None else min(best, s)
    return best if best else 1.0


def v40_wedge_path(cx, cy, r, a0, a1):
    """One pie wedge as an SVG path.

    Callers must not pass a full turn: an elliptical arc whose endpoints
    coincide is dropped by the SVG spec, so a sole wedge at frac=1.0 renders
    nothing. Full circles are emitted as <circle> by v40_render_node instead.
    """
    import math as _m
    x0 = cx + r * _m.cos(a0)
    y0 = cy + r * _m.sin(a0)
...
def v40_render_node(cx, cy, r, wedges, wedge_stroke, meta=None):
    """Emit one node. Single-wedge nodes become a circle, not a 360-degree arc.

    When `meta` is supplied the node is wrapped in a <g> carrying its cube id,
    membership and architecture breakdown as data attributes, so the HTML
    export can show the same hover detail as the KeplerMapper page without a
    force simulation running underneath it.
    """
    import math as _m
    out = []
    if meta:
        out.append('<g class="nd" data-name="{}" data-cube="{}" data-members="{}" '
                   'data-comp="{}">'.format(meta.get("name", ""), meta.get("cube", ""),
                                            meta.get("members", ""), meta.get("comp", "")))
    live = [w for w in (wedges or []) if float(w.get("frac", 0)) > 0]
    if not live:
        out.append(f'<circle cx="{cx:.2f}" cy="{cy:.2f}" r="{r:.2f}" '
                   f'fill="#999999" stroke="#ffffff" stroke-width="{wedge_stroke}"/>')
        if meta:
            out.append('</g>')
        return out
    if len(live) == 1 or float(live[0].get("frac", 0)) >= 0.9995:
        out.append(f'<circle cx="{cx:.2f}" cy="{cy:.2f}" r="{r:.2f}" '
                   f'fill="{live[0]["color"]}" stroke="#ffffff" '
                   f'stroke-width="{wedge_stroke}"/>')
        if meta:
            out.append('</g>')
        return out
    total = sum(float(w["frac"]) for w in live)
    start = -_m.pi / 2
    for w in live:
        end = start + (float(w["frac"]) / total) * 2 * _m.pi
        out.append(f'<path d="{v40_wedge_path(cx, cy, r, start, end)}" '
                   f'fill="{w["color"]}" stroke="#ffffff" '
                   f'stroke-width="{wedge_stroke}"/>')
        start = end
    if meta:
        out.append('</g>')
...
def v40_render_panel(panel, scale, *, k_radius, frame_w, frame_h, pad,
                     edge_width, wedge_stroke, label_mode, label_size,
                     show_caption=True, interactive=False):
    """Render one panel's contents as a list of SVG elements, origin at (0,0).

    `scale` is supplied by the caller and is identical across panels. Radii,
    stroke widths and font sizes are in output units and are never multiplied
    by it — that separation is the entire point of this function.
    """
    pos = panel["_pos"]
    x0, y0, x1, y1 = v40_panel_extent(pos)
    cx_layout = (x0 + x1) / 2.0
    ...
    def TX(x):
        return (x - cx_layout) * scale + frame_w / 2.0

    def TY(y):
        return (y - cy_layout) * scale + frame_h / 2.0

    out = []
    by_name = {n["name"]: n for n in panel["nodes"]}
    ...
    for a, b in panel["edges"]:
        if a not in pos or b not in pos:
            continue
        out.append(f'<line x1="{TX(pos[a][0]):.2f}" y1="{TY(pos[a][1]):.2f}" '
                   f'x2="{TX(pos[b][0]):.2f}" y2="{TY(pos[b][1]):.2f}" '
                   f'stroke="#bcbcbc" stroke-width="{edge_width}"/>')
        drawn_edges += 1
    ...
    for n in panel["nodes"]:
        ...
        if nm not in pos:
            continue
        cx, cy = TX(pos[nm][0]), TY(pos[nm][1])
        r = k_radius * (max(int(n.get("members", 1)), 1) ** 0.5)
        _meta = None
        if interactive:
            _comp = ", ".join(
                f'{w.get("label") or "?"} {round(float(w["frac"]) * 100)}%'
                for w in (n.get("wedges") or []))
            _meta = {"name": nm, "cube": n.get("cube", ""),
                     "members": int(n.get("members", 1)), "comp": _comp}
        out.extend(v40_render_node(cx, cy, r, n.get("wedges"), wedge_stroke,
                                   meta=_meta))
        drawn_nodes += 1
        if label_mode != "none":
            if label_mode == "all" or (label_mode == "large" and
                                       int(n.get("members", 1)) >= 10):
                labels.append(
                    ...
                    f'stroke-width="{max(2.5, label_size * 0.28):.1f}" '
                    f'fill="#111111" pointer-events="none">'
                    f'{n.get("cube", "")}</text>')
    out.extend(labels)

    if show_caption and panel.get("caption"):
        out.append(f'<text x="{pad}" y="{frame_h - pad / 2:.0f}" '
                   f'font-family="Arial,Helvetica,sans-serif" font-size="20" '
                   f'fill="#333333">{panel["caption"]}</text>')

    panel["_drawn"] = {"nodes": drawn_nodes, "edges": drawn_edges}
    return out


def v40_build_figure(panels, *, k_radius=7.0, frame_w=V40_FRAME_W,
                     frame_h=V40_FRAME_H, pad=60, edge_width=1.1,
                     wedge_stroke=0.9, label_mode="none", label_size=13,
                     align=True, cols=None, legend=None, panel_letters=True,
                     scale_mode="shared", interactive=False):
    """Render the panel set and return (combined_svg, per_panel_svgs, report).

    scale_mode='shared'   one position scale across every panel, so distances
                          between nodes are comparable panel to panel. The
                          panel needing the most room sets the scale; a
                          tighter panel therefore occupies less of its frame,
                          which is the honest depiction.
    scale_mode='per_panel' each panel fills its own frame. Node radii and
                          stroke widths stay in output units and stay
                          identical, so this is still safe for a figure — it
                          gives up only the comparability of DISTANCE.
    """
    for p in panels:
        raw = {k: tuple(v) for k, v in p["positions"].items()}
        p["_pos"] = v40_pca_align(raw) if align else raw

    shared = v40_shared_scale(panels, k_radius, frame_w, frame_h, pad)
    if scale_mode == "per_panel":
        scales = {p["label"]: v40_shared_scale([p], k_radius, frame_w, frame_h, pad)
                  for p in panels}
    else:
        scales = {p["label"]: shared for p in panels}
    scale = shared

    # Defined before any drawing so it can be stamped into every SVG emitted
    # by this call, single-panel and combined alike.
    style = {"v40_k": k_radius, "v40_pad": pad, "v40_edge": edge_width,
             "v40_lblmode": label_mode, "v40_lblsize": label_size,
             "v40_align": align, "v40_cols": cols,
             "wedge_stroke": wedge_stroke,
             "frame_w": frame_w, "frame_h": frame_h,
             "scale_mode": scale_mode}

    per_panel = {}
    for p in panels:
        body = v40_render_panel(
            p, scales[p["label"]], k_radius=k_radius, frame_w=frame_w, frame_h=frame_h,
            pad=pad, edge_width=edge_width, wedge_stroke=wedge_stroke,
            label_mode=label_mode, label_size=label_size,
            interactive=interactive)
        svg = (f'<svg xmlns="http://www.w3.org/2000/svg" '
               f'viewBox="0 0 {frame_w} {frame_h}" width="{frame_w}" '
               f'height="{frame_h}">'
               f'<rect width="{frame_w}" height="{frame_h}" fill="#ffffff"/>'
               + "\n".join(body)
               + "<!-- SYNIQ " + TOOL_VERSION + " figure style: "
               + json.dumps({"v40_k": k_radius, "scale_mode": scale_mode,
                             "panel": p["label"]}) + " -->"
               + '</svg>')
        per_panel[p["label"]] = svg
    ...
    rows = (n + cols - 1) // cols
    W, H = cols * frame_w, rows * frame_h + (90 if legend else 0)
    ...
    for i, p in enumerate(panels):
        rx, ry = (i % cols) * frame_w, (i // cols) * frame_h
        parts.append(f'<g transform="translate({rx},{ry})">')
        parts.extend(v40_render_panel(
            p, scales[p["label"]], k_radius=k_radius, frame_w=frame_w, frame_h=frame_h,
            pad=pad, edge_width=edge_width, wedge_stroke=wedge_stroke,
            label_mode=label_mode, label_size=label_size,
            interactive=interactive))
        if panel_letters:
            parts.append(f'<text x="{pad}" y="{pad + 10}" '
                         f'font-family="Arial,Helvetica,sans-serif" '
                         f'font-size="34" font-weight="bold" fill="#111111">'
                         f'{letters[i] if i < len(letters) else str(i + 1)}</text>')
        parts.append('</g>')
    ...
    if legend:
        ...
        for name, color in legend.items():
            parts.append(f'<circle cx="{lx + 10}" cy="{ly}" r="11" fill="{color}"/>')
            parts.append(f'<text x="{lx + 30}" y="{ly}" font-family="Arial,Helvetica,sans-serif" '
                         f'font-size="24" dominant-baseline="central" fill="#222222">{name}</text>')
            lx += 60 + 15 * len(name)
    # Stamp the style into the file so a figure can be checked against its
    # siblings months later without depending on anyone's notes.
    parts.append("<!-- SYNIQ " + TOOL_VERSION + " figure style: "
                 + json.dumps(style) + " -->")
    parts.append('</svg>')
    ...
    report = {
        ...
        "k_radius_px": k_radius,
        "frame": f"{frame_w}x{frame_h}",
        "panels": [
            {"label": p["label"],
             "position_scale": round(scales[p["label"]], 5),
             "nodes_in_payload": len(p["nodes"]),
             "positions_supplied": len(p["positions"]),
             "nodes_drawn": p.get("_drawn", {}).get("nodes", 0),
             "edges_drawn": p.get("_drawn", {}).get("edges", 0),
             "largest_node_members": max([n["members"] for n in p["nodes"]] or [0]),
             "largest_node_radius_px": round(
                 k_radius * (max([n["members"] for n in p["nodes"]] or [1]) ** 0.5), 1)}
            for p in panels
        ],
    }
    return "\n".join(parts), per_panel, report



def v40_panel_from_graph(graph, subset, label, *, agent_col="agent"):
    """Turn a KeplerMapper graph plus its row subset into a Figure Builder panel.

    Reads membership straight off the graph rather than off anything drawn,
    so node area encodes the number of responses in the node and nothing else.
    """
    nodes_map = graph.get("nodes", {}) or {}
    links = graph.get("links", {}) or {}

    edges, seen = [], set()
    for s, ts in links.items():
        for t in (ts or []):
    ...
    nodes = []
    for nid, idxs in nodes_map.items():
        rows = subset.iloc[list(idxs)]
        counts = (rows[agent_col].value_counts().to_dict()
                  if agent_col in rows.columns else {})
        total = sum(counts.values())
        wedges = []
        if total:
            ordered = [a for a in AGENT_COLORS if a in counts]
            ordered += [a for a in counts if a not in ordered]
            for a in ordered:
                if counts[a] > 0:
                    wedges.append({"color": get_agent_color(a),
                                   "frac": counts[a] / total, "label": str(a)})
        m = re.search(r"cube(\d+)", str(nid))
        nodes.append({"name": str(nid), "cube": m.group(1) if m else str(nid),
                      "members": int(len(idxs)), "wedges": wedges})

    return {"label": label, "caption": label, "nodes": nodes, "edges": edges,
            "positions": {}, "tool_version": TOOL_VERSION, "layout": "python"}


def v40_figure_html(combined_svg, title="SYN-IQ Mapper figure", report=None):
    """Wrap a rendered figure in a standalone HTML page with hover detail.

    KeplerMapper's page is recognisable but it is not reproducible: a live
    force simulation settles somewhere different on every load, which is why
    the same graph could not be captured twice at the same scale. This page
    carries the same visual language and the same hover information with the
    coordinates already fixed, so it opens identically every time, on any
    machine, with no network access and no simulation running.
    """
    prov = ""
    if report:
        rows = "".join(
            f"<tr><td>{p['label']}</td><td>{p['nodes_drawn']}</td>"
            f"<td>{p['edges_drawn']}</td><td>{p['largest_node_members']}</td></tr>"
            for p in report.get("panels", []))
        prov = (
            "<details><summary>Figure provenance</summary>"
            f"<p>Rendered by SYN-IQ {TOOL_VERSION}. Scale mode "
            f"<b>{report.get('scale_mode')}</b>, shared position scale "
            f"{report.get('shared_scale')}, node radius "
            f"{report.get('k_radius_px')} px per sqrt(member), frame "
            f"{report.get('frame')}.</p>"
            "<table><tr><th>Panel</th><th>Nodes</th><th>Edges</th>"
            "<th>Largest node</th></tr>" + rows + "</table></details>")
...
<style>
 ...
 #tt {{ position:fixed; display:none; pointer-events:none; z-index:99;
        background:#111; color:#fff; padding:8px 11px; border-radius:6px;
        font-size:12px; line-height:1.5; max-width:280px;
        box-shadow:0 3px 10px rgba(0,0,0,.3); }}
 details {{ margin-top:18px; font-size:13px; color:#444; }}
...
<script>
 ...
 document.querySelectorAll('.nd').forEach(function(el){{
   el.addEventListener('mousemove', function(e){{
     tt.innerHTML = '<b>cube ' + el.dataset.cube + '</b><br>'
       + el.dataset.members + ' responses<br>' + el.dataset.comp;
...
# ── V40 Figure Builder UI ────────────────────────────────────────────────────
st.markdown("---")
st.markdown("## 📐 Figure Builder")
st.caption(
    "Renders the settled layout in Python at one shared scale across every "
    "panel. Node radius is k·√members in output pixels and is never multiplied "
    "by the position scale, so the same membership count is the same physical "
    "size in every panel of the figure."
)

if "_v40_panels" not in st.session_state:
    st.session_state["_v40_panels"] = {}

_v40_tab4, _v40_tab1, _v40_tab2, _v40_tab3 = st.tabs(
    ["0 · Batch build (no browser)", "1 · Capture panel",
     "2 · Panels in set", "3 · Render figure"])


with _v40_tab4:
    st.markdown(
        "Builds every question's graph in Python with one set of parameters, "
        "lays each out deterministically, and renders the whole figure. No "
        "settling, no copy-paste, no switching the lens between panels."
    )
    _v41_df = (st.session_state.stored_df
               if st.session_state.get("stored_df") is not None else None)
    if _v41_df is None:
        st.info("Load a dataset above first.")
    else:
        _v41_qcol = ("question_id" if "question_id" in _v41_df.columns
                     else "question_label" if "question_label" in _v41_df.columns
                     else None)
        if not _v41_qcol:
            st.warning("No question_id or question_label column in this dataset.")
        else:
            _v41_qs = sorted(_v41_df[_v41_qcol].dropna().astype(str).unique().tolist())
            _v41_pick = st.multiselect("Questions (one panel each)", _v41_qs,
                                       default=_v41_qs, key="v41_qs")
            # V43 — the batch inherits the sidebar settings rather than
            # carrying its own copies. Two sets of parameter widgets for the
            # same computation is a trap: the panels silently come out of a
            # different configuration from the graph on screen, so the same
            # data yields a different node count and nothing in the figure
            # says why.
            _v41_keys = tuple(st.session_state.get(
                "_v23_feature_keys", tuple(V23_TOGGLE_FEATURES)))
            _v41_lens = projection_type
            _v41_cubes, _v41_ov = int(n_cubes), float(overlap)
            _v41_eps, _v41_ms = float(eps), int(min_samples)

            st.markdown("**Settings inherited from the sidebar**")
            st.dataframe(pd.DataFrame([
                {"setting": "Feature type", "value": use_iep},
                {"setting": "Lens (projection)", "value": _v41_lens},
                {"setting": "IEP features",
                 "value": (f"{len(_v41_keys)} selected"
                           if ("live score" in use_iep or "pre-scored" in use_iep)
                           else "n/a for this feature type")},
                {"setting": "N cubes", "value": _v41_cubes},
                {"setting": "Overlap", "value": _v41_ov},
                {"setting": "DBSCAN eps", "value": _v41_eps},
                {"setting": "min_samples", "value": _v41_ms},
            ]), use_container_width=True, hide_index=True)
            with st.expander("Which features"):
                st.write(", ".join(_v41_keys) if _v41_keys else "(none)")
            st.caption(
                "Change these in the sidebar, where the on-screen graph reads "
                "them from, so every panel matches the graph you have been "
                "looking at. They are held constant across all five panels, "
                "as the figure requires."
            )

            if st.button("Build all panels", key="v41_build", type="primary"):
                _made, _failed = [], []
                _bar = st.progress(0.0)
                for _i, _q in enumerate(_v41_pick):
                    try:
                        _sub = (_v41_df[_v41_df[_v41_qcol].astype(str) == _q]
                                .reset_index(drop=True))
                        # V44 — run the SAME feature branch the on-screen
                        # graph runs. V41/V43 always called build_iep_features
                        # and so ignored the Feature Type radio entirely: with
                        # embeddings selected, the graph came from 384 SBERT
                        # dimensions while the panels came from 12 IEP
                        # features, on identical rows.
                        if "live score" in use_iep or "pre-scored" in use_iep:
                            _dat, _vi = build_iep_features(_sub, _v41_keys)
                            if len(_dat) > 0:
                                _dat = StandardScaler().fit_transform(_dat)
                        elif "SBERT" in use_iep:
                            from sentence_transformers import SentenceTransformer
                            _mdl = SentenceTransformer('all-MiniLM-L6-v2')
                            _txt = _sub['response_text'].fillna('').tolist()
                            _vi = [i for i, t in enumerate(_txt) if t.strip()]
                            _dat = _mdl.encode([_txt[i] for i in _vi])
                        else:
                            _dat, _vi = parse_embeddings(_sub)
                        if len(_dat) == 0:
                            raise ValueError("no valid feature rows")
                        _mp, _g = run_mapper_analysis(
                            _dat, _sub, _vi, _v41_cubes, _v41_ov,
                            _v41_eps, _v41_ms, _v41_lens)
                        _ss = _sub.iloc[_vi].reset_index(drop=True)
                        _panel = v40_panel_from_graph(_g, _ss, _q)
                        _panel["positions"] = {
                            k: list(v) for k, v in v40_component_layout(
                                [n["name"] for n in _panel["nodes"]],
                                _panel["edges"]).items()}
                        _panel["lens"] = _v41_lens
                        _panel["feature_type"] = use_iep
                        _panel["feature_set"] = (
                            ",".join(_v41_keys)
                            if ("live score" in use_iep or "pre-scored" in use_iep)
                            else use_iep)
                        _panel["n_dims"] = int(np.asarray(_dat).shape[1])
                        _panel["cover"] = (f"n_cubes={_v41_cubes}, "
                                           f"overlap={_v41_ov}, eps={_v41_eps}, "
                                           f"min_samples={_v41_ms}")
                        st.session_state["_v40_panels"][_q] = _panel
                        _made.append((_q, len(_panel["nodes"]),
                                      len(_panel["edges"]), len(_ss),
                                      _panel["n_dims"]))
                    except Exception as _e:
                        _failed.append((_q, str(_e)[:110]))
                    _bar.progress((_i + 1) / max(len(_v41_pick), 1))
                if _made:
                    st.success(f"Built {len(_made)} panel(s).")
                    st.dataframe(pd.DataFrame(
                        _made, columns=["question", "nodes", "edges",
                                        "responses", "feature dims"]),
                        use_container_width=True)
                    st.caption("Now open **3 · Render figure**.")
                for _q, _msg in _failed:
                    st.error(f"{_q}: {_msg}")

with _v40_tab1:
    _v40_src = st.session_state.get("_v40_panel_graph")
    if not _v40_src:
        st.info(
            "Build a Mapper graph above in a pie-composition coloring mode. "
            "The panel payload — node membership, wedge colors and fractions, "
            "cube ids, edge list — is captured automatically when the graph "
            "is built."
        )
    else:
        st.success(
            f"Panel payload ready: {len(_v40_src['nodes'])} nodes, "
            f"{len(_v40_src['edges'])} edges "
            f"(paint mode: {_v40_src.get('paint_mode', '?')})."
        )
        st.markdown(
            "**On the graph**, wait until it stops moving, click "
            "**📍 Copy node positions**, then paste below."
        )
        _v40_label = st.text_input(
            "Panel label", value=_v40_src.get("question_hint", ""),
            key="v40_label",
            help="Used as the panel key, the caption and the filename. "
                 "Keep it short — Consciousness, Grief, Leave Job.")
        _v40_paste = st.text_area(
            "Paste node positions", height=110, key="v40_paste",
            placeholder='{"n_nodes": 53, "max_drift_px": 0.0, "positions": {...}}')
        if st.button("Capture panel", key="v40_capture"):
            try:
                _pj = json.loads(_v40_paste.strip())
                _pos = _pj.get("positions", _pj)
                if not isinstance(_pos, dict) or not _pos:
                    raise ValueError("no positions found in the pasted text")
                _pos = {str(k): [float(v[0]), float(v[1])] for k, v in _pos.items()}
                if not _v40_label.strip():
                    raise ValueError("give the panel a label first")
                _names = {n["name"] for n in _v40_src["nodes"]}
                _matched = len(_names & set(_pos.keys()))
                _panel = {
                    "label": _v40_label.strip(),
                    "caption": _v40_label.strip(),
                    "nodes": _v40_src["nodes"],
                    "edges": _v40_src["edges"],
                    "positions": _pos,
                    "paint_mode": _v40_src.get("paint_mode", ""),
                    "tool_version": TOOL_VERSION,
                    "captured": _pj.get("captured", ""),
                    "max_drift_px": _pj.get("max_drift_px", ""),
                }
                st.session_state["_v40_panels"][_v40_label.strip()] = _panel
                if _matched < len(_names):
                    st.warning(
                        f"{_matched} of {len(_names)} nodes matched a pasted "
                        f"position. Unmatched nodes will not be drawn — "
                        f"re-copy after the graph has fully rendered."
                    )
                else:
                    st.success(
                        f"Captured **{_v40_label.strip()}** — {_matched}/"
                        f"{len(_names)} nodes matched, drift "
                        f"{_pj.get('max_drift_px', '?')} px."
                    )
                st.download_button(
                    "📥 Download this panel (JSON)",
                    json.dumps(_panel),
                    file_name=f"v40_panel_{_v40_label.strip().replace(' ', '_')}.json",
                    mime="application/json", key="v40_dl_panel")
            except Exception as _e:
                st.error(f"Could not read that: {_e}")

with _v40_tab2:
    _v40_up = st.file_uploader(
        "Add panels saved earlier", type=["json"], accept_multiple_files=True,
        key="v40_upload",
        help="Panel JSONs downloaded from step 1. Lets a five-panel figure be "
             "assembled across sessions, since each question needs its own "
             "graph build.")
    if _v40_up:
        for _f in _v40_up:
            try:
                _p = json.loads(_f.read().decode("utf-8"))
                st.session_state["_v40_panels"][_p["label"]] = _p
            except Exception as _e:
                st.error(f"{_f.name}: {_e}")

    _v40_have = st.session_state["_v40_panels"]
    if not _v40_have:
        st.info("No panels captured yet.")
    else:
        st.caption(
            "The caption is what prints under the panel. It is free text and "
            "is independent of the question_id it was built from, so a "
            "misspelling in the source column does not have to reach the "
            "figure."
        )
        for _k in list(_v40_have.keys()):
            _c1, _c2, _c3 = st.columns([3, 4, 1])
            _c1.markdown(f"**{_k}**")
            _c1.caption(
                f"{len(_v40_have[_k]['nodes'])} nodes · "
                f"{len(_v40_have[_k]['positions'])} positions · "
                f"{_v40_have[_k].get('layout', 'browser')} layout")
            _newcap = _c2.text_input(
                "Panel caption", value=_v40_have[_k].get("caption", _k),
                key=f"v40_cap_{_k}", label_visibility="collapsed")
            if _newcap != _v40_have[_k].get("caption", _k):
                st.session_state["_v40_panels"][_k]["caption"] = _newcap
            if _c3.button("✕", key=f"v40_rm_{_k}"):
                del st.session_state["_v40_panels"][_k]
                st.rerun()

with _v40_tab3:
    _v40_have = st.session_state["_v40_panels"]
    if not _v40_have:
        st.info("Capture at least one panel first.")
    else:
        # V45 — a figure style is a file, not a set of slider positions.
        # Every figure in a paper has to be rendered identically; remembering
        # to reproduce six slider positions across separate sessions is not a
        # guarantee, it is an opportunity to be inconsistent.
        with st.expander("🎨 Figure style profile — lock settings across every figure",
                         expanded=False):
            st.caption(
                "Render the first figure, download the profile, then load it "
                "before rendering every other figure in the paper. Node radius "
                "is k·√members, so an identical k means a 40-response node is "
                "the same physical size in Figure 6 and Figure 7."
            )
            _sty_up = st.file_uploader("Load a style profile", type=["json"],
                                       key="v40_sty_up")
            if _sty_up is not None and st.button("Apply this profile",
                                                 key="v40_sty_apply"):
                try:
                    _sty = json.loads(_sty_up.read().decode("utf-8"))
                    for _kk, _vv in _sty.get("settings", {}).items():
                        st.session_state[_kk] = _vv
                    st.success("Profile applied.")
                    st.rerun()
                except Exception as _e:
                    st.error(f"Could not read that profile: {_e}")

        _order = st.multiselect(
            "Panels to render, in order",
            options=list(_v40_have.keys()),
            default=list(_v40_have.keys()), key="v40_order")
        _cc1, _cc2, _cc3 = st.columns(3)
        _v40_k = _cc1.slider("Node radius k (px per √member)", 2.0, 20.0, 7.0, 0.5,
                             key="v40_k",
                             help="Radius = k·√members, in output pixels, "
                                  "identical in every panel.")
        _v40_cols = _cc2.slider("Columns", 1, 5, min(len(_order) or 1, 3), key="v40_cols")
        _v40_pad = _cc3.slider("Frame padding (px)", 20, 200, 60, key="v40_pad")
        _dd1, _dd2, _dd3 = st.columns(3)
        _v40_edge = _dd1.slider("Edge width (px)", 0.3, 4.0, 1.1, 0.1, key="v40_edge")
        _v40_lblmode = _dd2.selectbox("Cube labels", ["none", "large", "all"],
                                      key="v40_lblmode",
                                      help="'large' labels nodes with 10 or "
                                           "more members.")
        _v40_lblsize = _dd3.slider("Label size (px)", 8, 30, 13, key="v40_lblsize")
        _v40_scalemode = st.radio(
            "Position scale",
            ["Shared across panels (distances comparable)",
             "Per-panel fit (each panel fills its frame)"],
            index=0, key="v40_scalemode",
            help="Shared is what a multi-panel figure normally wants: the "
                 "panel needing the most room sets the scale and a tighter "
                 "panel occupies less of its frame. Per-panel fit gives up "
                 "comparability of distance only — node radii and stroke "
                 "widths stay identical either way.")
        _v40_align = st.checkbox(
            "PCA-align each panel", value=True, key="v40_align",
            help="Rotates each settled layout so its principal axis is "
                 "horizontal. The topology is unchanged; it removes the "
                 "arbitrary per-run rotation so the panels read as a series.")
        _v40_legend = st.checkbox("Agent legend on combined figure", value=True,
                                  key="v40_legend")

        if st.button("Render figure", key="v40_render", type="primary"):
            try:
                _panels = [dict(_v40_have[k]) for k in _order]
                _combined, _each, _rep = v40_build_figure(
                    _panels, k_radius=_v40_k, pad=_v40_pad,
                    edge_width=_v40_edge, label_mode=_v40_lblmode,
                    label_size=_v40_lblsize, align=_v40_align,
                    cols=_v40_cols,
                    scale_mode=("per_panel"
                                if _v40_scalemode.startswith("Per-panel")
                                else "shared"),
                    interactive=True,
                    legend=dict(AGENT_COLORS) if _v40_legend else None)
                st.session_state["_v40_out"] = (_combined, _each, _rep)
            except Exception as _e:
                st.error(f"Render failed: {_e}")

        if st.session_state.get("_v40_out"):
            _combined, _each, _rep = st.session_state["_v40_out"]
            st.markdown(
                f"**Scale mode: {_rep['scale_mode']}** · "
                f"shared scale {_rep['shared_scale']} · "
                f"radius k = {_rep['k_radius_px']} px/√member · "
                f"frame {_rep['frame']}")
            st.dataframe(pd.DataFrame(_rep["panels"]), use_container_width=True)
            _bad = [p for p in _rep["panels"]
                    if p["nodes_drawn"] < p["nodes_in_payload"]]
            if _bad:
                st.warning(
                    "Some nodes had no position and were not drawn: "
                    + ", ".join(f"{p['label']} ({p['nodes_drawn']}/"
                                f"{p['nodes_in_payload']})" for p in _bad))
            st.download_button(
                "📥 Download combined figure (SVG)", _combined,
                file_name="v40_figure_combined.svg", mime="image/svg+xml",
                key="v40_dl_combined")
            st.download_button(
                "🌐 Download figure as HTML", 
                v40_figure_html(_combined,
                                title=" · ".join(_order) or "SYN-IQ figure",
                                report=_rep),
                file_name="v46_figure.html", mime="text/html",
                key="v40_dl_html",
                help="Standalone page with the same hover detail as the "
                     "KeplerMapper export, but with the coordinates fixed, so "
                     "it opens identically every time.")
            st.download_button(
                "🎨 Download style profile (JSON)",
                json.dumps({"tool_version": TOOL_VERSION,
                            "settings": _rep.get("style", {})}, indent=1),
                file_name="v45_figure_style.json", mime="application/json",
                key="v40_dl_style",
                help="Load this before rendering every other figure in the "
                     "paper so all of them share one style.")
            _cols_dl = st.columns(min(len(_each), 5) or 1)
            for _i, (_lab, _svg) in enumerate(_each.items()):
                _cols_dl[_i % len(_cols_dl)].download_button(
                    f"📥 {_lab}", _svg,
                    file_name=f"v40_panel_{_lab.replace(' ', '_')}.svg",
                    mime="image/svg+xml", key=f"v40_dl_{_lab}")
            st.components.v1.html(
                f'<div style="background:#fff">{_combined}</div>', height=620,
                scrolling=True)
            st.caption(
                "SVG is vector — it can be placed at any size without "
                "resampling, and rasterized to 300 dpi TIFF or PNG from the "
                "same file when the journal asks for it."
            )
...
<div style="text-align: center; color: #a0a0a0; padding: 1rem;">
    <strong>SYN-IQ Mapper Analyzer {TOOL_VERSION}</strong><br>
