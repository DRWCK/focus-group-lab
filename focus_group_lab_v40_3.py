"""
Focus Group Lab V40.3 — Research Edition
...
st.set_page_config(
    page_title="Focus Group Lab V40.3",
    page_icon="🧬",
    layout="wide",
...
VT_CHANNELS = ('S_t', 'B_t', 'Q_t', 'D_t', 'R_t')   # V40.3 canonical order: S, B, Q, D, R

# V40.3 saturation ledger. Counts how often a RAW channel exceeded 1.0 before
# clamping, i.e. how often the heuristic normalization constants are saturating.
# Module-level so it works both inside Streamlit and in offline verification.
_VT_SATURATION = {'n_scored': 0, 'n_saturated_responses': 0,
                  'by_channel': {c: 0 for c in VT_CHANNELS}}

def vt_saturation_reset():
    """Zero the saturation ledger. Call at the start of a harvest run."""
    _VT_SATURATION['n_scored'] = 0
    _VT_SATURATION['n_saturated_responses'] = 0
    for c in VT_CHANNELS:
        _VT_SATURATION['by_channel'][c] = 0

def vt_saturation_snapshot():
    """Return a copy of the saturation ledger for end-of-run reporting."""
    return {'n_scored': _VT_SATURATION['n_scored'],
            'n_saturated_responses': _VT_SATURATION['n_saturated_responses'],
            'by_channel': dict(_VT_SATURATION['by_channel'])}

def _vt_default(status):
    """Fallback return for empty / too-short text.

    V_raw and V_t are zero (nothing was measured). V̂ₜ keeps the historical
    uniform-0.2 simplex default. score_status flags the row so downstream
    analysis can exclude it.
    """
    zeros   = {c: 0.0 for c in VT_CHANNELS}
    uniform = {c: 0.2 for c in VT_CHANNELS}
    return {**zeros,
            'V_t': dict(zeros), 'V_raw': dict(zeros), 'V_hat': dict(uniform),
            'raw': dict(zeros),
            'saturated': False, 'saturated_channels': [],
            'score_status': status}

def score_vt(text):
    """Compute the voice-state vector and return THREE distinct quantities.

    V40.3 corrections (naming / bounding / export only — no channel formula
    was changed; the raw arithmetic is byte-identical to V40.2):

    1. CHANNEL RENAME A_t → B_t. The abstraction channel was named A_t, which
       collided with A_t = Action in center-state C_t = [I_t, E_t, A_t]. The
       Center-State manuscript is canonical and uses B_t for abstraction.
       Canonical channel order is now S, B, Q, D, R.

    2. THREE RETAINED QUANTITIES. Raw channels were floored at 0.0 but had no
       upper bound, so they could exceed 1.0 while the manuscript defines every
       channel as varying independently in [0, 1]. This function now emits:

         V_t   — CANONICAL. Each raw channel clamped to [0,1]. Channels
                 independent; does NOT sum to 1.0 in general. This is what the
                 paper defines and what analysis should use.
         V_raw — unclamped values, retained for calibration. Any response with
                 a raw channel > 1.0 before clamping is flagged (`saturated`)
                 and tallied in the module saturation ledger.
         V̂_t  — the simplex-normalized vector, sums to 1.0. Retained, but it
                 is a COMPOSITIONAL VIEW, not V_t as defined.

       Top-level S_t/B_t/Q_t/D_t/R_t keys carry V_t (the canonical, clamped,
       independent vector). V̂ₜ is reachable only via the explicit 'V_hat' key
       so the two can never be silently confused.

    3. score_status: 'measured' / 'default_empty' / 'default_short' so
       downstream analysis can exclude fallback values.

    Channel labels (Center-State manuscript, canonical):
      S_t = Structure Density
      B_t = Abstraction Level (concrete to conceptual)   [was A_t]
      Q_t = Querying Intensity
      D_t = Directiveness (strength of recommendations)
      R_t = Relational Warmth
    """
    if not text:
        return _vt_default('default_empty')
    if len(text.strip()) < 10:
        return _vt_default('default_short')
    ...
    # B_t — Abstraction Level (V40.3: renamed from A_t; formula UNCHANGED)
    ...
    raw_B = norm_score*0.50 + (latinate/n_words)*3.0*0.25 + long_r*2.5*0.25
    raw_B = max(0.0, raw_B)
    ...
    # --- V_raw: unclamped, retained for calibration -------------------------
    V_raw = {'S_t': round(raw_S, 4), 'B_t': round(raw_B, 4), 'Q_t': round(raw_Q, 4),
             'D_t': round(raw_D, 4), 'R_t': round(raw_R, 4)}

    # --- Saturation: which raw channels blew past the [0,1] definition ------
    sat_channels = [c for c in VT_CHANNELS if V_raw[c] > 1.0]
    _VT_SATURATION['n_scored'] += 1
    if sat_channels:
        _VT_SATURATION['n_saturated_responses'] += 1
        for c in sat_channels:
            _VT_SATURATION['by_channel'][c] += 1

    # --- V_t: CANONICAL. Clamped to [0,1], channels independent -------------
    # Does NOT sum to 1.0 in general — that is correct and intended.
    V_t = {c: round(min(1.0, max(0.0, V_raw[c])), 4) for c in VT_CHANNELS}

    # --- V̂ₜ: compositional view. Simplex projection of the RAW values, so ---
    # it is byte-identical to V40.2's output. Sums to 1.0. NOT V_t.
    total_vt = raw_S + raw_B + raw_Q + raw_D + raw_R
    if total_vt == 0:
        # All channels zero — historical uniform simplex default, flagged.
        V_hat = {c: 0.2 for c in VT_CHANNELS}
        return {**{c: 0.0 for c in VT_CHANNELS},
                'V_t': {c: 0.0 for c in VT_CHANNELS}, 'V_raw': V_raw, 'V_hat': V_hat,
                'raw': V_raw, 'saturated': False, 'saturated_channels': [],
                'score_status': 'default_empty'}

    V_hat = {'S_t': round(raw_S/total_vt, 4), 'B_t': round(raw_B/total_vt, 4),
             'Q_t': round(raw_Q/total_vt, 4), 'D_t': round(raw_D/total_vt, 4),
             'R_t': round(raw_R/total_vt, 4)}

    # Top-level channel keys carry V_t (canonical). V̂ₜ only via 'V_hat'.
    return {
        **V_t,
        'V_t': V_t,
        'V_raw': V_raw,
        'V_hat': V_hat,
        'raw': V_raw,                      # backward-compat alias for V_raw
        'saturated': bool(sat_channels),
        'saturated_channels': sat_channels,
...
V40_VERSION_STAMPS = {
    ...
    # V40.3: A_t→B_t rename, V_t clamped to [0,1], V_raw + V̂ₜ retained alongside
    "vt_engine_version":        "V40.3_SBQDR_clamped_triple",
    "tool_version":             "V40.3",
    "tool_role":                "focus_group",    # vs. "baseline_harvester" for V50
    "vt_channel_order":         "S,B,Q,D,R",      # canonical; B_t = abstraction
}
...
def check_password():
    ...
    <div class="main-header">
        <h1>🧬 Focus Group Lab <span class="v40-badge">V40.3</span></h1>
...
def render_score_badge(iep: dict, vt: dict):
    ...
    _sat_flag = ('<span style="color:#ffaa44;font-size:0.66rem;">⚠ raw&gt;1: '
                 + ",".join(c[0] for c in vt.get('saturated_channels', []))
                 + '</span>') if vt.get('saturated') else ''
    ...
    <div class="score-panel">
      ...
      <div style="margin-top:5px;color:#ccc;font-size:0.70rem;">
        ...
        <span class="vt-badge">B:{_vt_t['B_t']:.2f}</span>
      ...
      <div style="margin-top:3px;color:#888;font-size:0.68rem;">
        ...
        <span class="vt-badge">B:{_vt_h['B_t']:.2f}</span>
...
def call_claude(prompt: str, system: str, max_tokens: int = 4096) -> str:
    try:
        ...
        r = requests.post("https://api.anthropic.com/v1/messages",
            ...
            json={"model": AGENT_MODELS["Claude"], "max_tokens": max_tokens, "system": system,
...
def call_gemini(prompt: str, system: str, max_tokens: int = 4096) -> str:
    try:
        ...
        r = requests.post(
            f"https://generativelanguage.googleapis.com/v1beta/models/{AGENT_MODELS['Gemini']}:generateContent?key={key}",
...
"""
    for entry in score_history[-8:]:
        ...
        msg += (f"  {entry['agent']} (R{entry['round']}): "
                ...
                f"Vt S:{vt['S_t']:.2f} B:{vt['B_t']:.2f} Q:{vt['Q_t']:.2f} D:{vt['D_t']:.2f} R:{vt['R_t']:.2f}\n")
...
def export_to_markdown() -> str:
    md  = f"# Focus Group Lab V40.3 — Session Export\n"
    ...
    if st.session_state.score_history:
        ...
        for e in st.session_state.score_history:
            ...
            md += (f"- R{e['round']} {e['agent']}: IEP={iep['dominant']} "
                   ...
                   f"S:{_t['S_t']:.2f} B:{_t['B_t']:.2f} Q:{_t['Q_t']:.2f} "
                   f"D:{_t['D_t']:.2f} R:{_t['R_t']:.2f}"
                   f"{'  ⚠ raw>1: ' + ','.join(vt.get('saturated_channels', [])) if vt.get('saturated') else ''}\n"
                   f"    - V̂ₜ (compositional, Σ=1): "
                   f"S:{_h['S_t']:.2f} B:{_h['B_t']:.2f} Q:{_h['Q_t']:.2f} "
    ...
    if st.session_state.round1_responses:
        ...
        for agent, response in st.session_state.round1_responses.items():
            md += f"### {AGENT_EMOJIS.get(agent,'🤖')} {agent}\n{response}\n\n---\n\n"
    md += "\n---\n*Focus Group Lab V40.3 — Research Edition · SYNINT Team · April 2026*\n"
    return md
...
if session_type == "🔬 Auto Run":
    ...
    if run_auto_btn:
        ...
        experiment_run_id = datetime.now().strftime('%Y%m%d_%H%M%S') + "_V40_3"

        # V40.3: zero the saturation ledger so the end-of-run report counts
        # only this run's responses.
        vt_saturation_reset()
        ...
        for run_num in range(1, auto_n + 1):
            for agent_name in auto_agents:
                ...
                row = {
                    ...
                    **{f"vt_{c[0]}":   vt['V_t'][c]   for c in VT_CHANNELS},
                    **{f"vraw_{c[0]}": vt['V_raw'][c] for c in VT_CHANNELS},
                    **{f"vhat_{c[0]}": vt['V_hat'][c] for c in VT_CHANNELS},
        ...
        # V40.3: report how often the raw heuristics saturated past the [0,1]
        # definition. High counts mean the normalization constants for that
        # channel need recalibration, not that the response was extreme.
        _sat = vt_saturation_snapshot()
        st.session_state["vt_saturation_report"] = _sat
        if _sat['n_saturated_responses']:
            _by = ", ".join(f"{c}={n}" for c, n in _sat['by_channel'].items() if n)
            st.warning(
                f"⚠️ Vₜ raw saturation: {_sat['n_saturated_responses']}/{_sat['n_scored']} "
                f"responses had a raw channel exceed 1.0 before clamping ({_by}). "
                f"V_t is clamped and valid; vraw_* columns retain the unclamped values "
                f"for calibration of the normalization constants."
            )
        else:
            st.success(
                f"✓ Vₜ raw saturation: 0/{_sat['n_scored']} responses exceeded 1.0 "
                f"before clamping — normalization constants are not saturating."
            )
        st.rerun()
