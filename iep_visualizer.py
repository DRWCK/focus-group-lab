...
# Make sure syniq_core resolves to the *patched* local copy (v1.1.0+),
# not an older copy that might be sitting in /mnt/project. We put the
# script's own directory first, and explicitly drop any syniq_core that
# was already imported elsewhere.
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
for path in [SCRIPT_DIR, os.path.dirname(SCRIPT_DIR), '/home/claude']:
    if path in sys.path:
        sys.path.remove(path)
    sys.path.insert(0, path)
# Force re-import in case something already loaded an older copy
for _k in list(sys.modules.keys()):
    if _k == 'syniq_core' or _k.startswith('syniq_core.'):
        del sys.modules[_k]
...
from syniq_core import (
    ...
    score_iep,
    IEP_DEFAULT_WEIGHTS, IEP_CASCADE_WEIGHTS_V1,
    CORE_VERSION,
)
...
V48_PATH = '/mnt/user-data/uploads/mapper_all_20260302_193147_V48.csv'
V51_PATH = '/mnt/user-data/uploads/mapper_all_20260504_204740_V51.csv'
...
@st.cache_data
def load_grid(path):
    """Load mapper grid CSV. Returns None if not present."""
...
def score_both(text):
    ...
    word = score_iep(text, weights=IEP_DEFAULT_WEIGHTS)
...
def view_side_by_side(grid_v51):
    ...
    if grid_v51 is None:
        st.error(f"V51 mapper grid not found at {V51_PATH}")
        return
    ...
    for col, row, cond in [(col_l, left_row, cond_left), (col_r, right_row, cond_right)]:
        with col:
            ...
            score_word = score_iep(text, weights=IEP_DEFAULT_WEIGHTS)
        ...
        with st.spinner("Scanning grid for dominance flips..."):
            ...
            for _, r in grid_v51.iterrows():
                ...
                if not isinstance(t, str) or len(t) < 50:
                    continue
                w = score_iep(t, weights=IEP_DEFAULT_WEIGHTS)
