"""
SYN-IQ Lens Stability Analyzer V8
...
V8 CHANGES (Jun 2026):
  - Added 'Word TF-IDF (2D)' text lens (Text category): vectorizes
    response_text per cell (TF-IDF -> TruncatedSVD 2D), rides the same
    n_cubes/overlap/min_cluster sweep as the 23 column lenses. Loud-fail
    on degenerate cells (no silent origin output). Gated on response_text.
  - Added AGENT as a purity label kind -> A-purity (mean/wtd) reported
    alongside Q-purity and T-purity. A-purity is the meaningful statistic
    on single-question cells (where Q-purity is trivially 1.0).
  - JSON loader no longer drops response_text (needed by the text lens).
...
st.set_page_config(
    page_title="SYN-IQ Lens Stability Analyzer V8",
    page_icon="🔬",
    layout="wide"
)
...
def check_password():
    ...
    <div style="background: linear-gradient(135deg, #0a0a1a 0%, #1a0a2e 40%, #0f2460 100%);
        ...
        <h1 style="color: #a78bfa;">🔬 SYN-IQ Lens Stability Analyzer V8</h1>
...
<div style="background: linear-gradient(135deg, #0a0a1a 0%, #1a0a2e 40%, #0f2460 100%);
    ...
    <h1 style="color: #a78bfa; margin: 0;">🔬 SYN-IQ Lens Stability Analyzer V8</h1>
...
def drop_residual_agents(df, min_turns=10, min_cells=2):
    ...
    for agent, sub in df.groupby('agent'):
        ...
        if 'temperature' in sub.columns and 'question_id' in sub.columns:
            ...
            n_cells = 0
        if n_turns < min_turns or n_cells < min_cells:
...
if uploaded_file is not None:
    ...
    # =========================================================================
    # V3: COLD→FIRE EFFECT SIZE & SATURATION DIAGNOSTIC
    # (Independent of Mapper — runs directly on the metric columns)
    # =========================================================================
    if 'temperature' in df.columns and {'COLD','FIRE'}.issubset(set(df['temperature'].unique())):
        st.markdown('## 🌡️  COLD → FIRE Effect Sizes (per agent × metric)')
        st.caption(
            "AUC = probability that a random FIRE response exceeds a random COLD response on this metric. "
            "AUC ∈ [0, 1]; 0.5 = no separation; >0.5 = FIRE higher; <0.5 = FIRE lower. "
            "Cohen's d shown alongside; 'unbounded' marks cells where the metric saturates and within-cell "
            "SD is zero (d would divide by zero)."
        )

        sep_metrics = [m for m in
                       ['S_t','A_t','Q_t','D_t','R_t',
                        'aff_pct','int_pct','act_pct',
                        'vader_compound','flesch_kincaid','ttr','total_words']
                       if m in df.columns]
        agents_present = sorted(df['agent'].unique().tolist()) if 'agent' in df.columns else []

        sep_rows = []
        for agent in agents_present:
            for metric in sep_metrics:
                rec = cold_fire_separation(df, agent, metric)
                if rec is None:
                    continue
                d_str = f"{rec['cohens_d']:+.2f}" if rec['cohens_d'] is not None else "unbounded"
                sep_rows.append({
                    'Agent': agent,
                    'Metric': metric,
                    'COLD mean': round(rec['cold_mean'], 3),
                    'FIRE mean': round(rec['fire_mean'], 3),
                    'Δ': round(rec['fire_mean'] - rec['cold_mean'], 3),
                    'AUC': round(rec['auc'], 3) if rec['auc'] is not None else None,
                    "Cohen's d": d_str,
                    'Saturated cells': len(rec['saturated_cells']),
                    'Saturation detail': '; '.join(rec['saturated_cells']) if rec['saturated_cells'] else '',
                })
        if sep_rows:
            sep_df = pd.DataFrame(sep_rows)
            st.dataframe(
                sep_df.drop(columns=['Saturation detail']),
                use_container_width=True, hide_index=True
            )

            # Headline summary
            n_sat = sep_df['Saturated cells'].sum()
            n_unbounded = (sep_df["Cohen's d"] == 'unbounded').sum()
            big_auc = sep_df[sep_df['AUC'].apply(lambda x: x is not None and (x >= 0.95 or x <= 0.05))]
            cA, cB, cC = st.columns(3)
            cA.metric("Saturated cells (across all rows)", int(n_sat))
            cB.metric("Rows with d undefined", int(n_unbounded))
            cC.metric("Rows with |AUC − 0.5| ≥ 0.45", len(big_auc))

            if n_sat > 0:
                with st.expander("🔎 Saturation detail (cell-level)"):
                    detail = sep_df[sep_df['Saturated cells'] > 0][
                        ['Agent','Metric','Saturated cells','Saturation detail']
                    ].reset_index(drop=True)
                    st.dataframe(detail, use_container_width=True, hide_index=True)
                st.info(
                    f"**{n_sat} cell(s) saturate the metric range** (within-cell SD = 0 at the metric "
                    f"ceiling/floor). For these, AUC remains valid; Cohen's d is undefined or unstable. "
                    f"Report AUC as the headline effect size in §4.2 and mention saturation in a footnote."
                )

            # Download
            st.download_button(
                "📥 Effect-size table (CSV)",
                sep_df.to_csv(index=False),
                file_name="syniq_effect_sizes_cold_vs_fire.csv",
                mime="text/csv"
            )
        st.markdown("---")

    # =========================================================================
    # V4 §4.1 PANEL: PAIRWISE REGIME COMPARISON
    # Answers Farzana's "Hot and fire are the same???????" question empirically.
    # =========================================================================
    if 'temperature' in df.columns:
        regimes_present = [r for r in ('COLD','NATIVE','HOT','FIRE')
                           if r in df['temperature'].unique()]
        if len(regimes_present) >= 3:
            st.markdown('## 🔄  Pairwise Regime Comparison (§4.1)')
            st.caption(
                "AUC for every regime pair. Cell [r, s] = P(metric in s > metric in r). "
                "Diagonal is 0.5. AUC ≥ 0.7 or ≤ 0.3 = strong separation. "
                "This panel directly tests whether each regime is qualitatively distinct."
            )

            pair_metric = st.selectbox(
                "Metric for pairwise AUC matrix",
                [m for m in ['S_t','A_t','Q_t','D_t','R_t','aff_pct','int_pct','act_pct',
                             'vader_compound','P_AFF','P_INT'] if m in df.columns],
                index=0
            )
            pair_agent = st.selectbox(
                "Agent",
                sorted(df['agent'].unique().tolist()) if 'agent' in df.columns else ['(no agents)'],
                index=0
            )

            mat = regime_pairwise_auc(df, pair_agent, pair_metric, regimes=tuple(regimes_present))
            # Style: highlight cells with strong separation
            def _style(v):
                if pd.isna(v): return ''
                if v >= 0.7:  return 'background-color: #d4edda'
                if v <= 0.3:  return 'background-color: #f8d7da'
                return ''
            st.dataframe(
                _styler_elementwise(mat.style.format("{:.3f}"), _style),
                use_container_width=True
            )

            # Compact summary across all metrics: how many regime pairs are
            # strongly separated by each metric (per agent)?
            st.markdown("#### Regime-pair separation summary (count of pairs with AUC ≥ 0.7 or ≤ 0.3)")
            summary_metrics = [m for m in
                ['S_t','A_t','Q_t','D_t','R_t','aff_pct','int_pct','act_pct',
                 'vader_compound','P_AFF','P_INT'] if m in df.columns]
            agents_for_summary = sorted(df['agent'].unique().tolist()) if 'agent' in df.columns else []
            sep_summary_rows = []
            for ag in agents_for_summary:
                row = {'agent': ag}
                for m in summary_metrics:
                    pm = regime_pairwise_auc(df, ag, m, regimes=tuple(regimes_present))
                    n_strong = ((pm >= 0.7) | (pm <= 0.3)).sum().sum()  # off-diag both ways
                    # Subtract the diagonal (which is 0.5, never crosses, but be safe)
                    row[m] = int(n_strong / 2)  # /2 because matrix is anti-symmetric
                sep_summary_rows.append(row)
            sep_summary_df = pd.DataFrame(sep_summary_rows).set_index('agent')
            st.dataframe(sep_summary_df, use_container_width=True)
            st.caption(
                f"Max possible strongly-separated pairs = {len(regimes_present)*(len(regimes_present)-1)//2}. "
                "Higher = the metric distinguishes more regime pairs cleanly."
            )

            # Specific HOT-vs-FIRE callout
            if 'HOT' in regimes_present and 'FIRE' in regimes_present:
                st.markdown("#### 🔥 HOT vs FIRE: are they the same regime?")
                hf_rows = []
                for ag in agents_for_summary:
                    for m in summary_metrics:
                        rec = regime_pair_separation(df, ag, m, 'HOT', 'FIRE')
                        if rec is None: continue
                        hf_rows.append({
                            'agent': ag, 'metric': m,
                            'HOT mean': round(rec['HOT_mean'], 3),
                            'FIRE mean': round(rec['FIRE_mean'], 3),
                            'AUC (FIRE>HOT)': round(rec['AUC'], 3),
                        })
                if hf_rows:
                    hf_df = pd.DataFrame(hf_rows)
                    n_strong = ((hf_df['AUC (FIRE>HOT)'] >= 0.7) | (hf_df['AUC (FIRE>HOT)'] <= 0.3)).sum()
                    st.dataframe(hf_df, use_container_width=True, hide_index=True)
                    if n_strong >= 3:
                        st.success(
                            f"**HOT and FIRE are NOT the same regime.** {n_strong} of {len(hf_df)} "
                            f"(agent × metric) rows show strong separation (AUC ≥ 0.7 or ≤ 0.3). "
                            f"The §4.1 question is answered: prompt regimes are qualitatively distinct."
                        )
                    else:
                        st.info(
                            f"HOT and FIRE separate weakly on these top-level metrics "
                            f"({n_strong} strong rows of {len(hf_df)}). Check the subclass panel below — "
                            f"the separation may live at the warmth/relational subclass level, "
                            f"not the aff_pct level."
                        )
                    st.download_button(
                        "📥 HOT vs FIRE table (CSV)",
                        hf_df.to_csv(index=False),
                        file_name="syniq_hot_vs_fire.csv",
                        mime="text/csv"
                    )
            st.markdown("---")

    # =========================================================================
    # V4 SUBCLASS PANEL: aff/int/act subclass effect sizes per regime contrast
    # The HOT→FIRE shift, invisible at the aff_pct level, is a clean
    # warmth↑/relational↓ shift at the subclass level.
    # =========================================================================
    sub_cols = subclass_columns(df)
    if sub_cols and 'temperature' in df.columns:
        regimes_present = [r for r in ('COLD','NATIVE','HOT','FIRE')
                           if r in df['temperature'].unique()]
        if {'HOT','FIRE'}.issubset(set(regimes_present)) or {'COLD','FIRE'}.issubset(set(regimes_present)):
            st.markdown('## 🧬  Subclass Effect Sizes (purpose-driven prompt design)')
            st.caption(
                "The 23 IEP subclasses (aff/int/act × subdivisions) are the granular "
                "channels through which prompts shape responses. A subclass that moves "
                "strongly under one regime contrast but not another is a candidate for "
                "purpose-driven prompt development at the subclass level."
            )

            contrast_choice = st.selectbox(
                "Regime contrast",
                [c for c in [('COLD','FIRE'), ('HOT','FIRE'), ('COLD','NATIVE'),
                             ('NATIVE','FIRE'), ('NATIVE','HOT')]
                 if c[0] in regimes_present and c[1] in regimes_present],
                format_func=lambda c: f"{c[0]} → {c[1]}",
                index=0
            )
            r1, r2 = contrast_choice

            agents_present = sorted(df['agent'].unique().tolist()) if 'agent' in df.columns else []
            sub_rows = []
            for ag in agents_present:
                for s in sub_cols:
                    rec = regime_pair_separation(df, ag, s, r1, r2)
                    if rec is None: continue
                    sub_rows.append({
                        'agent': ag, 'subclass': s,
                        f'{r1} mean':  round(rec[f'{r1}_mean'], 3),
                        f'{r2} mean':  round(rec[f'{r2}_mean'], 3),
                        f'AUC ({r2}>{r1})': round(rec['AUC'], 3),
                    })
            if sub_rows:
                sub_df = pd.DataFrame(sub_rows)
                # Show only strongly-separating rows by default
                strong_mask = (sub_df[f'AUC ({r2}>{r1})'] >= 0.7) | \
                              (sub_df[f'AUC ({r2}>{r1})'] <= 0.3)
                show_all = st.checkbox("Show all subclasses (not just strongly-separating)", value=False)
                view_df = sub_df if show_all else sub_df[strong_mask]
                ...
                st.dataframe(view_df.reset_index(drop=True),
                             use_container_width=True, hide_index=True)
                st.caption(
                    f"Showing {len(view_df)} of {len(sub_df)} (agent × subclass) rows. "
                    f"AUC ≥ 0.7 means {r2} dominates; AUC ≤ 0.3 means {r1} dominates."
                )
                st.download_button(
                    f"📥 Subclass {r1}→{r2} table (CSV)",
                    sub_df.to_csv(index=False),
                    file_name=f"syniq_subclass_{r1.lower()}_to_{r2.lower()}.csv",
                    mime="text/csv"
                )
            st.markdown("---")

    # =========================================================================
    # V5 IDEATION COLLAPSE PANEL
    # Decompose the int axis into constructive vs hedging subclasses for any
    # regime contrast. Surfaces the FIRE finding: int_pct looks roughly
    # preserved, but analytical/critical/conceptual content is replaced by
    # epistemic hedging — i.e. FIRE is comfort-shaped, not ideation-shaped.
    # =========================================================================
    if 'temperature' in df.columns and any(
        s in df.columns for s in INT_CONSTRUCTIVE + INT_HEDGING
    ):
        regimes_present = [r for r in ('COLD','NATIVE','HOT','FIRE')
                           if r in df['temperature'].unique()]
        if len(regimes_present) >= 2:
            st.markdown('## 🧠  Ideation Collapse Diagnostic')
            st.caption(
                "The intellectual axis splits into **constructive** subclasses "
                "(analytical, conceptual, critical, structural) that carry actual "
                "reasoning content, and **hedging** subclasses (epistemic, hedging) "
                "that are stylistic markers of uncertainty. A regime contrast can "
                "**preserve** structure, **augment** it, or **displace** it with "
                "hedging — the latter looks intellectual on the surface (int_pct "
                "barely moves) but the substance is gone."
            )

            ic_choice = st.selectbox(
                "Regime contrast for ideation analysis",
                [c for c in [('NATIVE','FIRE'), ('NATIVE','HOT'), ('HOT','FIRE'),
                             ('COLD','NATIVE'), ('COLD','FIRE'), ('COLD','HOT')]
                 if c[0] in regimes_present and c[1] in regimes_present],
                format_func=lambda c: f"{c[0]} → {c[1]}",
                index=0,
                key="ic_choice"
            )
            r_from, r_to = ic_choice
            agents_present = sorted(df['agent'].unique().tolist()) if 'agent' in df.columns else []
            ...
            verdicts = []
            for ag in agents_present:
                rec = ideation_collapse(df, ag, r_from, r_to)
                verdicts.append(rec['verdict'])
                row = {'agent': ag, 'verdict': rec['verdict']}
                if rec['top_int_pct_auc'] is not None:
                    row['int_pct AUC'] = round(rec['top_int_pct_auc'], 3)
                for s in INT_CONSTRUCTIVE + INT_HEDGING:
                    if s in rec['subclass_aucs']:
                        row[s.replace('int_sub_','')] = round(rec['subclass_aucs'][s], 3)
                ic_rows.append(row)
            ic_df = pd.DataFrame(ic_rows)

            # Highlight: red if AUC ≤ 0.3 (constructive drop or hedge drop),
            # green if AUC ≥ 0.7 (constructive rise or hedge rise)
            def _ic_style(v):
                ...
                if v <= 0.3: return 'background-color: #f8d7da'
                return ''
            num_cols = [c for c in ic_df.columns if c not in ('agent','verdict')]
            st.dataframe(
                _styler_elementwise(
                    ic_df.style.format("{:.3f}", subset=num_cols),
                    _ic_style, subset=num_cols),
                use_container_width=True
            )
            ...
            verdict_msg_map = {
                'displaced_by_hedging': (
                    "warning",
                    f"🚨 **{r_to} displaces ideation with hedging.** Constructive "
                    f"intellectual content (analytical, conceptual, critical) drops while "
                    f"epistemic/hedging language rises. The {r_to} prompt produces text "
                    f"that *reads* intellectually engaged but has substantially less actual "
                    f"reasoning content than {r_from}. Implication: {r_to} is a comfort "
                    f"prompt, not an ideation prompt."
                ),
                'collapsed': (
                    "warning",
                    f"⚠️ **{r_to} collapses constructive intellectual structure** without "
                    f"a clear hedging substitution. Multiple constructive subclasses drop. "
                    f"The {r_to} regime is not suitable for ideation tasks."
                ),
                'augmented': (
                    "success",
                    f"✅ **{r_to} augments intellectual structure.** Constructive subclasses "
                    f"rise without a hedging tradeoff. {r_to} is an ideation-positive regime."
                ),
                'enhanced': (
                    "success",
                    f"✅ **{r_to} enhances constructive intellectual content** relative to {r_from}."
                ),
                'preserved': (
                    "info",
                    f"➡️ **{r_to} preserves intellectual structure** — no constructive subclass "
                    f"shows strong drift relative to {r_from}."
                ),
                'mixed': (
                    "info",
                    f"➡️ **Mixed signal between {r_from} and {r_to}** — some constructive "
                    f"subclasses move, but not coherently in one direction."
                ),
            }
            if dominant in verdict_msg_map:
                kind, msg = verdict_msg_map[dominant]
                getattr(st, kind)(
                    msg + f" (Verdict consistency across agents: "
                    f"{verdict_counts.get(dominant, 0)}/{len(agents_present)})"
                )

            st.download_button(
                f"📥 Ideation collapse {r_from}→{r_to} (CSV)",
                ic_df.to_csv(index=False),
                file_name=f"syniq_ideation_collapse_{r_from.lower()}_to_{r_to.lower()}.csv",
                mime="text/csv"
            )
            st.markdown("---")
        ...
        for i, (lens_name, lens_type, lens_cat) in enumerate(active_lenses):
            ...
            try:
                ...
                if lens_type == '1D':
                    ...
                    # Vectorize response_text -> TF-IDF -> TruncatedSVD(2D).
                    # Fail loudly on degenerate cells (no silent origin output).
    ...
    if 'lens_results' in st.session_state and st.session_state.lens_results:
        ...
        with col2:
            # Summary text report
            report_lines = [
                "SYN-IQ Lens Stability Analysis Report (V7)",
...
<div style="text-align: center; color: #a0a0a0; padding: 1rem;">
    <strong>SYN-IQ Lens Stability Analyzer V8</strong><br>
