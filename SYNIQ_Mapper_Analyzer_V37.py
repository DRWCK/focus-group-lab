"""
SYN-IQ Mapper Analyzer V37 — Streamlit App
Topological Data Analysis for AI Response Profiles

V37 (new in this version)
  * RUN-LEVEL METRICS LOG. Every graph build appends one row to a
    ...
    produces a pasteable table directly instead of a set of screenshots
    to be transcribed by hand. Append-only; header written once.
    Columns: log meta (run_id, tool_version, log_schema), run identity
    (timestamp, question_id, feature_set, source_csv), parameters
    (n_cubes, perc_overlap, dbscan_eps, dbscan_min_samples, projection,
    scaler, nerve_min_intersection), graph structure (nodes, edges,
  ...
  * SINGLE SOURCE OF TRUTH FOR THE VERSION STRING. TOOL_VERSION is defined
    once, immediately below this docstring. The page title, both H1
    headers, the footer caption, figure titles and the run log all read
    from it. V36 shipped with its filename reading V36, its docstring
    header V34 and its page title V29 — three disagreeing stamps in one
    file, which means no output can be traced to the code that produced
    it. This is the second occurrence; see the V28 note below recording
    that V27 shipped with docstring, page title, both H1 headers and the
    footer all still reading V25.
...
def log_mapper_run(graph, analysis, subset, n_input_rows, **kw):
    ...
    """
    try:
        row = build_runlog_row(graph, analysis, subset, n_input_rows, **kw)
