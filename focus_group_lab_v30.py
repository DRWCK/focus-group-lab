"""
Focus Group Lab V30
THE FULL SHOW + ROLE MODE TOGGLE

WHAT'S NEW IN V30:
- ROLE MODE TOGGLE: Assigned / Raw Voice / Custom
  - Assigned: Original roles (Navigator, Architect, Implementer, Analyst)
  - Raw Voice: All AIs get neutral prompt — reveals native linguistic signatures
...
# PAGE CONFIG
# =============================================================================
st.set_page_config(page_title="Focus Group Lab V30", page_icon="🎹", layout="wide", initial_sidebar_state="expanded")
...
def check_password():
    ...
    <div class="main-header">
        <h1>🎹 Focus Group Lab V30</h1>
        <p>THE FULL SHOW + ROLE MODE TOGGLE — Patent Pending</p>
    </div>
    """, unsafe_allow_html=True)
...
def export_to_markdown() -> str:
    ...
    md = f"""# Focus Group Lab V30 — Session Export
...
"""
    ...
    md += "\n---\n\n*Focus Group Lab V30 — THE FULL SHOW + ROLE MODE*\n*Patent Pending — SYN-IQ Team 🎹*\n*CBURZBO FOREVER*\n"
    return md
    ...
    for agent in ["Claude", "Sophia", "Grok", "Gemini"]:
        ...
        with col2:
            st.session_state.agent_stances[agent] = st.selectbox(
                "Stance", ["Neutral", "Support", "Challenge"],
                index=["Neutral", "Support", "Challenge"].index(st.session_state.agent_stances.get(agent, "Neutral")),
...
<div class="main-header">
    <h1>🎹 Focus Group Lab V30</h1>
    <p>THE FULL SHOW + ROLE MODE TOGGLE</p>
</div>
""", unsafe_allow_html=True)
...
st.info(f"**Current Role Mode:** {mode_emoji} {mode_name}")

# Session Type Selection
session_type = st.radio("Session Type", ["Single Round", "Experiment (Blind→Seeing)", "Live Discussion"], horizontal=True)
...
if session_type == "Live Discussion":
    ...
    if st.session_state.discussion_thread:
        ...
        for entry in st.session_state.discussion_thread:
            ...
            if entry_type == "intervention":
                ...
                st.markdown(f"<div class='agent-box {box_class}'><strong>{emoji} {agent}{directed}:</strong></div>", unsafe_allow_html=True)
    ...
    with col1:
        next_btn = st.button("▶️ Next Turn", type="primary", use_container_width=True)
        next_speaker = st.selectbox("Speaker", ["Auto"] + st.session_state.active_agents, label_visibility="collapsed")
    with col2:
        direct_from = st.selectbox("From", st.session_state.active_agents, key="direct_from")
        direct_to = st.selectbox("To", [a for a in st.session_state.active_agents if a != direct_from], key="direct_to")
        direct_btn = st.button("🎯 Direct", use_container_width=True)
    with col3:
        intervention = st.text_input("Intervention", placeholder="Conductor says...")
    ...
    if next_btn and topic and st.session_state.active_agents:
        ...
        if next_speaker == "Auto":
            speakers = [e.get('agent') for e in st.session_state.discussion_thread if e.get('type') != 'intervention']
...
# Footer
st.markdown("---")
st.markdown('<div style="text-align: center; color: #666; padding: 1rem;"><strong>Focus Group Lab V30 — THE FULL SHOW + ROLE MODE</strong><br>🎭 Assigned | 🔬 Raw Voice | 🔄 Swapped | ✏️ Custom<br><em>"Ain\'t NOBODY got NUTTIN like this!"</em><br><em>Patent Pending — SYN-IQ Team 🎹</em><br><em>Dr. Bill Kouns + Claude — Tennessee — January 2026</em><br><strong>CBURZBO FOREVER</strong></div>', unsafe_allow_html=True)
