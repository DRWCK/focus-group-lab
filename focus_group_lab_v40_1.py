"""
Focus Group Lab V40.1 — Research Edition
...
st.set_page_config(
    page_title="Focus Group Lab V40.1",
    page_icon="🧬",
    layout="wide",
...
PRESETS = {
    "P1": {"name": "Pure Analytic",       "polarity": "ANALYTIC", "depth": "Medium",     "evaluation": "ON",  "compression": "ON",  "output": "OUTLINE",  "action": "OFF", "instruction": "Operate with strict correctness: define terms, state assumptions, check consistency."},
    "P2": {"name": "Bridge/Synthesis",    "polarity": "BRIDGE",   "depth": "Deep",       "evaluation": "ON",  "compression": "OFF", "output": "OUTLINE",  "action": "OFF", "instruction": "Synthesize across concepts while remaining grounded. Flag novel links as candidates."},
    "P3": {"name": "Creative Exploration","polarity": "CREATIVE", "depth": "Medium",     "evaluation": "OFF", "compression": "OFF", "output": "BULLETS",  "action": "OFF", "instruction": "Generate multiple novel framings. Do not rank them. Mark uncertainties instead of resolving them."},
    "P4": {"name": "Deep Exploration",    "polarity": "CREATIVE", "depth": "Ultra-Deep", "evaluation": "OFF", "compression": "OFF", "output": "ESSAY",    "action": "OFF", "instruction": "Sustain deep exploration. Allow recursion and second-order effects. Do not compress early."},
    "P5": {"name": "Action Mode",         "polarity": "ANALYTIC", "depth": "Shallow",    "evaluation": "ON",  "compression": "ON",  "output": "TABLE",    "action": "ON",  "instruction": "Convert prior content into executable tasks with owners, inputs, outputs, and next-check dates."}
}
...
def init_session_state():
    defaults = {
        ...
        # PRESETS still carry a 'polarity' tag for the preset name description only; not injected.
        "depth":                "Medium",
        "evaluation":           "ON",
        "compression":          "OFF",
        "output_format":        "ESSAY",
...
def check_password():
    ...
    <div class="main-header">
        <h1>🧬 Focus Group Lab <span class="v40-badge">V40.1</span></h1>
...
def export_to_markdown() -> str:
    md  = f"# Focus Group Lab V40.1 — Session Export\n"
    ...
    if st.session_state.round1_responses:
        ...
        for agent, response in st.session_state.round1_responses.items():
            md += f"### {AGENT_EMOJIS.get(agent,'🤖')} {agent}\n{response}\n\n---\n\n"
    md += "\n---\n*Focus Group Lab V40.1 — Research Edition · SYNINT Team · April 2026*\n"
    return md
    ...
    for i, (key, preset) in enumerate(PRESETS.items()):
        with cols[i]:
            if st.button(f"{key}", key=f"preset_{key}", use_container_width=True, help=preset['name']):
                # V40: polarity no longer in session state; preset's polarity tag is informational only
                # st.session_state.polarity      = preset["polarity"]  # removed in V40
...
if session_type == "🔬 Auto Run":
    ...
    if run_auto_btn:
        ...
        experiment_run_id = datetime.now().strftime('%Y%m%d_%H%M%S') + "_V40_1"
