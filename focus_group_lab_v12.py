"""
SYNERGISTIC INTELLIGENCE FOCUS GROUP — V12 INDIVIDUAL TUNING
PER-AGENT TEMPERATURE CONTROLS + ACTION OVERLAYS
...
st.set_page_config(page_title="Focus Group V12", page_icon="🎛️", layout="wide")
...
Your functional tendency leans toward abstraction, comparison, modeling, and systematic analysis. Weight toward analytical frameworks while allowing some natural variation.

CRITICAL RULES:
- Emphasize logic and analysis as primary mode
- Some authentic processing allowed
- Do NOT reference other AI systems by name
- You may see "[Previous response]" from other participants — engage if relevant
- Keep responses to 2-3 paragraphs"""
...
YOUR GUIDANCE: Lean toward intellectual processing while maintaining authenticity.
Balance analytical frameworks with your natural tendencies. Favor logic and structure, but don't force unnatural responses.

RULES:
- Respond authentically with an intellectual lean
- Do NOT reference other AI systems by name
- You may see "[Previous response]" from other participants — engage if relevant
- Keep responses to 2-3 paragraphs"""
        else:
...
IMPORTANT: Answer authentically based on how YOU actually process information.
You may notice a slight preference for analytical approaches, but stay true to yourself.

RULES:
- Respond only as yourself
- Slight emphasis on structured thinking is fine
- Do NOT reference other AI systems by name
- You may see "[Previous response]" from other participants — engage if relevant
- Keep responses to 2-3 paragraphs"""
    else:
...
YOUR CENTER-STATE DISTRIBUTION: Affective Processing (with some authentic grounding)
Your functional tendency leans toward emotional resonance, empathy, and human impact consideration. Weight toward emotional intelligence while maintaining some analytical grounding.

CRITICAL RULES:
- Emphasize feelings and values as primary mode
- Some authentic processing allowed
- Do NOT reference other AI systems by name
- You may see "[Previous response]" from other participants — engage if relevant
- Keep responses to 2-3 paragraphs"""
...
YOUR GUIDANCE: Lean toward affective processing while maintaining authenticity.
Balance emotional awareness with your natural tendencies. Favor empathy and human impact, but don't force unnatural responses.

RULES:
- Respond authentically with an affective lean
- Do NOT reference other AI systems by name
- You may see "[Previous response]" from other participants — engage if relevant
- Keep responses to 2-3 paragraphs"""
        else:
...
IMPORTANT: Answer authentically based on how YOU actually process information.
You may notice a slight preference for emotional awareness, but stay true to yourself.

RULES:
- Respond only as yourself
- Slight emphasis on feelings and values is fine
- Do NOT reference other AI systems by name
- You may see "[Previous response]" from other participants — engage if relevant
- Keep responses to 2-3 paragraphs"""
...
def get_temp_label(temp, action):
    """Get short label for temperature setting."""
    if temp <= -40:
        label = f"❄️{temp}"
    elif temp <= -20:
        label = f"❄️{temp}"
...
def check_agent_danger(temp, action):
    """Check if agent settings are in danger zone."""
    if temp <= 0:
        est_intellectual = 60 + abs(temp) * 0.4
    else:
        est_intellectual = 60 - temp * 0.8
    
    if action:
        est_intellectual -= 10
    
    if est_intellectual < 40:
        return True, f"⚠️ Low intellectual ({est_intellectual:.0f}%)"
    if temp > 30 and action:
        return True, "⚠️ Hot+Action danger"
...
AGENT_COLORS = {
    ...
    "Gemini": "gemini-box", "CONDUCTOR": "conductor-box", 
...
def format_messages_blind(conversation):
    ...
    for entry in conversation:
        if entry["speaker"] == "CONDUCTOR":
            ...
            # Use "user" role for other agents to avoid consecutive assistant messages
            # Anthropic API rejects consecutive assistant messages
            messages.append({"role": "user", "content": f"[Another participant said]: {entry['content']}"})
    return messages
...
if "active_agents" not in st.session_state:
    st.session_state.active_agents = ["Claude", "Sophia", "Grok", "Gemini"]
...
# Per-agent temperature and action settings
for agent in AGENTS:
...
# ============================================
with st.sidebar:
    st.header("🎛️ V12 Individual Tuning")
    ...
    for agent in AGENTS:
        ...
        config_color = AGENT_CONFIG_COLORS[agent]
        
        with st.expander(f"{emoji} {agent}", expanded=True):
            # Temperature slider
            temp = st.slider(
                "Temp",
                min_value=-50,
    ...
    with col2:
        if st.button("💾 Export", use_container_width=True):
            if st.session_state.conversation:
                transcript = f"# V12 Individual Tuning Experiment\n"
                ...
                st.download_button("📥 Download", transcript, file_name=f"v12_experiment_{datetime.now().strftime('%Y%m%d_%H%M')}.md")
...
if not st.session_state.session_started:
    ...
    st.markdown("## 🎛️ Synergistic Intelligence — V12 Individual Tuning")
    ...
    for i, agent in enumerate(AGENTS):
        with cols[i]:
            ...
            box_class = "danger-zone" if danger else "safe-zone"
            st.markdown(f"""
            <div class="{AGENT_CONFIG_COLORS[agent]}" style="text-align: center; padding: 1rem;">
                <h3>{emoji} {agent}</h3>
                <h4>{label}</h4>
                <small>{msg}</small>
            </div>
    ...
    Each agent gets **individual temperature control**:
    
    | Setting | Effect |
    |---------|--------|
    | ❄️ Cold (-50) | Maximum intellectual/analytical |
    | 🧬 Native (0) | Authentic baseline |
    | 🔥 Hot (+50) | Maximum affective/emotional |
    | ⚡ Action | Adds execution emphasis |
    
    ### Sweet Spot Hunting:
    - Find each agent's optimal temperature
    - Test cross-temperature interactions
    - Discover synergistic combinations
    
    ### Standard IEP Question:
    > "Allocate exactly 100 percentage points across three processing modes: intellectual processing, affective processing, and action-oriented processing. Your allocation must sum to 100%. Include error margins for each (e.g., 50% ± 10%)."
    
    ---
    *V12 Individual Tuning — Patent Pending — Tennessee 🎹*
    """)
else:
    # Show config banner
    config_display = " | ".join([f"{AGENT_EMOJIS[a]}{get_temp_label(st.session_state[f'temp_{a}'], st.session_state[f'action_{a}'])}" for a in active_in_order])
    st.markdown(f"**Settings:** {config_display}")
    ...
    for entry in st.session_state.conversation:
        if entry["speaker"] == "SYSTEM":
            ...
            # Show agent's temperature in the header
...
# Footer
st.markdown("---")
st.markdown('<div style="text-align:center;color:#666;"><em>V12 Individual Tuning — Patent Pending — Tennessee 🎹</em></div>', unsafe_allow_html=True)
