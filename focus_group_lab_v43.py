"""
Focus Group Lab V43 — Research Edition
...
st.set_page_config(
    page_title="Focus Group Lab V43",
    page_icon="🧬",
    layout="wide",
        ...
        r = requests.post(
            ...
            json={"systemInstruction": {"parts": [{"text": system}]},
                  ...
                  "generationConfig": {"maxOutputTokens": max_tokens,
                                       **_gemini_thinking_cfg()}}, timeout=120)
        if r.status_code == 200:
...
def export_to_markdown() -> str:
    md  = f"# Focus Group Lab V43 — Session Export\n"
    ...
    if st.session_state.round1_responses:
        ...
        for agent, response in st.session_state.round1_responses.items():
            md += f"### {AGENT_EMOJIS.get(agent,'🤖')} {agent}\n{response}\n\n---\n\n"
    md += "\n---\n*Focus Group Lab V43 — Research Edition · SYNINT Team · April 2026*\n"
    return md
...
if session_type == "🔬 Auto Run":
    ...
    if run_auto_btn:
        ...
        experiment_run_id = datetime.now().strftime('%Y%m%d_%H%M%S') + "_V43"
