"""
SYNERGISTIC INTELLIGENCE FOCUS GROUP — V23
...
st.set_page_config(page_title="Focus Group V23", page_icon="🎯", layout="wide")
...
def calculate_syniq_quick(responses, synthesis):
    ...
    # V23 FIX: Single measure, no double-counting
...
def calculate_syniq_full(responses, synthesis, api_key, solved_status="Partial"):
    """
    V23 FULL SYN-IQ calculation with EPM (requires OpenAI API).
...
# ============================================
    ...
    st.markdown('<div class="boardroom-header"><h2>🏛️ BOARDROOM V23</h2><p>Multi-Round Discussion + EPM Scoring</p></div>', unsafe_allow_html=True)
            ...
            if not document_input and not uploaded_image:
                ...
                for round_idx in range(num_rounds):
                    for agent in active_agents:
                        ...
                        # V23: Adjust prompt for subsequent rounds
                        if round_idx == 0:
    ...
    if st.session_state.boardroom_history:
        ...
        # V23: Full SYN-IQ with EPM
        if auto_analyze:
            st.markdown("---")
            st.markdown("### 🔬 SYN-IQ Analysis (V23 EPM)")
...
<div style="text-align: center; color: #666;">
    <em>Focus Group Lab V23 — EPM Scoring Fix + Multi-Round Boardroom</em><br>
