"""
SYN-IQ Mapper Analyzer V21 — Streamlit App
...
# =============================================================================
st.set_page_config(
    page_title="SYN-IQ Mapper Analyzer V21",
    page_icon="🗺️",
    layout="wide"
)
...
def _build_node_membership_map(graph, df, valid_indices):
    """V21: For each node_id, compute agent_counts.

    Returns dict[node_id_str] = Counter[agent_name -> count]
    """
    ...
    for node_id, members in graph["nodes"].items():
        ...
        out[str(node_id)] = dict(agent_counts)
    return out
...
(function() {
  function v21Apply() {
    ...
    paths.each(function(d) {
      ...
      // Pie mode: replace path with a <g> group of wedges
      if (d.v21_wedges && d.v21_wedges.length > 0) {
        var pathNode = this;
        // Get the path's current bounding box to find its center
        var bbox;
        try { bbox = pathNode.getBBox(); }
        catch (e) { return; }
        var cx = bbox.x + bbox.width / 2;
        var cy = bbox.y + bbox.height / 2;
        // Pie radius scaled relative to current path radius
        var origR = Math.max(bbox.width, bbox.height) / 2;
        ...
        g.setAttribute('data-node-id', d.id || '');
        var cur = 0;
        d.v21_wedges.forEach(function(w) {
          var frac = w.frac;
          if (frac <= 0) return;
          var a0 = -Math.PI/2 + cur * 2*Math.PI;
          var a1 = -Math.PI/2 + (cur+frac) * 2*Math.PI;
          var x0 = cx + r*Math.cos(a0), y0 = cy + r*Math.sin(a0);
          var x1 = cx + r*Math.cos(a1), y1 = cy + r*Math.sin(a1);
          var large = (frac > 0.5) ? 1 : 0;
          var dAttr;
          if (frac >= 0.9999) {
            // Whole circle: render as a full closed circle path
            dAttr = 'M ' + (cx-r) + ' ' + cy
                  + ' A ' + r + ' ' + r + ' 0 1 1 ' + (cx+r) + ' ' + cy
        ...
        // Hover: stroke ring on the whole group
        g.addEventListener('mouseenter', function() {
          d3.select(g).selectAll('.v21-wedge')
            .style('stroke', '#1a1a1a').style('stroke-width', '2px');
        });
        g.addEventListener('mouseleave', function() {
          d3.select(g).selectAll('.v21-wedge')
            .style('stroke', '#ffffff').style('stroke-width', '0.8px');
        });
        // Preserve kepler's tooltip click behavior: forward events to the original path
        g.addEventListener('click', function(e) {
          pathNode.dispatchEvent(new MouseEvent('click', e));
        });
        // Replace path with the group
        parent.insertBefore(g, pathNode);
        pathNode.style.display = 'none';
        didAny = true;
      }
    });
    if (!didAny) {
      // path.circle exists but no v21 data — retry once in case D3 hasn't
      // bound data yet
      setTimeout(v21Apply, 400);
    }
  }
  // Multiple kick-offs because kepler's D3 render is asynchronous
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() { setTimeout(v21Apply, 300); });
  } else {
    setTimeout(v21Apply, 300);
  }
  [800, 1500, 3000, 5000].forEach(function(t) { setTimeout(v21Apply, t); });
})();
</script>
"""
...
def inject_v21_node_coloring(
    ...
    """V21: Augment kepler's `const graph = {...}` JSON blob with per-node
    v21_fill (overlap mode) or v21_wedges (pie mode), then inject the
    ...
    mode : str
        "overlap_orange" or "pie_composition".
    purity_threshold : float
    ...
    if mode not in ("overlap_orange", "pie_composition"):
        return html_content
    ...
    for n in gj.get("nodes", []):
        ...
        counts = membership.get(nid)
        if not counts:
            # Try the node "name" too — kmapper sometimes uses one and sometimes the other
            nid_alt = str(n.get("name", ""))
            counts = membership.get(nid_alt)
        if not counts:
            continue
