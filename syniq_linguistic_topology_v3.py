"""
SYN-IQ Linguistic Topology Analyzer V3
...
V3 — FUNCTION word classification · Parallel IEP V4 pipeline · Literature validation
"""
...
<div class="main-header">
    <h1>🔬 SYN-IQ Linguistic Topology Analyzer V3</h1>
...
with tabs[1]:
    ...
    with col1:
        ...
        st.dataframe(fk_q.round(1), use_container_width=True)
    ...
    with col2:
        ...
        st.dataframe(li_q.round(1), use_container_width=True)
...
with tabs[2]:
    ...
    st.dataframe(ld_tab.round(3), use_container_width=True)
    ...
    with col1:
        ...
        st.dataframe(ld_q.round(3), use_container_width=True)
    ...
    with col2:
        ...
        st.dataframe(ae.round(3), use_container_width=True)
...
with tabs[3]:
    ...
    st.dataframe(sf_tab.round(2), use_container_width=True)
    ...
    with col1:
        ...
        st.dataframe(cov_q.round(1), use_container_width=True)
    ...
    with col2:
        ...
        st.dataframe(ih_q.round(1), use_container_width=True)
...
with tabs[4]:
    ...
    st.dataframe(suf.round(1), use_container_width=True)
...
with tabs[5]:
    ...
    if gap_df.empty:
        ...
        st.dataframe(gap_df[avail_cols].head(100), use_container_width=True)
        ...
        export_df['source'] = 'Linguistic Topology Analyzer V3'
...
# ── TAB 8: VALIDATION AGENT ───────────────────────────────────────────────
with tabs[7]:
    st.markdown('<div class="section-header">🔍 IEP Validation Agent — Literature Evidence for Gap Candidates</div>', unsafe_allow_html=True)
    st.markdown("""
    Select up to 20 gap candidates. The Validation Agent will:
    - Find **peer-reviewed evidence** linking each word to INT, AFF, ACT, or **FUNCTION**
    - Show **how the word was actually used** in your SYN-IQ response data
    - Return a **confidence score** and **citation** for each word
    - Flag FUNCTION words — structural scaffolding excluded from IEP V4 scoring
    - Flag where literature **conflicts** with the gap analysis suggestion

    **FUNCTION** = temporal/structural scaffolding words (months, years, first, within, rather...)
    These feed the **parallel IEP V4 pipeline** — IEP V3 is unchanged.
    """)

    api_key = st.text_input("Anthropic API Key", type="password",
                             placeholder="sk-ant-...",
                             help="Required to run validation searches")

    # Load gap candidates
    with st.spinner("Loading gap candidates..."):
        val_gap_df = compute_gap_candidates(fdf)

    if val_gap_df.empty:
        st.warning("No gap candidates found — upload CSV data first.")
    else:
        # Word selector
        top_words = val_gap_df.head(50)['word'].tolist()
        selected_words = st.multiselect(
            "Select words to validate (recommended: top 20 by candidate score)",
            options=top_words,
            default=top_words[:10],
            max_selections=20
        )

        if selected_words and api_key:
            if st.button("🔍 Run Validation Agent", type="primary"):
                ...
                for i, word in enumerate(selected_words):
                    status.markdown(f"**Validating:** `{word}` ({i+1}/{len(selected_words)})")
                    progress.progress((i+1) / len(selected_words))

                    # Get word metadata from gap_df
                    word_row = val_gap_df[val_gap_df['word']==word].iloc[0]
                    suggested_dim = word_row['suggested_dim']
                    avg_int = word_row['avg_int']
                    avg_aff = word_row['avg_aff']
                    avg_act = word_row['avg_act']
                    questions = word_row['questions_str']
                    freq = word_row['frequency']

                    # Get example sentences from response text
                    example_sentences = []
                    pattern = re.compile(r'[^.!?]*\b' + re.escape(word) + r'\b[^.!?]*[.!?]', re.IGNORECASE)
                    for _, row in fdf.iterrows():
                        matches = pattern.findall(str(row.get('response_text','')))
                        for m in matches[:2]:
                    ...
                    examples_text = "\n".join([
                        f"  - [{s['question']} / {s['depth']}]: \"{s['sentence'][:150]}\""
                        for s in example_sentences[:5]
                    ])

                    prompt = f"""You are a scientific lexicon validator for the IEP (Intellectual-Affective-Action) framework used in AI communication research.

The IEP framework classifies language into FOUR categories:

SEMANTIC REGISTER WORDS (enter IEP dictionary):
- INT (Intellectual): analytical, logical, reasoning, knowledge-based language. Examples: analyze, evidence, hypothesis, logical, conclude
- AFF (Affective): emotional, empathic, feeling-based language. Examples: grief, comfort, fear, empathy, sadness
- ACT (Action/Pragmatic): solution-oriented, behavioral, implementation language. Examples: implement, strategy, deploy, advocate, reform

STRUCTURAL SCAFFOLDING (excluded from IEP scoring — parallel FUNCTION layer):
- FUNCTION: words that organize discourse temporally or structurally but carry NO intrinsic semantic register.
  Subcategories:
  * Temporal scaffolding: months, years, days, recently, currently, soon, first, then, next
  * Structural connectors: rather, specific, response, question, given, within, based, several
  * Generic intensifiers: significant, important, major, various, certain, particular
  * Meta-linguistic: example, context, approach, process, point, aspect, factor
  FUNCTION words appear frequently in responses because they structure discourse, NOT because they express INT/AFF/ACT meaning.
  If confidence for any semantic dimension is below 70%, classify as FUNCTION.

WORD TO VALIDATE: "{word}"

DATA CONTEXT:
- Appears {freq} times across these questions: {questions}
- IEP context scores: INT={avg_int:.1f}%, AFF={avg_aff:.1f}%, ACT={avg_act:.1f}%
- Suggested dimension: {suggested_dim}

EXAMPLE SENTENCES FROM ACTUAL AI RESPONSES:
{examples_text if examples_text else "  (no examples extracted)"}

YOUR TASK — respond in this EXACT format:

DIMENSION: [INT/AFF/ACT/FUNCTION]
FUNCTION_SUBCATEGORY: [temporal/structural/intensifier/meta-linguistic/NA]
CONFIDENCE: [0-100]%
LITERATURE_EVIDENCE: [1-2 sentences citing real peer-reviewed research that supports this classification. Include author, year, journal if possible.]
USAGE_PATTERN: [1-2 sentences describing HOW this word is used in the example sentences — what role does it play linguistically?]
CONFLICT: [YES/NO — does the literature conflict with the suggested dimension?]
CONFLICT_NOTE: [If YES, explain briefly. If NO, write "None."]
VALIDATION_SUMMARY: [1 sentence suitable for a methods section of a paper.]"""
                    ...
                    try:
                        ...
                        dimension = extract_field(response_text, 'DIMENSION')
                        func_subcat = extract_field(response_text, 'FUNCTION_SUBCATEGORY')
                        confidence = extract_field(response_text, 'CONFIDENCE')
                        literature = extract_field(response_text, 'LITERATURE_EVIDENCE')
                        usage = extract_field(response_text, 'USAGE_PATTERN')
                        conflict = extract_field(response_text, 'CONFLICT')
                        conflict_note = extract_field(response_text, 'CONFLICT_NOTE')
                        summary = extract_field(response_text, 'VALIDATION_SUMMARY')

                        results.append({
                            'word': word,
                            'suggested_dim': suggested_dim,
                            'validated_dim': dimension,
                            'function_subcategory': func_subcat,
                        ...
                        results.append({
                            ...
                            'confidence': '0%',
                            'literature_evidence': f'API error: {str(e)[:100]}',
                            'usage_pattern': '',
                            'conflict': 'UNKNOWN',
                            'conflict_note': '',
                            'validation_summary': '',
                ...
                # Display results
                st.markdown("---")
                st.markdown(f"### ✅ Validation Complete — {len(results)} words")

                conflicts = [r for r in results if r['conflict']=='YES']
                confirmed = [r for r in results if r['validated_dim'] in ['INT','AFF','ACT']]
                function_words = [r for r in results if r['validated_dim']=='FUNCTION']

                col1, col2, col3, col4 = st.columns(4)
                with col1:
                    st.markdown(f'<div class="metric-card"><div class="val">{len(confirmed)}</div><div class="lbl">IEP Confirmed</div></div>', unsafe_allow_html=True)
                with col2:
                    st.markdown(f'<div class="metric-card"><div class="val">{len(function_words)}</div><div class="lbl">FUNCTION (excluded)</div></div>', unsafe_allow_html=True)
                with col3:
                    st.markdown(f'<div class="metric-card"><div class="val">{len(conflicts)}</div><div class="lbl">Conflicts</div></div>', unsafe_allow_html=True)
                with col4:
                    errors = [r for r in results if r['validated_dim']=='ERROR']
                    st.markdown(f'<div class="metric-card"><div class="val">{len(errors)}</div><div class="lbl">Errors</div></div>', unsafe_allow_html=True)

                # Per-word display
                for r in results:
                    conflict_color = "#ff4444" if r['conflict']=='YES' else "#44ff88"
                    conf_color = "#7eb8ff"
                    border_color = "#ff4444" if r['conflict']=='YES' else ("#444444" if r['validated_dim']=='FUNCTION' else "#1a4a1a")

                    dim_colors = {'INT':'#4488ff','AFF':'#ff6688','ACT':'#44ff88','FUNCTION':'#888888','ERROR':'#ff4444'}
                    dim_color = dim_colors.get(r['validated_dim'], '#888888')

                    st.markdown(f"""
                    <div style="background:#0a0f1a; border:1px solid {border_color}; border-radius:10px;
                         padding:1.2rem; margin:0.8rem 0;">
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.8rem;">
                            <span style="font-family:'JetBrains Mono',monospace; font-size:1.2rem;
                                  color:#ffffff; font-weight:700;">"{r['word']}"</span>
                            <span style="font-family:'JetBrains Mono',monospace; font-size:0.85rem;">
                                <span style="color:#8890cc;">suggested: </span>
                                <span style="color:{dim_colors.get(r['suggested_dim'],'#888')};">{r['suggested_dim']}</span>
                                <span style="color:#8890cc;"> → validated: </span>
                                <span style="color:{dim_color}; font-weight:700;">{r['validated_dim']}</span>
                                <span style="color:{conf_color};"> {r['confidence']}</span>
                            </span>
                        </div>
                        <div style="color:#aabbcc; font-size:0.82rem; margin-bottom:0.5rem;">
                            <span style="color:#6677aa;">📚 Literature: </span>{r['literature_evidence']}
                        </div>
                        <div style="color:#aabbcc; font-size:0.82rem; margin-bottom:0.5rem;">
                            <span style="color:#6677aa;">🔤 Usage: </span>{r['usage_pattern']}
                        </div>
                        {"<div style=\'color:#888888; font-size:0.82rem; margin-bottom:0.5rem;\'>🔧 FUNCTION — " + r.get('function_subcategory','') + " scaffolding — excluded from IEP V3 scoring, enters parallel FUNCTION layer</div>" if r['validated_dim']=='FUNCTION' else ""}
                        {"<div style=\'color:#ff8888; font-size:0.82rem; margin-bottom:0.5rem;\'>⚠️ CONFLICT: " + r['conflict_note'] + "</div>" if r['conflict']=='YES' else ""}
                        <div style="color:#88aacc; font-size:0.78rem; font-style:italic; border-top:1px solid #1a2a3a;
                    ...
                    # Show example sentences in expander
                    if r['example_sentences']:
                        with st.expander(f"📝 Example sentences for '{r['word']}'"):
                            st.markdown(f"```\n{r['example_sentences']}\n```")
                ...
                export_cols = ['word','suggested_dim','validated_dim','function_subcategory','confidence',
                               'literature_evidence','usage_pattern','conflict',
                               'conflict_note','validation_summary','frequency','questions']
                val_export = pd.DataFrame(results)[export_cols]
                val_export['export_timestamp'] = datetime.now().strftime('%Y-%m-%d %H:%M')

                csv_val = val_export.to_csv(index=False).encode('utf-8')
                st.download_button(
                    label="⬇️ Download Validation Report CSV",
                    data=csv_val,
                    file_name=f"iep_validation_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                    mime='text/csv'
                )
                st.caption("INT/AFF/ACT → Dictionary Builder V2 (IEP V3 unchanged) · FUNCTION → parallel IEP V4 exclusion list · Reviewer-ready evidence trail")

        elif selected_words and not api_key:
            st.info("Enter your Anthropic API key above to run the Validation Agent")
        elif not selected_words:
            st.info("Select words from the dropdown above to validate")

# Footer
st.markdown("---")
st.markdown(f"""
<div style="text-align:center; color:#444488; font-family:'JetBrains Mono',monospace; font-size:0.75rem; padding:1rem;">
    SYN-IQ Linguistic Topology Analyzer V3 · Tennessee 🎹 CUZ Partnership · SYNINT March 2026<br>
    Linguistic topology · IEP Gap Export · Literature Validation Agent → Dictionary Builder V2
</div>
""", unsafe_allow_html=True)
