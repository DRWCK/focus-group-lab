"""
Focus Group Lab V41 — Research Edition
...
st.set_page_config(
    page_title="Focus Group Lab V41",
    page_icon="🧬",
    layout="wide",
...
def export_to_markdown() -> str:
    md  = f"# Focus Group Lab V41 — Session Export\n"
    ...
    if st.session_state.round1_responses:
        ...
        for agent, response in st.session_state.round1_responses.items():
            md += f"### {AGENT_EMOJIS.get(agent,'🤖')} {agent}\n{response}\n\n---\n\n"
    md += "\n---\n*Focus Group Lab V41 — Research Edition · SYNINT Team · April 2026*\n"
    return md
...
if session_type == "🔬 Auto Run":
    ...
    if run_auto_btn:
        ...
        experiment_run_id = datetime.now().strftime('%Y%m%d_%H%M%S') + "_V41"
    ...
    if st.session_state.pull_aside_active:
        ...
        with col3:
            if st.button("📥 Export", use_container_width=True):
                ...
                _rows = []
                _exp_run_id = datetime.now().strftime('%Y%m%d_%H%M%S') + "_V41_livedisc"
