"""
SYN-IQ Self-Model Harvester V1
Architecture Probe Experiment — 40 Questions Across 4 AI Architectures

PURPOSE: Harvest AI responses to architecture-revealing self-model questions.
         Tests boundary condition of η²≈.953 question-attractor effect:
         do self-referential probes reorganize topology by AGENT instead of QUESTION?
...
V1 FEATURES:
  - Question set selector (by author or all 40)
  - Full V50 IEP pipeline (1,897-term dictionary + VADER + FK + TTR)
  - NATIVE condition only (architecture as the only variable)
  - Live transcript with IEP scoring
  - Claude topology analyzer (architecture-aware system prompt)
  - CSV + JSON export — Mapper-ready
  - Agent η² vs Question η² comparison (the boundary condition test)
...
<div class="main-header">
    <h1>🧠 SYN-IQ Self-Model Harvester V1</h1>
    <p class="subtitle">Architecture Probe · 40 Questions · 4 AI Architectures · IEP Topology</p>
    <p class="subtitle">Boundary Condition Test: Does agent η² overtake question η² on self-referential probes?</p>
...
with st.sidebar:
    ...
    # --- DEPTH + RUNS ---
    st.markdown("#### 📊 Depth")
    depth = st.selectbox("Response depth", ["Shallow", "Medium", "Deep"], index=1)
    ...
    total_calls = n_questions * len(selected_agents) * num_runs
    est_min = total_calls * (pause_seconds + 15) / 60
    st.markdown(f"**Questions:** {n_questions}")
    st.markdown(f"**Total calls:** {total_calls}")
    st.markdown(f"**Est. time:** ~{est_min:.0f} min")

    st.markdown("---")
    st.markdown("<div style='text-align:center;color:#6b7280;font-size:0.72rem;font-family:JetBrains Mono,monospace;'>SYNINT Team<br>Tennessee 🎹 CUZ</div>", unsafe_allow_html=True)
...
def build_task_list():
    tasks = []
    max_tokens = DEPTH_CONFIGS[st.session_state.depth]["max_tokens"]
    if selected_set == "All 40":
        questions_flat = {}
        for set_name, s in QUESTION_SETS.items():
            if set_name == "Composite":
...
if st.session_state.running:
    ...
    <div style="font-size:0.78rem;color:#888;margin-bottom:2px;">
        ...
        &nbsp;|&nbsp; Run {r["run"]} &nbsp;|&nbsp; {r.get("latency_ms",0):.0f}ms
    </div>
...
if st.session_state.results:
    ...
    for _, row in browse_df.head(20).iterrows():
        ...
        with st.expander(f"{AGENT_EMOJIS.get(row['agent'], '🔵')} {row['agent']} | {row['question_id']} | Run {row['run']} | INT:{row['int_pct']:.0f}% AFF:{row['aff_pct']:.0f}% ACT:{row['act_pct']:.0f}%"):
            st.markdown(f"**Question:** {row['question_text']}")
            st.markdown("---")
            st.markdown(row["response_text"])
            st.markdown(f"<div class='mono'>INT:{row['int_pct']}% | AFF:{row['aff_pct']}% | ACT:{row['act_pct']}% | VADER:{row['vader_compound']} | FK:{row['flesch_kincaid']} | TTR:{row['ttr']} | {row['total_words']}w | {row['latency_ms']:.0f}ms</div>", unsafe_allow_html=True)
    ...
    export_cols = ["turn_id", "run", "agent", "temperature", "depth", "question_id", "question_text",
                   "question_set", "anchor", "int_pct", "aff_pct", "act_pct", "total_words",
                   "vader_compound", "vader_pos", "vader_neg", "vader_neu",
