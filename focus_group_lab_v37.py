...
st.set_page_config(
    page_title="Focus Group Lab V37",
    page_icon="🧬",
    layout="wide",
    ...
    try:
        if name.endswith('.docx'):
            ...
            if name.endswith('.pdf'):
                try:
                    import zipfile
                    # PDFs aren't zips — just try text extraction
                    raw = uploaded_file.read()
                    # Try to extract text between stream markers
                    import re as _re
                    text_parts = _re.findall(rb'BT(.*?)ET', raw, _re.DOTALL)
                    if text_parts:
                        decoded = []
                        for part in text_parts:
                            chars = _re.findall(rb'\((.*?)\)', part)
                            for c in chars:
                                try: decoded.append(c.decode('latin-1'))
                                except: pass
                        result = ' '.join(decoded)
                        if len(result.strip()) > 50:
                            return result
                    return "[PDF loaded — for best results convert to .txt or .docx first. Raw text extraction may be incomplete.]"
...
def check_password():
    ...
    <div class="main-header">
        <h1>🧬 Focus Group Lab <span class="v37-badge">V37</span></h1>
...
def export_to_markdown() -> str:
    md  = f"# Focus Group Lab V37 — Session Export\n"
    ...
    md += f"- **Polarity:** {st.session_state.polarity} | **Depth:** {st.session_state.depth} | **Evaluation:** {st.session_state.evaluation} | **Compression:** {st.session_state.compression}\n"
    ...
    if st.session_state.round1_responses:
        ...
        for agent, response in st.session_state.round1_responses.items():
            md += f"### {AGENT_EMOJIS.get(agent,'🤖')} {agent}\n{response}\n\n---\n\n"
    md += "\n---\n*Focus Group Lab V37 — Research Edition · SYNINT Team · April 2026*\n"
    return md
...
<div style="text-align:center; color:#666; padding:1rem;">
    <strong>Focus Group Lab V37</strong> — Research Edition<br>
