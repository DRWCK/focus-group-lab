"""
Focus Group Lab V42.3 — Research Edition
...
st.set_page_config(
    page_title="Focus Group Lab V42.3",
    page_icon="🧬",
    layout="wide",
...
def export_to_markdown() -> str:
    md  = f"# Focus Group Lab V42.3 — Session Export\n"
    ...
    if st.session_state.round1_responses:
        ...
        for agent, response in st.session_state.round1_responses.items():
            md += f"### {AGENT_EMOJIS.get(agent,'🤖')} {agent}\n{response}\n\n---\n\n"
    md += "\n---\n*Focus Group Lab V42.3 — Research Edition · SYNINT Team · April 2026*\n"
    return md
        ...
        with cols[i % 2]:
            ...
            if st.session_state.get("iep_highlight", False):
                st.markdown(
                    ...
                    + iep_highlight_html(responses[agent]),
                    unsafe_allow_html=True)
            else:
...
if session_type == "🔬 Auto Run":
    ...
    if run_auto_btn:
        ...
        experiment_run_id = datetime.now().strftime('%Y%m%d_%H%M%S') + "_V42_3"
