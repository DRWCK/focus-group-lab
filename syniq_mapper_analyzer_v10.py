"""
SYN-IQ Mapper Analyzer V10 — Streamlit App
Topological Data Analysis for AI Response Profiles

FEATURES:
  - KeplerMapper topology with Plotly publication-grade visualization
  - IEP Dictionary V3 (1,897 words) fully embedded
  - Topology statistics: nodes, edges, connected components, largest component
  - AI analysis: Exploratory Analysis + Full Dual Report
  - DOCX export: topology stats + node analysis + AI report + conversation
  - White background, node size by member count, vibrant colors
  - Pre-scored IEP columns (default) or live scoring from raw text
...
# =============================================================================
st.set_page_config(
    page_title="SYN-IQ Mapper Analyzer V10",
    page_icon="🗺️",
    layout="wide"
)
...
<div class="main-header">
    <h1>🗺️ SYN-IQ Mapper Analyzer V10</h1>
    ...
    <p class="subtitle">KeplerMapper + IEP Framework &nbsp;|&nbsp;
        ...
        &nbsp;|&nbsp;
        <span class="dict-badge">🆕 V10 — Multi-file combined mapper · Dual regional+factorial report · Exact overlap allocation · Chat questions about your topology!</span>
    </p>
</div>
""", unsafe_allow_html=True)
...
def build_dual_report_prompt(analysis, df):
    """
    V10 dual-report: Part A = topological regions, Part B = factorial interpretation.
...
- Most important findings and standout patterns with specific node IDs and IEP values."""
...
def build_topology_context(analysis, df, hypothesis_guided=False):
    ...
    mode_str = "Exploratory mode — no assumed shape. Describe what the data shows."
    return (
...
feature_options = [
    "Pre-scored columns (recommended for V48 data)",
    "Score from text (no pre-scored columns)",
...
if "pre-scored" in use_iep:
    st.sidebar.success("✅ Using pre-scored IEP columns.")
...
# Analysis always runs in exploratory mode — no pre-loaded hypothesis
analysis_mode = "🔬 Blind (no hypothesis)"
    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 Run Mapper Analysis", type="primary"):
            ...
            with st.spinner("Generating visualization..."):
                ...
                # V10: WHITE BACKGROUND
                for dark in ['#000000','#000','black','#111111','#1a1a1a']:
                    html_content = html_content.replace(
                        f'background-color: {dark}', 'background-color: #ffffff')
                    html_content = html_content.replace(
                        f'background-color:{dark}', 'background-color:#ffffff')
                    html_content = html_content.replace(
                        f'background: {dark}', 'background: #ffffff')
                    html_content = html_content.replace(
                        f'background:{dark}', 'background:#ffffff')

                # V10: NODE SIZE + NAV FIX
                _node_sizes = {str(nid): info["size"] for nid, info in analysis["nodes"].items()}
                _max_sz = max(_node_sizes.values()) if _node_sizes else 1
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('svg').forEach(function(s) {
        ...
        s.setAttribute('style', (s.getAttribute('style')||'') + ';background:#ffffff;');
    });
    ...
    var nodeSizes = """ + _json.dumps(_node_sizes) + """;
    var maxSz = """ + str(_max_sz) + """;

    // Vibrant colorscale — replaces KeplerMapper muted palette
    // Maps 0→1 through: deep blue → cyan → lime → orange → red
    function vibrantColor(t) {
        var stops = [
            [0.00, [0,   71,  171]],  // cobalt blue
            [0.25, [0,   200, 220]],  // bright cyan
            [0.50, [50,  205, 50 ]],  // lime green
            [0.75, [255, 165, 0  ]],  // vivid orange
            [1.00, [220, 20,  60 ]],  // crimson red
        ];
        ...
        var r = Math.round(lo[1][0] + f*(hi[1][0]-lo[1][0]));
        var g = Math.round(lo[1][1] + f*(hi[1][1]-lo[1][1]));
        var b = Math.round(lo[1][2] + f*(hi[1][2]-lo[1][2]));
        return 'rgb('+r+','+g+','+b+')';
    }
    ...
    setTimeout(function() {
        ...
        circles.forEach(function(c, idx) {
            ...
            var titleEl = parent ? parent.querySelector('title') : null;
            var nid = titleEl ? titleEl.textContent.trim() : null;

            // Size by member count
            if (nid && nodeSizes[nid] !== undefined) {
                var sz = nodeSizes[nid];
                var r = 8 + (sz / maxSz) * 28;
                c.setAttribute('r', r.toFixed(1));
            }

            // Vibrant color — use existing fill luminance to map to new palette
            var existingFill = c.getAttribute('fill') || c.style.fill || '';
            var t = total > 1 ? idx / (total - 1) : 0.5;
            // Try to extract hue from existing fill if rgb
            var m = existingFill.match(/rgb\((\d+),\s*(\d+),\s*(\d+)\)/);
            if (m) {
                var brightness = (parseInt(m[1])*299 + parseInt(m[2])*587 + parseInt(m[3])*114) / 1000;
                t = brightness / 255;
            }
            var newColor = vibrantColor(t);
...
"""
                ...
                # Remove KeplerMapper logo
                html_content = html_content.replace(
                    '<img ',
                    '<img style="display:none" data-orig-'
                )
                html_content = html_content.replace('</body>', enhancements + '</body>')

            st.session_state.mapper_results_html = html_content
            st.session_state.mapper_analysis_data = analysis
            st.session_state.mapper_graph_links = graph.get('links', graph.get('edges', []))
        ...
        if st.session_state.mapper_analysis_data is not None:
            ...
            # ── V10: FARZANA'S TOPOLOGY STATS TABLE ─────────────────────────
            st.markdown("### 📐 Topology Statistics")
            import networkx as _nx
            _G = _nx.Graph()
            _G.add_nodes_from(analysis["nodes"].keys())
            if st.session_state.get("mapper_graph_links"):
                for link in st.session_state.mapper_graph_links:
                    _G.add_edge(link[0], link[1])

            _n_nodes = analysis["n_nodes"]
            _n_edges = analysis["n_edges"]
            _components = list(_nx.connected_components(_G)) if _G.number_of_edges() > 0 else [[n] for n in _G.nodes()]
            _n_components = len(_components)
            _largest_component = max(len(c) for c in _components) if _components else 0
            _avg_degree = round(2 * _n_edges / _n_nodes, 2) if _n_nodes > 0 else 0
            _density = round(_nx.density(_G), 4) if _G.number_of_nodes() > 1 else 0

            _stats_df = pd.DataFrame([{
                "Metric": "Number of Nodes",
                "Value": _n_nodes,
                "Description": "Total clusters in the Mapper graph"
            }, {
                "Metric": "Number of Edges",
                "Value": _n_edges,
                "Description": "Connections between overlapping clusters"
            }, {
                "Metric": "Connected Components",
                "Value": _n_components,
                "Description": "Isolated subgraphs (1 = fully connected)"
            }, {
                "Metric": "Largest Component (nodes)",
                "Value": _largest_component,
                "Description": "Size of the dominant connected region"
            }, {
                "Metric": "Average Node Degree",
                "Value": _avg_degree,
                "Description": "Mean edges per node"
            }, {
                "Metric": "Graph Density",
                "Value": _density,
                "Description": "Edges / max possible edges (0=sparse, 1=complete)"
            }])
            ...
            if not api_key:
                ...
                if not st.session_state.chat_active:
                    ...
                    with c1:
                        if st.button("🔬 Analyse Topology", type="primary",
                                     help="Exploratory analysis — Claude describes what it finds with no prior assumptions"):
                            with st.spinner("Analysing topology…"):
                                try:
                                    ...
                                    st.error(f"Error: {e}")
                    with c2:
                        btn_label = "📋 Full Report" if not is_combined else "📋 Combined Full Report"
                ...
                if st.session_state.chat_active and st.session_state.chat_history:
                    ...
                    try:
                        ...
                        def _build_results_docx():
                            doc = _DocxDoc()
                            doc.styles['Normal'].font.name = 'Arial'
                            doc.styles['Normal'].font.size = _Pt(10)

                            # Title
                            t = doc.add_heading('SYN-IQ Mapper Analyzer V10 — Results', level=0)
                            t.alignment = _WDA.CENTER
                            meta = doc.add_paragraph()
                            meta.alignment = _WDA.CENTER
                            for line in [
                                f"Generated: {_dt.now().strftime('%Y-%m-%d %H:%M')}",
                                "Tennessee 🎹 CUZ Partnership · SYNINT",
                            ]:
                                r = meta.add_run(line + "\n")
                                r.font.size = _Pt(9)
                                r.font.color.rgb = _RGB(128,128,128)

                            # Topology Stats
                            _an = st.session_state.mapper_analysis_data
                            if _an:
                                doc.add_heading('Topology Statistics', level=1)
                                tbl = doc.add_table(rows=1, cols=3)
                                tbl.style = 'Light Grid Accent 1'
                                for i, h in enumerate(['Metric','Value','Description']):
                                ...
                                _G2 = _nx2.Graph()
                                _G2.add_nodes_from(_an["nodes"].keys())
                                for lnk in st.session_state.get("mapper_graph_links", []):
                                    if isinstance(lnk, dict):
                                        _G2.add_edge(lnk.get("source",""), lnk.get("target",""))
                                    elif hasattr(lnk, "__len__") and len(lnk) >= 2:
                                        _G2.add_edge(lnk[0], lnk[1])
                                _comps = list(_nx2.connected_components(_G2)) if _G2.number_of_edges()>0 else [[n] for n in _G2.nodes()]

                                stats_rows = [
                                    ("Nodes", str(_an["n_nodes"]), "Total clusters"),
                                    ("Edges", str(_an["n_edges"]), "Connections between clusters"),
                                    ("Connected Components", str(len(_comps)), "Isolated subgraphs"),
                                    ("Largest Component", str(max(len(c) for c in _comps) if _comps else 0), "Size of dominant region"),
                                    ("COLD-only nodes", str(len(_an["cold_only"])), "Nodes with only cold responses"),
                                    ("HOT-only nodes", str(len(_an["hot_only"])), "Nodes with only hot responses"),
                                    ("Mixed nodes", str(len(_an["overlap_nodes"])), "Nodes spanning temperatures"),
                                ]
                                for metric, value, desc in stats_rows:
                                    row = tbl.add_row()
                                ...
                                # Node detail table
                                doc.add_heading('Node Analysis', level=1)
                                ntbl = doc.add_table(rows=1, cols=6)
                                ntbl.style = 'Light Grid Accent 1'
                                for i, h in enumerate(['Node','Size','INT%','AFF%','ACT%','Agents']):
                                    c = ntbl.rows[0].cells[i]
                                    c.text = h
                                    c.paragraphs[0].runs[0].font.bold = True
                                    c.paragraphs[0].runs[0].font.size = _Pt(8)
                                for nid, info in sorted(_an["nodes"].items(), key=lambda x: -x[1]["size"]):
                                    row = ntbl.add_row()
                                    agents_str = ", ".join(f"{a}:{n}" for a,n in info["agents"].items())
                                    for i, v in enumerate([str(nid), str(info["size"]),
                                        f"{info['avg_int']:.1f}", f"{info['avg_aff']:.1f}",
                                        f"{info['avg_act']:.1f}", agents_str]):
                                        row.cells[i].text = v
                                        row.cells[i].paragraphs[0].runs[0].font.size = _Pt(8)

                            # Claude Analysis
                            if st.session_state.get("claude_analysis"):
                                doc.add_page_break()
                                doc.add_heading('AI Topology Analysis', level=1)
                                for line in st.session_state.claude_analysis.split("\n"):
                                    if line.strip():
                                        if line.startswith("## "):
                                            ...
                                            p = doc.add_paragraph()
                                            p.add_run(line.strip()).font.size = _Pt(9)

                            # Chat conversation
                            chat = [m for m in st.session_state.chat_history if m.get("display", True)]
                            if chat:
                                doc.add_page_break()
                                doc.add_heading('Analysis Conversation', level=1)
                                for m in chat:
                                    ...
                                    hr.font.bold = True
                                    hr.font.size = _Pt(9)
                                    hr.font.color.rgb = _RGB(0x1E,0x3A,0x5F) if m["role"]=="user" else _RGB(0x8B,0x45,0x13)
                                    p = doc.add_paragraph()
                                    pr = p.add_run(str(m["content"]))
                                    pr.font.size = _Pt(9)
                                    doc.add_paragraph()
...
**NEW in V4:** Select *"Score from text"* to score raw responses on-the-fly
using the embedded 1,897-word dictionary. **For V48 pipeline data, use "Pre-scored columns" (default) for most accurate results.**
        """)
...
<div style="text-align: center; color: #a0a0a0; padding: 1rem;">
    <strong>SYN-IQ Mapper Analyzer V10</strong><br>
    IEP Dictionary V3 — 1,897 words · Claude AI Analysis · Topology Statistics · DOCX Export<br>
