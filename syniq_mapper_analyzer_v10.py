    ...
    if uploaded_file is not None:
        ...
        if st.button("🚀 Run Mapper Analysis", type="primary"):
            ...
            with st.spinner("Generating visualization..."):
                ...
                # ── White background — replace specific dark hex values ──────
                ...
                html_content = html_content.replace('background-color: black', 'background-color: white')
                html_content = html_content.replace('background: black', 'background: white')

                # ── Remove KeplerMapper logo (target id="logo" only) ─────────
                html_content = re.sub(
                    r'<[^>]+id=["\']logo["\'][^>]*/?>(?:.*?</[^>]+>)?', '',
                    html_content, flags=re.DOTALL
                )

                # ── Publication-quality node colors + scaled sizing via D3 ───
                pub_script = """
<script>
(function() {
  function applyPublicationStyle() {
    var svg = document.querySelector('svg');
    if (!svg) { setTimeout(applyPublicationStyle, 300); return; }

    // Publication color palette (ColorBrewer Set1 — vibrant, print-safe)
    var palette = [
      '#e41a1c','#377eb8','#4daf4a','#984ea3',
      '#ff7f00','#a65628','#f781bf','#999999'
    ];

    // Make edges visible on white background
    svg.querySelectorAll('line, path.link, .link').forEach(function(el) {
      el.style.stroke = '#555555';
      el.style.strokeOpacity = '0.7';
      el.style.strokeWidth = '1.5px';
    });

    // Scale nodes by member count, color by index
    var circles = svg.querySelectorAll('circle');
    var counts = [];
    circles.forEach(function(c) {
      var title = c.parentNode ? c.parentNode.querySelector('title') : null;
      var txt = title ? title.textContent : '';
      var m = txt.match(/\\b(\\d+)\\s+members?\\b/i) || txt.match(/n\\s*=\\s*(\\d+)/i) || txt.match(/^(\\d+)$/);
      counts.push(m ? parseInt(m[1]) : 1);
    });
    ...
    circles.forEach(function(c, i) {
      ...
      c.style.fill = palette[i % palette.length];
    ...
    if (rect) { rect.style.fill = '#ffffff'; }
  }
  setTimeout(applyPublicationStyle, 600);
...
"""
        ...
        if st.session_state.mapper_analysis_data is not None:
            ...
            try:
                ...
                G_nx = nx.Graph()
                G_nx.add_nodes_from(analysis["nodes"].keys())
                # Reconstruct edges from stored graph — use node member overlap
                # (edges already counted in analysis["n_edges"] via graph['links'])
                _stored_graph = st.session_state.get("stored_graph", None)
                if _stored_graph is not None:
                    for src, targets in _stored_graph["links"].items():
                        for tgt in targets:
                            G_nx.add_edge(src, tgt)
                else:
                    # Fallback: infer edges from shared members via node IDs in analysis
                    pass
