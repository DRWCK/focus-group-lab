...
with st.sidebar:
    ...
    # Show key status (not the actual keys)
    with st.expander("🔑 API Key Status", expanded=False):
        st.write("✅ Anthropic (Claude)" if keys.get("anthropic") else "❌ Anthropic")
        st.write("✅ OpenAI (Sophia/Lyra)" if keys.get("openai") else "❌ OpenAI")
        st.write("✅ xAI (Grok)" if keys.get("xai") else "❌ xAI")
        st.write("✅ Google (Gemini)" if keys.get("google") else "❌ Google")
...
if not st.session_state.session_started:
    ...
    ### Setup:
    1. Select agents
    2. Set order (can change anytime!)
