"""
SYNERGISTIC INTELLIGENCE FOCUS GROUP — V7
Full Featured: Configurable Order + Individual Asks + Auto-Clear
Patent Pending — Tennessee 🎹

LAUNCH: cd ~/Desktop && python3 -m streamlit run focus_group_lab_v7.py
"""
...
st.set_page_config(page_title="Focus Group V7", page_icon="🎹", layout="wide")
...
def call_agent(agent_name, messages, keys, prompts):
    system = prompts.get(agent_name, DEFAULT_PROMPTS.get(agent_name, "You are a helpful assistant."))
    if agent_name == "Claude":
        ...
        return call_gemini(messages, keys.get("gemini", ""), system)
...
def format_messages(conversation):
    ...
    for entry in conversation:
        if entry["speaker"] == "SYSTEM":
            continue
        role = "user" if entry["speaker"] in ["CONDUCTOR", "NOTE"] else "assistant"
        messages.append({"role": role, "content": f"[{entry['speaker']}]: {entry['content']}"})
    return messages
...
<div class="main-header">
    <h1>🎹 Synergistic Intelligence Focus Group</h1>
    <p>V7 — Configurable Order + Individual Asks + Auto-Clear</p>
</div>
""", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with st.expander("🔑 API Keys", expanded=not st.session_state.session_started):
        anthropic_key = st.text_input("Anthropic (Claude)", type="password", key="api_anthropic")
        openai_key = st.text_input("OpenAI (Sophia/Lyra)", type="password", key="api_openai")
        xai_key = st.text_input("xAI (Grok)", type="password", key="api_xai")
        gemini_key = st.text_input("Google (Gemini)", type="password", key="api_gemini")
    
    keys = {"anthropic": anthropic_key, "openai": openai_key, "xai": xai_key, "gemini": gemini_key}
    ...
    with col1:
        use_claude = st.checkbox("🟤 Claude", value="Claude" in st.session_state.active_agents)
        use_sophia = st.checkbox("🟢 Sophia", value="Sophia" in st.session_state.active_agents)
        use_grok = st.checkbox("🔴 Grok", value="Grok" in st.session_state.active_agents)
    with col2:
        use_gemini = st.checkbox("🔵 Gemini", value="Gemini" in st.session_state.active_agents)
        use_lyra = st.checkbox("🟣 Lyra", value="Lyra" in st.session_state.active_agents)
    
    # Update active agents
    new_active = []
    if use_claude: new_active.append("Claude")
    if use_sophia: new_active.append("Sophia")
    if use_grok: new_active.append("Grok")
    if use_gemini: new_active.append("Gemini")
    if use_lyra: new_active.append("Lyra")
    st.session_state.active_agents = new_active
    
    st.markdown("---")
    
    # ORDER CONFIGURATION
    st.subheader("📋 Response Order")
    
    # Get current order for active agents only
    active_in_order = [a for a in st.session_state.agent_order if a in st.session_state.active_agents]
    # Add any active agents not yet in order
    for a in st.session_state.active_agents:
        if a not in active_in_order:
            active_in_order.append(a)
    
    # Display current order
    st.markdown('<div class="order-box">', unsafe_allow_html=True)
    st.write("**Current Order:**")
    order_str = " → ".join([f"{AGENT_EMOJIS.get(a, '💬')} {a}" for a in active_in_order])
    st.write(order_str if order_str else "No agents selected")
    st.markdown('</div>', unsafe_allow_html=True)
    
    # Reorder controls
    if len(active_in_order) > 1:
        st.write("**Change Order:**")
        col1, col2 = st.columns(2)
        with col1:
            agent_to_move = st.selectbox("Move:", active_in_order, key="move_select")
        with col2:
            positions = list(range(1, len(active_in_order) + 1))
            current_pos = active_in_order.index(agent_to_move) + 1 if agent_to_move in active_in_order else 1
            new_pos = st.selectbox("To position:", positions, index=current_pos-1, key="pos_select")
        ...
        if st.button("🔄 Apply New Order"):
            if agent_to_move in active_in_order:
                ...
                # Update full order (preserve inactive agents at end)
...
if not st.session_state.session_started:
    ...
    *Synergistic Intelligence Focus Group — Patent Pending — Tennessee 🎹*
    ...
    # INPUT - Uses counter to force clear
    ...
    # BUTTONS ROW 1: Ask All + Add Note
...
# FOOTER
st.markdown("---")
st.markdown('<div style="text-align:center;color:#666;"><em>Synergistic Intelligence Focus Group V7 — Patent Pending — Tennessee 🎹</em></div>', unsafe_allow_html=True)
