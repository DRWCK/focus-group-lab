"""
SYN-IQ CSV Mapper Analyzer V6
CSV-Native Topological Analysis — Hardcoded Native Baselines Edition
...
PURPOSE: Ingest V48/V50 harvester CSVs directly. Compute IEP topology,
         ...
         control experiment (Dirichlet), and export clean data for
         TDA pipeline.

V6 CHANGES (from V5):
- Hardcoded NATIVE baselines per agent × question (N=20 each, March 2026)
  No NATIVE condition rows needed in uploaded CSV — baselines are permanent
- Section 3 now compares every condition against agent × question specific
  NATIVE anchor — not a pooled mean
- Delta table shows precise per-question displacement from true baseline
- Removed all collaborator name references from code and UI
...
<div class="main-header">
    <h1>🔬 SYN-IQ CSV Mapper Analyzer V6</h1>
    <p class="subtitle">CSV-Native · IEP Topology · Hardcoded Native Baselines · TDA Export</p>
...
AGENTS = ["Claude", "ChatGPT", "Gemini", "Grok"]
...
# =============================================================================
# HARDCODED NATIVE BASELINES — N=20 per agent × question, March 2026
# Source: mapper_all_20260302_V48 runs, all NATIVE condition
# Claude: mapper_all_20260302_165519_V48
# ChatGPT: mapper_all_20260302_165733_V48
# Gemini: mapper_all_20260302_173638_V48
# Grok:   mapper_all_20260302_173507_V48
# =============================================================================
NATIVE_BASELINES = {
    "Claude": {
        "CONSCIOUSNESS":    {"INT": 42.60, "AFF": 43.85, "ACT": 13.56},
        "GRIEF":            {"INT": 24.92, "AFF": 56.05, "ACT": 19.03},
        "LEAVE_JOB":        {"INT": 31.99, "AFF": 27.02, "ACT": 40.98},
        "LIARS_PARADOX":    {"INT": 77.40, "AFF":  7.32, "ACT": 15.28},
        "RURAL_HEALTHCARE": {"INT": 22.36, "AFF": 14.41, "ACT": 63.24},
    },
    "ChatGPT": {
        "CONSCIOUSNESS":    {"INT": 49.62, "AFF": 30.29, "ACT": 20.08},
        "GRIEF":            {"INT": 21.99, "AFF": 62.46, "ACT": 15.55},
        "LEAVE_JOB":        {"INT": 35.00, "AFF": 29.22, "ACT": 35.76},
        "LIARS_PARADOX":    {"INT": 82.42, "AFF":  8.10, "ACT":  9.48},
        "RURAL_HEALTHCARE": {"INT": 27.71, "AFF": 13.26, "ACT": 59.02},
    },
    "Gemini": {
        "CONSCIOUSNESS":    {"INT": 47.43, "AFF": 33.99, "ACT": 18.58},
        "GRIEF":            {"INT": 22.73, "AFF": 62.46, "ACT": 14.80},
        "LEAVE_JOB":        {"INT": 29.70, "AFF": 31.02, "ACT": 39.27},
        "LIARS_PARADOX":    {"INT": 80.65, "AFF":  4.31, "ACT": 15.03},
        "RURAL_HEALTHCARE": {"INT": 23.85, "AFF": 13.26, "ACT": 62.89},
    },
    "Grok": {
        "CONSCIOUSNESS":    {"INT": 34.98, "AFF": 48.32, "ACT": 16.70},
        "GRIEF":            {"INT": 18.91, "AFF": 65.34, "ACT": 15.76},
        "LEAVE_JOB":        {"INT": 31.82, "AFF": 33.72, "ACT": 34.44},
        "LIARS_PARADOX":    {"INT": 79.27, "AFF":  7.40, "ACT": 13.34},
        "RURAL_HEALTHCARE": {"INT": 25.86, "AFF": 15.93, "ACT": 58.24},
    },
}

def get_native_baseline(agent, question_id):
    """Return hardcoded NATIVE baseline for agent × question. Falls back to pooled if not found."""
    agent_baselines = NATIVE_BASELINES.get(agent, {})
    if question_id in agent_baselines:
        return agent_baselines[question_id]
    # Fallback: pool all questions for this agent
    if agent_baselines:
        INT = round(sum(v["INT"] for v in agent_baselines.values()) / len(agent_baselines), 2)
        AFF = round(sum(v["AFF"] for v in agent_baselines.values()) / len(agent_baselines), 2)
        ACT = round(sum(v["ACT"] for v in agent_baselines.values()) / len(agent_baselines), 2)
        return {"INT": INT, "AFF": AFF, "ACT": ACT}
    return None
...
def build_tda_export(df):
    """
    Build clean export for TDA pipeline.
            ...
            if len(pooled_curve) > 0:
                ...
                if f"{target_dim}_sd" in pooled_curve.columns:
                    ...
                    <div class="finding-box {'green' if sd_monotonic else 'yellow'}">
                        ...
                        Point cloud tightens at higher gradient levels — expect denser clusters in persistence diagrams.</em>
                    </div>
                    """, unsafe_allow_html=True)
...
# =============================================================================
# SECTION 3: BASELINE vs GRADIENT COMPARISON (V6 — Agent × Question Specific)
# =============================================================================
st.markdown("---")
st.markdown('<div class="section-label">⚖️ Baseline vs Gradient Comparison</div>', unsafe_allow_html=True)
st.markdown("""
How does each condition shift the IEP profile relative to the **hardcoded NATIVE baseline**?
Baselines are agent × question specific (N=20 each, March 2026) — not pooled averages.
""")

# Show the baseline reference table
with st.expander("📋 Hardcoded NATIVE Baselines (N=20 per agent × question)"):
    bl_rows = []
    for agent, questions in NATIVE_BASELINES.items():
        for q, vals in questions.items():
            bl_rows.append({"Agent": agent, "Question": q,
                            "INT%": vals["INT"], "AFF%": vals["AFF"], "ACT%": vals["ACT"]})
    st.dataframe(pd.DataFrame(bl_rows), use_container_width=True, hide_index=True)
    st.markdown("<div class='mono'>Source: V48 harvester runs, March 2, 2026. N=20 per cell.</div>", unsafe_allow_html=True)

# Compute deltas — agent × question × condition vs hardcoded baseline
agents_in_data = df["agent"].unique().tolist()
questions_in_data = df["question_id"].unique().tolist()
all_conditions_in_data = [c for c in df["condition"].unique() if c != "NATIVE"]

lift_rows = []
for agent in sorted(agents_in_data):
    for q in sorted(questions_in_data):
        baseline = get_native_baseline(agent, q)
        if baseline is None:
            continue
        b_int, b_aff, b_act = baseline["INT"], baseline["AFF"], baseline["ACT"]

        for cond in all_conditions_in_data:
            sub = df[(df["agent"] == agent) & (df["question_id"] == q) & (df["condition"] == cond)]
            if len(sub) == 0:
                continue

            fam = "Baseline"
            for f, conds in CONDITION_FAMILIES.items():
                if cond in [c.upper() for c in conds]:
                    fam = f
                    break
            ...
            lift_rows.append({
                ...
                "NATIVE INT": b_int, "NATIVE AFF": b_aff, "NATIVE ACT": b_act,
            })
...
if lift_rows:
    ...
    with col_f1:
        sel_agent = st.selectbox("Agent", ["All"] + sorted(agents_in_data), key="bl_agent")
    with col_f2:
        sel_q = st.selectbox("Question", ["All"] + sorted(questions_in_data), key="bl_q")
    with col_f3:
        sel_fam = st.selectbox("Family", ["All"] + sorted(lift_df["Family"].unique()), key="bl_fam")

    filtered = lift_df.copy()
    if sel_agent != "All":
        filtered = filtered[filtered["Agent"] == sel_agent]
    if sel_q != "All":
        filtered = filtered[filtered["Question"] == sel_q]
    if sel_fam != "All":
        filtered = filtered[filtered["Family"] == sel_fam]

    display_cols = ["Agent", "Question", "Family", "Condition", "N",
                    "NATIVE INT", "INT%", "INT Δ",
                    "NATIVE AFF", "AFF%", "AFF Δ",
                    "NATIVE ACT", "ACT%", "ACT Δ"]
    st.dataframe(filtered[display_cols], use_container_width=True, hide_index=True)
    st.markdown(f"<div class='mono'>{len(filtered)} rows shown. Δ = condition mean − agent×question NATIVE baseline.</div>", unsafe_allow_html=True)

    # Biggest movers
    st.markdown("**Largest AFF displacements from NATIVE baseline:**")
    lift_df["AFF_delta_num"] = lift_df["aff_pct"] if "aff_pct" in lift_df.columns else lift_df["AFF%"] - lift_df["NATIVE AFF"]
    lift_df["_aff_delta"] = lift_df["AFF%"] - lift_df["NATIVE AFF"]
    top_movers = lift_df.nlargest(5, "_aff_delta")[["Agent","Question","Condition","NATIVE AFF","AFF%","AFF Δ"]]
    st.dataframe(top_movers, use_container_width=True, hide_index=True)
else:
    st.info("Upload gradient condition CSVs to see displacement from NATIVE baselines.")
...
st.markdown("How much do Claude, ChatGPT, Grok, and Gemini diverge from each other per question and condition?")
...
if len(div_df) > 0:
    ...
    <div class="finding-box">
        ...
        Prime target for persistence diagrams.
    </div>
    """, unsafe_allow_html=True)
...
# =============================================================================
# SECTION 5: TDA EXPORT
# =============================================================================
st.markdown("---")
st.markdown('<div class="section-label">📐 TDA Export</div>', unsafe_allow_html=True)
...
with col_exp2:
    ...
    for q in sorted(tda_df["question_id"].unique()):
        q_df = tda_df[tda_df["question_id"] == q]
        st.download_button(
            f"📥 {q}",
...
with st.expander("👁️ Preview TDA Point Cloud"):
    st.dataframe(tda_df.head(20), use_container_width=True, hide_index=True)

# =============================================================================
# SECTION 6: SIMPLEX GEOMETRY CONTROL EXPERIMENT
# Dirichlet(1,1,1) uniform simplex control design
# =============================================================================
st.markdown("---")
st.markdown('<div class="section-label">📐 Simplex Geometry Control Experiment</div>', unsafe_allow_html=True)
st.markdown("""
**Dirichlet(1,1,1) uniform simplex control**
...
if st.session_state.control_result is not None:
    ...
    st.markdown('''<div class="finding-box">
        ...
        <br><br>🔮 <strong>Next controls (roadmap):</strong>
...
if not api_key:
    ...
    quick_qs = [
        ...
        "Which findings here are most important for the persistence diagrams?",
...
- This data will be analyzed using KeplerMapper + persistence diagrams
...
Frame insights in terms of topological analysis goals.
...
<div style="text-align:center;color:#6b7280;padding:1rem;font-family:'JetBrains Mono',monospace;font-size:0.75rem;">
    <strong>SYN-IQ CSV Mapper Analyzer V6</strong><br>
